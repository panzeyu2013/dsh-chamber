...
- **载入停滞的自动重建改走官方 `Session.resync()`，并由页面级座席独占** —— 只有具象 `Session.openPromise` 证明**没有在途 open** 且状态仍是 loading 时才执行，借此原位重建该会话的流而不丢页面（5 分钟内至多 2 次、间隔 ≥15 s），慢宿主不被打断；vendor 头部座位保留 `error` 阶段的迁移与用户主动重建，两个座席共用同一能力守卫，隐藏的头部不再对用户没在看的会话动手。
...
- **通知失败不再是无解的死路（design 19 §3.3/§4.1）** —— `dsh-chamber:notify` 由布尔改为 `{ shown, outcome, error? }`（`outcome` 区分 shown / suppressed / retryable / permanent）：宿主/系统的拒绝原因与裁决侧抑制原因穿过 IPC 保留；新增 `dsh-chamber:open-notification-settings`（固定主进程侧 URL，不接受渲染端传入），设置页据此渲染原因、权限提示与「打开系统设置」按钮（中英）；映射纯函数有单测，Swift 壳把授权裁决与投递失败写入 native-shell.log。
...
- **双 flavor 防漂移锁步与新 CI 腿** —— IPC 面镜像（main/preload 两侧字面量与结构）、桥 manifest（`bridge-manifest.json` ↔ 生成的 Swift 白名单，通道 70 = 61 invoke + 9 push）、注入 shim 表面、core 的 electron-free 传递闭包、打包清单与产物命名（`-native` 不含碰撞、更新 feed 归属唯一）各有独立门禁；新增 macOS CI 腿 `test-macos`（Swift 构建 + XCTest + 打包干跑 + darwin 目录锁与打包脚本套件），发布证明要求 linux/windows/macos 三腿同时通过。
...
- **托管宿主用随包 pnpm 安装/卸载插件** —— Finder 双击的打包 App 里，宿主插件管理器此前在 PATH 解析不出可执行 pnpm 时无法工作：控制面现在于 `<stateDir>/pnpm-shim/` 生成 wrapper 并**只前置到该次宿主 spawn 的 PATH**（design 02 §3.1，`withPnpmShim`）；宿主已有 pnpm 时逐字不动（此时不保证与随包版本一致）。
...
- **原生壳文案本地化并跟随应用内语言（design 25 §5.3）** —— 原生壳的菜单、托盘、对话框与错误文案此前是写死的中文；现在整表（122 键）以 en / zh-Hans 两份字符串资源随包发布，键表由 `NativeTextKey` 单一来源锁步，两份文件的占位符逐键同型同序。壳自建文案跟随**页面语言**（应用内切换即生效），系统与框架面（Sparkle 标准窗、AppKit 内建串、右键菜单）默认跟随系统语言，仅当语言族不同时才写 `AppleLanguages` 让它们一起跟随（下次启动生效）。装配期对每个 `.lproj` 做存在 + 逐字节 + `plutil` 三重 fail-closed 断言，缺件不再出包。

- **共享会话生命周期包与跨语言锁步门（design 14 §D4）** —— 新增零依赖、零时钟的纯状态包 `@dsh-chamber/dsh-stream-state`：载体重建 reducer 与节流表、页面生命周期账本/容器、揭幕仲裁、四条恢复阶梯的**单张阈值表**、有界异步原语（`withDeadline`/`waitForCondition`）、带世代围栏的 load 状态机与对账权威判定，并带 Foundation-only 的 Swift 镜像（`CarrierDecision.swift`/`LoadState.swift`）。api-gateway / renderer / sidebar / mobile / open-in 一律改用这一所有者，阈值只剩一处；锁步门进 static、CI 与发布验证（差分回放 drift≠0 即红、Swift 镜像编译断言、阶梯阈值逐模块比对），并新增**有界流取证环**（默认 256 条 FIFO、记录时脱敏、实时分发前先写入）与页面侧快照桥，无 DevTools 的 Swift 壳里也能事后回读载波决策。
...
- **发布验证腿扩围** —— validation 腿新增远端状态故障注入矩阵与验收矩阵（`blocked` 不判绿、空组为错）、stream-state 跨语言锁步门、包边界/死导出/上游生命周期契约等门；发布提交必须先有 `main` 上 linux+windows+macos 三腿的完整成功运行。

- **设置里的运行时 Web Inspector 开关（design 25 §5.1.1）** —— Swift 原生壳此前只在 `#if DEBUG` 构建里暴露 WKWebView 检查器，正式包在真机上无法调试，而两个 flavor 共用同一 userData 目录锁、换构建代价高。现在「设置 → 通用」的更新组里有一枚显式 opt-in 的调试开关：`chamber-settings` 新增嵌套 `debug.enabled` 块（局部补丁深合并，未知嵌套键与损坏块一律响亮拒绝）；新增 `setDebugMode` 边（缺少或非布尔载荷响亮拒绝），Swift 腿应用 `WKWebView.isInspectable` 并回报实测值、不可应用时带原因；Electron 腿不接线，开关按 `supported.debugInspectable === false` 带理由禁用，而不是摆一个死控件。设置投影携带 `debugRuntime`，每次 sidecar ready 经 `__host.debugModeApplied` 回填，页面显示宿主事实而非请求值；副作用与持久化失败回滚已应用值。偏差 T-10 / S-24 记录 DEBUG 默认值与缺失的 DevTools 菜单项。
### 变更
...
- **原生壳窗口几何与 Electron 视口一致（S-49 关闭）** —— 原生壳内容区从 1280×786 的折中改为取 Electron 的 web 视口 1280×772（Electron 侧保持外框 1280×800），两 flavor 的首屏可用高度不再相差 14pt；实机目检并入高刷批。
...
- **desktop IPC 面** —— 整表写 SSH 实例的 `desktop_ssh_instances_set` 通道端到端移除；连接变更只剩事务化 save/delete 与仅接受 clear 的三个单凭据 setter；`bridge-manifest.json` 与 Swift 白名单继续锁步（本版新增 SSH 清册健康 invoke 与渲染停滞取证 push）。
...
- **内置插件管理入口移入设置分页** —— 插件管理现位于「设置 → 内置插件」，复用官方启用、安装和配置流程；插件开关不再长期占据侧栏。
- **手动恢复控件退役，对话流健康 chip 退为纯报告（design 14 §D4）** —— chip 与失败遮罩上的两枚 chamber 专有控件（整页重载、按会话重建对话通道）一并移除：chip 不再有动作行与按压/双击时间戳，`role=status` 保留，只负责报告；自动恢复路径一条不少（`error` 面的自动 heal、页面级有界阶梯 resync → 实例重启 → 预算内整页重载、`loading` 停放态的 open-stall → resync、journal 看门狗），且未移动一个 heal-failed / loading-stall / loading-failed tick。
- **载波抖动提示退役（design 14 §D4）** —— chip 不再显示 chamber 专有的「对话通道重连中」提示（上游无对应面）：规划器去掉该 notice、`carrierChurn` 观测与其窗口，locale 文案与阶梯表阈值一并删除。重试道不受影响（持续失败的载波仍有界退避重开，不会终局逃逸），fork 继续把 `dsh-chamber:stream-carrier-failed` 作为 diagnostics-only 页面事实发出；代价 = 重开窗口重新静默，页面无法再把「重连中」与「安静」分开。
### 修复
- **SSH 实例清册的半读不再被当作权威空集** —— 连接注册表读取部分失败时，桌面此前会把它投影成一份「完整」清册，渲染端的完成未读账本据此把暂缺来源当作已删除并剪掉其持久记录；现在注册表把降级与缺行事实（`degraded` / `rosterIncomplete` / `droppedCount`）经新的健康通道交给页面，清册不确定期间不剪任何 durable 未读，整文件失败与行级缺行分别记账。
...
- **远端来源点开会话不再停在「加载历史」** —— 首帧期限收敛为**单档 30 s**（加宽阶梯与跨世代预算退役：实测开局帧 14–273 ms、3.65 MB 快照 41 ms，单档已有三个数量级余量），keep-alive 竞态改在微任务队列重排；重试道等活连接世代有 30s 上界（到点按 expired 交回调用方重开而不是抛错）；teardown 路径改走同一静默升级，并要求该流至少活过 15s（会话切换/中断请求/重挂不再误杀健康载波）；open-in 阶梯只对「openPromise 恰为 null 且状态 loading」的停滞自动重建，另有 90s 硬失败文案「会话内容未载入」。
...
- **veil 到外层上限不释放 / 0ms 重臂** —— 揭幕帧改为携带绝对截止：hero/settling 超过 70s 变为可操作（揭布并仍给退出动作），absent/未知相位 2s 回退，未知 data-phase 不再被折叠成 hero 停到 70s；「不挂定时器」只有一个词法出口 `null`（数值哨兵 `0`/`Infinity` 一并删除，类型系统强制调用方处理）。**0 ms 自激修复**：此前 `planVeilTimer` 对无需定时器的帧返回有限数 0，调用方的 `!Number.isFinite` 守卫放它过去并挂出 0 ms 定时器，而其回调写的 tick 正是同一 effect 的依赖、`frameNowMs` 又是每次渲染读钟 ⇒ 自激（打包态实测 33,896 次 `setTimeout`/秒、主线程 48% 在 WebKit 定时器堆、空闲 CPU 96–114%）；App 侧 reveal 保持定时器同形，0 一律判「不挂」。
...
- **归档或删除后的侧栏幽灵会话** —— 每个来源按自己的权威会话列表校验持久化基线与归档清理状态，清除已失效行并保留活跃、新建会话；点击失效行不再把用户弹回初始界面。
- **远端右侧栏终端无法加载** —— 为每实例额外宿主行登记动态 chunk 所有者，终端异步模块在 SSH 等远端来源中也能正确解析和加载。
- **本地实例启动后的初始页面与侧栏显示** —— 修复本地测试实例启动时页面和侧栏未正确恢复的问题。
- **归档清理不再重建上游 registry global 的形状（design 24 §4 step 9 / §11）** —— archiveCleanup 宿主绑定此前按硬编码形状（`initialized`/`workspaceIds`/`archivedSessionIds`）整块重写上游 workspace registry global，而 `dsh-storage-domain` 的 `global.set` 不跑 `schema.parse`，zod 默认值永不回填：dsh 0.1.7 加上 `pinnedSessionIds`/`defaultWorkspaceId` 后，pin 升级后的第一次 purge 就把它们从活状态里丢掉，下一次官方 `archiveSession()` 在 `state.pinnedSessionIds.filter(...)` 上抛错，表现为 `gateway/internal: Cannot read properties of undefined (reading 'filter')`；渲染器重载会保留坏掉的内存状态，只有重启实例经 load-time parse 才自愈。现在改为读-改-写活 global、只拥有 `archivedSessionIds`，上游新增/改名/未来字段一律存活，`pendingMutation` 标记也不再被丢；表面守卫额外要求活 global（缺失即响亮失败），回归门是写前 global 与写入状态的整对象 deepEqual（含未来字段占位）。
- **侧栏恢复 macOS 窗口 chrome 条带（design 05 §2 / design 06）** —— fork 丢掉的上游 ui-sidebar darwin 分支原样恢复：52px `.topStrip` 抵消根 padding 并承载面板开关（`isDarwinDesktop()`）、fullscreen 变体与 logo 行的 -12px 内缩；开关中心 y 25，与折叠态 `shell.leading` 座同高，收展不再跳位，字标不再贴红绿灯下沿。两处有意分歧（不标 `data-window-drag`、fullscreen 分支在壳把窗口状态镜像到 document 前保持不生效）已记入该表；Electron macOS 腿的收口（`hiddenInset` + 顶部带拖拽标记）仍在待办。
- **插件管理页容纳为设置 tab 体（design 05 §5）** —— 上游 `PluginManagerPage` 是主面板整页（居中列 + `clamp(24px,4vw,48px)` 内边距 + 28px 与窗口顶带两行页头 + 自有 h1/intro），复用为「设置 → 内置插件」的 `settings.plugins.tab` 体时内容被内缩居中、标题重复。现在经 chamber 自有 `EmbeddedPluginManagerPage` 包装，按上游自有钩子 `data-plugin-panel`/`data-window-drag` 覆盖为官方兄弟 tab 的体契约（左对齐、内容自撑、对话框唯一滚动），隐去重复的 h1+intro、保留工具栏行；属性缝登记在 `docs/checklists/upstream-touchpoints.md` §3 并按 §7 复核。
- **侧栏 running 位改按官方状态投影解析（design 06 §4.1/§4.3）** —— 此前只信会话列表行的 running 位，官方 ui-workspace nav 却按 `status?.running ?? s.running` 计算：陈旧行会让 running 环与实时事实在官方 UI 已前进后继续留存（反之亦然）。现在单点解析（`session-row-state.ts resolveSessionRunning`）并喂给全部消费者——running 环/实例快照、runtime-facts 通道（完成台账、通知边、未读、徽标）、子代理谱系计数与 run-identity mint；`??` 承载「false 观测必须压过陈旧行 true」。store-repair 面仍读行自身声明，分歧以 `status-divergence` 限频（≤30s 一次）记入有界权威日志；通道从未携带的幻影 `completed` 消费一并移除。
- **聊天尾部在采样 settle 时重新贴底（design 09 §3.6，第三类 vendor 补丁）** —— 500ms 采样在跟随容差（`FOLLOW_THRESHOLD`=24px）内 settle 时旧代码保留残余偏移：直接 bash 调用的 preparing→started 整行替换移动了 scrollport，最新一行半遮在输入框上方、`data-chat-following-tail` 仍置位且没有「回到最新」，直到下一次布局变化才自愈。补丁让 settle 在跟随仍拥有尾部时重新贴底，与 `onScroll` 首分支 / `onResize` 在同一容差内的既有行为一致；接受的取舍 = 24px 以内的真实阅读者微调也会在 settle 时回贴，上游带上修复（首选形状 = `use-chat-viewport.ts` 按意图归因）后删除该条。
- **facts 快照不可判定不再冻结未读完成（design 19 §3.7.1）** —— 蓝点、todo 完成条与 Dock 角标共读同一本 App 账本，此前本地 facts 快照「存在但不可判定」（verdict ≠ ok / serviceable=false / stale）时会把整本账冻结，dots 可以整天空着而通知照常到达；现在 rule 0 只冻结 facts 结论（通道边沿照旧 arm/disarm），「缺席即删除」的清扫要求 listComplete 且有通道，首次播种按来源世代只镜像一次宿主读取水位，绝不拿观察者域时间戳推进已读水位。另加 facts-health 权威日志（每次状态变化一条面包屑、计数进 detail）、chamber 全部监听边界的 never-throw 步守卫，角标 IPC 只在计数变化且成功派发后推送；被取代的通知规划孤岛（planRuntimeNotifications / planFactsNotifications / detectNotificationEdges 等）删除，仍有效的断言迁到活路径。
- **`$events` 观察者载波长连接化（design 19 §3.5）** —— 45s 无帧不再被当死 socket 静默换代（每次换代都关闭 socket、清 baselineTrusted、丢稳定位并整表重发）；载波现在活到真失败，重连走有界 1s→30s 退避（稳定复位需要一帧 ready），同一 socket 上重订阅，基线重采样按世代捕获且下限 250ms。
- **开帧相位机退役，交付即结算（design 14 §D4）** —— 开帧曾以消费者 `accept()` 为唯一结算转移，但 `accept()` 发生在传输层、早于页面应用快照，页面应用时抛出的错误对它不可观测，终局错误又是普通 `Error`、vendor 不写 `error`；会话域另有 90 s 总期限使其在会话打开路径基本不可达。现已整体退役（accept 通道、跨世代预算 `openingBudgetMaxMisses`、`RemoteStreamOpeningBudgetError` 与 `opening-orphaned`/`opening-budget-exhausted` 判词一并删除），reducer 的 `openingAnswered` 改义为「首帧已交付给消费者即结算」，页面侧开帧失败账本与健康 chip 的开帧失败臂同批删除。
- **停驻 loading 的自动档改由共享引擎的 `open-stall` 承担（design 14 §D4）** —— 页面级阶梯只在**没有在途 open**（`openInFlight === false`）的停放 loading 上触发现有有界 resync，且只到 resync 一档、不上探实例重启/整页重载；在飞 open 永不被跨越，慢宿主不被打断；桌面头部只报告、不再执行 `loading` 自动臂。
- **follow 的 cancel 只发往原载波，成功委派释放持有项** —— 控制面此前把 cancel 帧发往当前 socket，即使该 follow 开在已被替换的载波上（后继收到它从未承载的流的 cancel）；成功委派的 waterfall 又永久持有条目与本地 pending 镜像（宿主已把应答客户端移出投递集，不会有 cancel 到达）。现在 follow 记住自己的载波、只在那里取消，成功委派与真实 cancel 一样清掉定时器、条目与 pending 镜像（本地处理是 get/delete，迟到的真 cancel 仍无害）。
- **恢复相位不再进入被采样 state** —— 每秒返回新对象的健康采样让 InstanceView 按采样率重渲染（即使没有任何阈值被跨过）；现在新鲜对象只留在 ref，呈现相位由同一组输入在采样器里派生、state 只存相位，阈值窗内的采样直接 bail out。遮罩可见面逐字不变，导出面与锁同步。
- **测试运行器不再漏记失败** —— 池按声明序打印转写，并在 stop-on-failure 留下的「空洞」处停滞：空洞之后声明的红文件从未被记入 failures，gate 反报成功（用红 `test:mobile` 与红 `spawn-dsh.test.ts` 实测复现）。现在首个失败在完成回调里记录，`--continue` 仍记录全部，转写走有序遍历，遍历够不到时在末尾单独打印该文件自己的输出；新单测钉住空洞形状。
- **原生壳窗口 chrome 与上游 vibrancy 面对齐，整窗可从页面 chrome 行拖动（design 25 §5.5/§5.6）** —— Swift 腿移植上游 pin 的三个 darwin 面：红绿灯组按上游位置（首心 23,25）并在每次标题栏重排后重做（缩放/全屏/跨屏/最小化恢复/标题写入），`html[data-fullscreen]` 按两条窗口通知镜像，页面下加 NSVisualEffectView（.sidebar/.behindWindow/.active）让侧栏磨砂可见、窗口转非不透明、私有透明键不可用时回退；页面侧只有 `data-window-vibrancy` 标记命中时才去掉 sidebar darwin 规则与 renderer 容器的不透明底（Electron darwin 腿观感不变）。拖动面：标记 sidebar 的 `.topStrip`/`.logoRow`（与 ui-theme CHROME_ROWS 同源），documentStart 主 frame 注入把标记行上的按压转成 `dshChamberWindowDrag` 消息并由 AppKit 从当前鼠标位置起拖动环（合成 leftMouseDown + performDrag，窗口不跳）；控件、修饰键点击与页面已处理的事件穿透，交互选择器与 `regions.ts#INTERACTIVE_SELECTOR` 逐项锁步。
- **会话「载入历史」永久停驻的根因修复：WebKit 内建判定被源码文本击穿** —— 实测根因：JavaScriptCore 的 `Function.prototype.toString` 对**内建函数**返回多行文本（`{ [native code] }` 带换行/缩进），而 pin 住的 `dsh-util-values` 把该文本与**单行模板**严格比较 ⇒ 判定在 WebKit 里恒假 ⇒ `snapshotJsonValue` 对普通对象/数组返回 `undefined` ⇒ 页面在 raw chunk 校验（`expandAssistantStream`）处抛普通 `TypeError`，而 vendor `doOpen` 只对远程失败写 `error`、其余原样 rethrow ⇒ 会话**永久停在 loading**、既无判词也无出口（间歇性：raw chunk 只对 block-start/end、usage、finish 出现）。现在**两条载体**都补上空白归一：renderer 构建期的第三类 vendor 补丁（覆盖自建前端），以及 gateway 对代理的实例官方前端（手机 / WebKit 浏览器，无法在此重建）注入同一归一；两条都随上游提案 §9 落地一并删除。
- **未读面与 Dock 角标不再膨胀（W1/W2/W3/W4）** —— 三个已实测的根因一并修掉：①未读派生自重入（React #185）被步骤守卫吞掉后**整拍中止**，`read`/`edge` 因此从未落过一拍（9 天 18 份存储全空、整表武装成「全未读」）⇒ 新增重入闸 `createUnreadStepGate`（重入请求去重后补微任务，派生体永不嵌套）；②完成身份此前由 `Date.now()` 时钟族加一张生产从不相关的跨通道别名表铸成 ⇒ 改为宿主 `turn/end.seq`（`host:turn/<seq>`），时钟族与别名表及其存量键删除，缺失时逐字回退水位族；③0/不可用行水位不再当作「内容位置 0」的未读证据。另加 v5 影子写与启动期等价性对账、64 条只读仪表（判词、门计数、影子等价）。**Dock 角标**改为「写后真读回」：Swift 腿写 `NSApp.dockTile` 并读回 `{count, applied}`，不匹配就持续重放、退出时清意图；关窗不再拒收清 0 写（dockTile 是应用级状态），同值不写；node 侧删除按窗口状态预判拒写的短路口。

## [0.4.0-beta.4] - 2026-09-26
...
- **契约文档与许可清单** —— design 文档状态行去掉日期与「已实现」叙述，新增 design 17 §10.7/§12、design 21 undo 与栅栏、design 18 §7 产物纪律、design 02 §3.1、design 25 §6.3/§7，并在 11 篇设计文档补 Rejected alternatives；第三方许可清单按当前树再生成（38 个包）。
- **内置插件管理入口移入设置分页** —— 插件管理现位于「设置 → 内置插件」，复用官方启用、安装和配置流程；插件开关不再长期占据侧栏。
- **手动恢复控件退役，对话流健康 chip 退为纯报告（design 14 §D4）** —— chip 与失败遮罩上的两枚 chamber 专有控件（整页重载、按会话重建对话通道）一并移除：chip 不再有动作行与按压/双击时间戳，`role=status` 保留，只负责报告；自动恢复路径一条不少（`error` 面的自动 heal、页面级有界阶梯 resync → 实例重启 → 预算内整页重载、F1 开帧相位机、journal 看门狗），且未移动一个 heal-failed / loading-stall / loading-failed tick。
- **载波抖动提示退役（design 14 §D4）** —— chip 不再显示 chamber 专有的「对话通道重连中」提示（上游无对应面）：规划器去掉该 notice、`carrierChurn` 观测与其窗口，locale 文案与阶梯表阈值一并删除。重试道不受影响（持续失败的载波仍有界退避重开，不会终局逃逸），fork 继续把 `dsh-chamber:stream-carrier-failed` 作为 diagnostics-only 页面事实发出；代价 = 重开窗口重新静默，页面无法再把「重连中」与「安静」分开。
### 修复
- **SSH 实例清册的半读不再被当作权威空集** —— 连接注册表读取部分失败时，桌面此前会把它投影成一份「完整」清册，渲染端的完成未读账本据此把暂缺来源当作已删除并剪掉其持久记录；现在注册表把降级与缺行事实（`degraded` / `rosterIncomplete` / `droppedCount`）经新的健康通道交给页面，清册不确定期间不剪任何 durable 未读，整文件失败与行级缺行分别记账。
- **同一次完成不再因两条通道各投递一次而双响** —— 完成事件经壳通道与 facts 源两条入口到达时，待投递账本由同一 outbox 身份派生同一投递键，合并成一条横幅；主进程的已投递回执跨宿主重载保留（一次完成一条横幅），可重试的抑制自动补发、永久性拒绝如实带原因，新一轮完成是新身份、不被吞。
...
- **facts 快照不可判定不再冻结未读完成（design 19 §3.7.1）** —— 蓝点、todo 完成条与 Dock 角标共读同一本 App 账本，此前本地 facts 快照「存在但不可判定」（verdict ≠ ok / serviceable=false / stale）时会把整本账冻结，dots 可以整天空着而通知照常到达；现在 rule 0 只冻结 facts 结论（通道边沿照旧 arm/disarm），「缺席即删除」的清扫要求 listComplete 且有通道，首次播种按来源世代只镜像一次宿主读取水位，绝不拿观察者域时间戳推进已读水位。另加 facts-health 权威日志（每次状态变化一条面包屑、计数进 detail）、chamber 全部监听边界的 never-throw 步守卫，角标 IPC 只在计数变化且成功派发后推送；被取代的通知规划孤岛（planRuntimeNotifications / planFactsNotifications / detectNotificationEdges 等）删除，仍有效的断言迁到活路径。
- **`$events` 观察者载波长连接化（design 19 §3.5）** —— 45s 无帧不再被当死 socket 静默换代（每次换代都关闭 socket、清 baselineTrusted、丢稳定位并整表重发）；载波现在活到真失败，重连走有界 1s→30s 退避（稳定复位需要一帧 ready），同一 socket 上重订阅，基线重采样按世代捕获且下限 250ms。
- **开帧进度以消费者接受为准（design 14 §D4）** —— api-gateway 的逻辑流 opener 此前把「第一帧已交付」当进度并清空加宽账本，于是「到达页面但从未被接受」的开帧把会话永久停在 `chat.loadingHistory`、既无判词也无重试节奏；现在开帧是相位机 sent → itemReceived → accepted，只有消费者的 `accept()` 才结算，30/60/120/240/300s 阶梯按跨载波世代累计预算计，预算耗尽即终局（`RemoteStreamOpeningBudgetError` + opening-orphaned / opening-timeout / opening-budget-exhausted 事实）；载波的加宽账本在压力下保持 ACTIVE 近期项（FIFO 驱逐不再重置连续计数、终局档不再被永久压制），开帧预算上限进共享 tables。
- **已被证死的开帧立即可见，自动臂不自行重建（design 14 §D4）** —— 流健康探针只把终局开帧（预算耗尽 / 孤立 / 超时）记入 64 条账本，忽略非终局档，任何 opening-accepted 退休会话级与实例级证据，并暴露受守卫的页面侧读取；健康座位改从该证据取相位，renderer 的投递分类器与 open-recovery 消费它。桌面头部只报告停驻 loading，自动的在途门控重建归移动端与页面级阶梯。
- **follow 的 cancel 只发往原载波，成功委派释放持有项** —— 控制面此前把 cancel 帧发往当前 socket，即使该 follow 开在已被替换的载波上（后继收到它从未承载的流的 cancel）；成功委派的 waterfall 又永久持有条目与本地 pending 镜像（宿主已把应答客户端移出投递集，不会有 cancel 到达）。现在 follow 记住自己的载波、只在那里取消，成功委派与真实 cancel 一样清掉定时器、条目与 pending 镜像（本地处理是 get/delete，迟到的真 cancel 仍无害）。
- **恢复相位不再进入被采样 state** —— 每秒返回新对象的健康采样让 InstanceView 按采样率重渲染（即使没有任何阈值被跨过）；现在新鲜对象只留在 ref，呈现相位由同一组输入在采样器里派生、state 只存相位，阈值窗内的采样直接 bail out。遮罩可见面逐字不变，导出面与锁同步。
- **测试运行器不再漏记失败** —— 池按声明序打印转写，并在 stop-on-failure 留下的「空洞」处停滞：空洞之后声明的红文件从未被记入 failures，gate 反报成功（用红 `test:mobile` 与红 `spawn-dsh.test.ts` 实测复现）。现在首个失败在完成回调里记录，`--continue` 仍记录全部，转写走有序遍历，遍历够不到时在末尾单独打印该文件自己的输出；新单测钉住空洞形状。
- **原生壳窗口 chrome 与上游 vibrancy 面对齐，整窗可从页面 chrome 行拖动（design 25 §5.5/§5.6）** —— Swift 腿移植上游 pin 的三个 darwin 面：红绿灯组按上游位置（首心 23,25）并在每次标题栏重排后重做（缩放/全屏/跨屏/最小化恢复/标题写入），`html[data-fullscreen]` 按两条窗口通知镜像，页面下加 NSVisualEffectView（.sidebar/.behindWindow/.active）让侧栏磨砂可见、窗口转非不透明、私有透明键不可用时回退；页面侧只有 `data-window-vibrancy` 标记命中时才去掉 sidebar darwin 规则与 renderer 容器的不透明底（Electron darwin 腿观感不变）。拖动面：标记 sidebar 的 `.topStrip`/`.logoRow`（与 ui-theme CHROME_ROWS 同源），documentStart 主 frame 注入把标记行上的按压转成 `dshChamberWindowDrag` 消息并由 AppKit 从当前鼠标位置起拖动环（合成 leftMouseDown + performDrag，窗口不跳）；控件、修饰键点击与页面已处理的事件穿透，交互选择器与 `regions.ts#INTERACTIVE_SELECTOR` 逐项锁步。
...
### 新增
- **插件受保护集合：拒绝集从「按域名前缀猜」改为按运行时事实判定（design 21 §6.11；决策 19 的 修订）** —— 旧规则既挡住不该挡的（官方 opt-in 层 `@deepseek-ai/dsh-experimental-*` 装不上），又挡不住真正要防的（组合被拆、族成员被跨代副本 shadow），且上游改名/换 scope 就会失真。现在受保护集合 `P = B₀ ∪ S ∪ F` 全部来自目标实例的**可验证事实**：`B₀` = 安装自带组合快照（对拍 C12 门）、`S` = chamber 播种注册表名（C13 门）、`F` = **已提交**运行时锁文件闭包（C11 门，含核心、不含 opt-in 与 dev/test 包）；install/remove 分相判定（remove 永不判版本），官方 scope 的 install 另需**精确同代**，`F` 读不到时按保守阶梯只收紧不放松（gateway 侧 503 可重试）。同一判定贯穿三条写面：控制面谓词、gateway 的 install/materialize/remove 三条路由（含延迟意图排空与执行时复判、装后复验）、桌面壳的三条本地通道（文件夹/归档拾取、renderer 提交 spec、移除）+ ssh 申请与就地物化（整批拒绝，绝不绕过被拒行执行）。设置页「已安装」列表改为**服务端投影行** `{role, protected, owner?}` 驱动，渲染端不再持有镜像谓词。四道保鲜门 C11–C14（运行时线族集合 / profile 契约锚 / 播种注册表结构 / manifest 三方镜像 + rows 行类型）随 `pnpm run test:upgrade-tools` 在 CI 两腿运行。
- **归档清理：常驻会话删除内容后不再回流 workspace（design 24 §4 step 9）** —— 删除一个**本进程仍持有**的已归档会话的内容，会连它的归档成员关系一起清掉；而官方会话列表是 live 优先的（`listSessions()` = 持久化重扫 ∪ `ctx.sessions.list()`），宿主仍照供这一行，于是刚删掉的会话以普通行重新出现在 workspace 里，看起来像"内容被恢复了"。现在：删除瞬间仍常驻（本进程 attached）的完成树**保留**归档成员关系（内容照删），行继续隐藏；归档管理器对这些行标注「内容已删除，待实例重启收敛」（并写进行复选框的可访问名），purge 结果新增加性字段 `residentRetainedRoots`（旧客户端忽略未知字段）；实例重启后该行不存在，无内容的成员由后续 run 的孤儿清扫收敛。同批加固：live 事实读取对任何形状漂移**响亮拒绝**（此前静默丢弃条目会把 loaded 守卫与常驻判据同时推向不安全方向）；批量写之前再做一次 live 复核，写窗口内才 attach 的会话其成员关系同样不被摘除（复核读失败则整批不写）。
### 修复
- **重启进入已下载更新不再被「关窗到托盘」吞掉（design 11）** —— macOS 上 `quitAndInstall()` 会先关闭全部窗口再退出，而默认的 hide-to-tray 要等 `before-quit`（关窗**之后**）才放行关窗，于是更新退出腿的关窗被 hide 吞掉：页面消失、进程连同本地 dsh 与隧道永久留存、更新永不安装（实测确认）。现在重启武装期间关窗放行、宿主兜底在 5s 后强制退出，且停滞 watchdog 在原生退出事件上**重锚**而不是被清除——退出腿不会中途误报停滞，腿走不完时又仍有如实文案与就地重试（清除会让"重启中"永久卡住）。
...
- **chamber 探针与随会话数据量彻底解耦（design 02 §3.2/§3.5、design 18
  §3.4 定稿并实施）** —— 身份/健康/就绪/激活探针统一到固定小体积
...
- **dsh 运行时设置面统一（本地 × gateway 同构）** —— 彩色状态徽标词表、快照/磁盘并入「当前状态」组、registry 只读行 + 编辑态统一、常驻「清理已安装版本」；gateway 补齐 `cleanup-version` / `restore-pre-rollback` / `recover-metadata` 路由；FATAL 元数据损坏改 blocked-alive（gateway 存活、托管 dsh 停机、管理面可轮询）；status 增 metadata 健康投影；desktop env/只读平台放行「重启 dsh」；设置面细节打磨与插件对话框统一（）。
- **「内建版本」行引导（用户决策）** —— 桌面与 gateway 设置中选中与内建（随应用/部署锚）同版本的行且该版本尚未装成受管树时，主按钮引导「恢复内建」（清除用户选择回到内建副本/锚，零下载）；「仍下载并安装为受管版本」为显式次要动作；已缓存（曾装树）时保持普通切换。
...
- **macOS 发布构建现在面向 macOS 26**（`macos-latest` runner）—— macos-14 已弃用（2026-07）且到 不再受支持。
