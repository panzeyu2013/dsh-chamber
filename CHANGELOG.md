...
> v0.2.0 是 0.2 主线的首个正式版（2026-08-25 v0.2.0-beta.1 起经七轮 beta 验证后转正；
> 0.1.5 → 0.2.0 的完整变更见下方 beta.1…beta.7 各节）。本节为相对 v0.2.0-beta.7 的
> 收尾增量——Gateway 访问控制面（dsh 蓝登录页与边界诊断页、被吊销会话的 ready 态
> 周期再验证、浏览器登录 403 实机定位）、install-gateway.sh 安装器输入层/离线语义/
> 事务重写，以及侧边栏与移动端回归修复：
...
- **侧边栏折叠工作区改名不再是静默 no-op（v0.2.0 窗口）** —— 此前工作区改名在
  头部下方追加一行输入框、且表单只在分组展开时渲染：折叠工作区的改名点击无任何
  反应，失败也被折叠门吞掉。修复：编辑表单内嵌进工作区头部行本身（折叠开关与
  沟槽保留，标题/孤儿徽标/计数/git 占用者/悬停动作在编辑期间替换为输入框 +
  保存/取消），不再追加行、进出编辑仅 ±6px 高度放松；表单随头部常驻渲染（与
  折叠状态无关），折叠工作区改名即时可见，工作区级失败（新建/改名/删除/拖拽）
  在折叠时也能浮现。编辑期间头部不可拖拽、双击重入防旧标题清空、HoverCard
  关闭、折叠箭头悬停互换抑制；armed 改名在行离开投影（删除/断连）时清除。
  会话行改名不变。
- **移动端 details 列壳晚挂载出口重打标（alpha.4 锚点审计补漏，2026-09）** ——
  官方 AppFrame 自首帧常驻渲染 details 列壳，其 `[data-slot=details]` 出口按
  会话门控晚挂载：打标观察器原谓词只匹配根/框架直接子级，出口挂入常驻壳内不再
  触发打标（潜伏：官方当前无可达的 details 打开路径，恒关时打标与否视觉等价）。
  修复：(a) `isStructuralTarget` 谓词补框架孙级分支——出口挂入常驻列壳即重打标，
  谓词与批决策抽成纯函数并补 boot 空壳 + 晚挂载回归用例；(b) frame 属性观察双
  保险：`data-sidebar-collapsed`/`data-details-collapsed` 变更经独立通路重打标。
  接线仅实机可验，作为对未来上游 details 接线的廉价保险。
