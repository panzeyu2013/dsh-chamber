...
//  DSHChamberTests
//
//  BridgeClient 的默认 edge 应答兜底绝不假成功——
//  trayAvailable/notificationSupported/badgeCountApiAvailable/mainWindowAlive/
//  webViewContentAlive、showNativeNotification、showMessage 这些未实现方法一律
//  loud 拒绝 {ok:false,error:...}。
//  打包态子进程环境从基底与 overlay 双方剔除 DSH_CHAMBER_SHELL_*（否则基底合并会
//  把被壳过滤过的键重新泄漏给 sidecar）。
//  sidecar stderr 透传逐字保留——Swift flavor 无 Electron safeStorage
...
final class BridgeClientFallbackTests: XCTestCase {
    ...
    func testChildEnvironmentStripsShellOverridesWhenPackaged() {
        ...
        // 单源化锁：壳自身 env（AppDelegate）与子进程 env（childEnvironment）
        // 共用同一过滤器——打包剔除、dev 原样。
    ...
    /// sidecar 的精确凭证文案逐字透传（前缀 + 原文 + 换行，无截断）。
