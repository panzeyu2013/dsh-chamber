...
//      Contents/MacOS/dsh-chamber-electron（Electron 腿二进制，basename 含
//      "dsh-chamber"，需注入 ELECTRON_RUN_AS_NODE=1 才当 Node 用——AppDelegate.swift:45-52 同规）
//      存在则用之 → 否则 XCTSkip（提示本机路径）。
//    - sidecar：env DSH_CHAMBER_SHELL_SIDECAR → 否则自 cwd 向上（≤6 层，AppDelegate.
//      findSidecarUpwards 同款循环）找 packages/desktop/sidecar-stub.ts
...
final class BridgeClientStubIntegrationTests: XCTestCase {
    ...
    private static let sidecarRelativePath = "packages/desktop/sidecar-stub.ts"
    ...
    private func invokeWithTimeout(_ bridge: BridgeClient,
                                   ...
                                   timeout: TimeInterval = BridgeClientStubIntegrationTests.invokeTimeout) async throws -> AnyCodable {
    ...
    private func waitForCompletion(of expectation: XCTestExpectation,
                                   what: String,
                                   timeout: TimeInterval = BridgeClientStubIntegrationTests.eventTimeout) -> Bool {
    ...
    /// POC 期取 DSH_CHAMBER_SHELL_CP_URL ?? 缺省 17520，URL 值非本测试关心的契约）。
    ...
    /// （sidecar-stub.ts:146-151/244-251 phases map）回 phase:"idle"；stop() 同步
    ...
    func testDisconnectAndCleanStop() async throws {
        ...
        // sidecar 记忆语义（以实际实现 sidecar-stub.ts:244-251 为准）：disconnect
    ...
    func testInstancesGetMissingFileAnswersLoudError() async throws {
        let missingInstancesPath = NSTemporaryDirectory()
            + "dsh-chamber-missing-instances-\(UUID().uuidString).json"
    ...
    private func makeLauncher() throws -> Launcher {
        ...
        if let configured = env["DSH_CHAMBER_SHELL_NODE_BIN"],
           ...
           FileManager.default.isExecutableFile(atPath: configured) {
            ...
            throw XCTSkip("未找到 Node：DSH_CHAMBER_SHELL_NODE_BIN 未设置且缺省路径 "
                          + "\(Self.defaultNodePath) 不存在；请设置 DSH_CHAMBER_SHELL_NODE_BIN")
        }
        let sidecarPath: String
        if let configured = env["DSH_CHAMBER_SHELL_SIDECAR"], !configured.isEmpty {
            sidecarPath = configured
