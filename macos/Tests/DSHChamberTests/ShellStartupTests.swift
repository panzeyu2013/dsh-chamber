...
final class ShellStartupTests: XCTestCase {
    ...
    func testPortResolutionPrecedenceAndPackagedDefault() {
        XCTAssertEqual(ControlPlanePort.resolve(
            env: ["DSH_CHAMBER_SHELL_PORT": "18001", "DSH_CHAMBER_CP_PORT": "18002"],
    ...
    func testPortResolutionDegradesInsteadOfFailing() {
        let invalidPortOverride = ControlPlanePort.resolve(
            env: ["DSH_CHAMBER_SHELL_PORT": "abc"], isPackaged: true, probeDevPort: nil)
        XCTAssertEqual(invalidPortOverride.port, 17500, "非法 DSH_CHAMBER_SHELL_PORT 落到打包缺省，不再致命")
        XCTAssertEqual(invalidPortOverride.source, .packagedDefault)
        XCTAssertTrue(invalidPortOverride.notices.contains { $0.contains("DSH_CHAMBER_SHELL_PORT") },
    ...
    func testRealSidecarScriptShape() {
        ...
        XCTAssertFalse(ControlPlanePort.isRealSidecarScript("/repo/packages/desktop/sidecar-stub.ts"))
    ...
    func testPortSourceLabelsAreExplicit() {
        XCTAssertEqual(AppDelegate.portSourceLabel(.envShell), "DSH_CHAMBER_SHELL_PORT 显式覆盖")
    ...
    func testShellDebugForcedOffInPackagedApp() {
        XCTAssertFalse(ShellDebug.isEnabled(environment: ["DSH_CHAMBER_SHELL_DEBUG": "1"], isPackaged: true),
                       "打包态即使 DSH_CHAMBER_SHELL_DEBUG=1 也必须关闭（产品面不带调试回传/快照）")
        XCTAssertTrue(ShellDebug.isEnabled(environment: ["DSH_CHAMBER_SHELL_DEBUG": "1"], isPackaged: false))
        XCTAssertFalse(ShellDebug.isEnabled(environment: [:], isPackaged: false))
        XCTAssertTrue(ShellDebug.isPackaged(
            executablePath: "/Applications/dsh-chamber.app/Contents/MacOS/dsh-chamber"))
        XCTAssertFalse(ShellDebug.isPackaged(
            executablePath: "/repo/macos/.build/debug/DSHChamber"))
