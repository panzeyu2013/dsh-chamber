...
final class ShellLogTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("shell-log-tests-\(UUID().uuidString)")
    ...
    func testFileURLRuleIsUserDataLogsShellLog() {
        XCTAssertEqual(ShellLog.fileURL(userDataDir: "/u/data").path,
                       "/u/data/logs/shell.log")
        XCTAssertEqual(ShellLog.directoryName, "logs")
        XCTAssertEqual(ShellLog.fileName, "shell.log")
    ...
    func testSidecarFileURLRuleAndInstance() throws {
        ...
        XCTAssertEqual(ShellLog.sidecarFileName, "sidecar.log")
        XCTAssertEqual(ShellLog.sidecarFileURL(userDataDir: tempDir.path).deletingLastPathComponent(),
                       ShellLog.fileURL(userDataDir: tempDir.path).deletingLastPathComponent(),
        ...
        let text = try String(contentsOf: ShellLog.sidecarFileURL(userDataDir: tempDir.path), encoding: .utf8)
        XCTAssertTrue(text.contains("WebSocket stream 7 closed"), "拼接证据必须真的落盘：\(text)")
    }

    /// 轮转名必须按实例文件名派生：sidecar 实例绝不能把主日志轮转名（
    /// shell.log.1）当成自己的——两个实例共用同一份 rotateLocked 实现。
    func testSidecarRotationUsesItsOwnFileName() throws {
        ...
        let rotated = ShellLog.sidecarFileURL(userDataDir: tempDir.path)
            .deletingLastPathComponent()
        ...
        XCTAssertFalse(FileManager.default.fileExists(
            atPath: ShellLog.fileURL(userDataDir: tempDir.path)
                .deletingLastPathComponent()
                .appendingPathComponent(ShellLog.rotatedFileName).path),
            "sidecar 实例不得写出 shell.log.1")
    }
    ...
    func testAppendWritesTimestampedLinesToDisk() throws {
        ...
        log.append("[shell] 装配完成")
        log.append("[supervisor] sidecar 已启动")

        let text = try String(contentsOf: url, encoding: .utf8)
        let lines = text.split(separator: "\n")
        XCTAssertEqual(lines.count, 2)
        XCTAssertTrue(text.contains("[shell] 装配完成"))
    ...
    func testReopeningAppendsInsteadOfTruncating() throws {
        let url = ShellLog.fileURL(userDataDir: tempDir.path)
        ShellLog(fileURL: url).append("first-run")
        let second = ShellLog(fileURL: url)
    ...
    func testRotationKeepsFileBoundedAndKeepsWriting() throws {
        let url = ShellLog.fileURL(userDataDir: tempDir.path)
        let log = ShellLog(fileURL: url, maxBytes: 120)
        for index in 0..<20 { log.append("line-\(index)") }

        let rotated = url.deletingLastPathComponent()
            .appendingPathComponent(ShellLog.rotatedFileName)
        XCTAssertTrue(FileManager.default.fileExists(atPath: rotated.path),
                      "超过上限必须轮转出 shell.log.1")
    ...
    func testRotationAlsoHappensAtOpenForOversizedExistingFile() throws {
        ...
        _ = ShellLog(fileURL: url, maxBytes: 128)
        let rotated = url.deletingLastPathComponent()
            .appendingPathComponent(ShellLog.rotatedFileName)
        XCTAssertTrue(FileManager.default.fileExists(atPath: rotated.path))
    }
    ...
    func testSymlinkedLeafIsRefusedInsteadOfWritingThroughTheLink() throws {
        ...
        let link = tempDir.appendingPathComponent(ShellLog.directoryName)
            .appendingPathComponent(ShellLog.fileName)
        try FileManager.default.createDirectory(at: link.deletingLastPathComponent(),
                                                withIntermediateDirectories: true)
        try FileManager.default.createSymbolicLink(at: link, withDestinationURL: outside)
        let log = ShellLog(fileURL: link)
    ...
    func testSymlinkedDirectoryIsRefused() throws {
        ...
        let log = ShellLog(fileURL: link.appendingPathComponent(ShellLog.fileName))
        XCTAssertFalse(log.isActive, "符号链接目录不得打开")
        log.append("must-not-land-outside")
        XCTAssertFalse(FileManager.default.fileExists(
            atPath: outsideDir.appendingPathComponent(ShellLog.fileName).path),
            "绝不透过目录链接写出去")
    }
    ...
    func testFifoLeafIsRefusedInsteadOfBlockingTheMainThread() throws {
        let url = tempDir.appendingPathComponent(ShellLog.directoryName)
            .appendingPathComponent(ShellLog.fileName)
        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(),
                                                withIntermediateDirectories: true)
        let mkfifo = Process()
        ...
        let log = ShellLog(fileURL: url, maxBytes: 256)
    ...
    func testExistingLooseDirectoryIsTightenedTo0700() throws {
        ...
        let log = ShellLog(fileURL: directory.appendingPathComponent(ShellLog.fileName))
        XCTAssertTrue(log.isActive)
    ...
    func testOpenTimeRotationFailureDegradesInsteadOfOpeningTheOversizedFile() throws {
        let directory = tempDir.appendingPathComponent(ShellLog.directoryName)
        let url = directory.appendingPathComponent(ShellLog.fileName)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    func testOpenFailureDegradesSilently() throws {
        ...
        let blocker = tempDir.appendingPathComponent(ShellLog.directoryName)
        try Data("not a directory".utf8).write(to: blocker)
        let log = ShellLog(fileURL: ShellLog.fileURL(userDataDir: tempDir.path))
        XCTAssertFalse(log.isActive)
        log.append("x")
        log.note("y")
    }
    ...
    func testKeyLifecyclePointsRouteThroughShellLog() throws {
        let appDelegate = try source("Sources/DSHChamber/AppDelegate.swift")
        XCTAssertTrue(appDelegate.contains("ShellLog.shared.configure(userDataDir: stateDir)"),
        ...
        XCTAssertTrue(appDelegate.contains("shellLog(\"[shell] applicationDidFinishLaunching")
            || appDelegate.contains("shellLog(\"[shell] 装配完成"),
                      "启动装配行必须落盘")

        let controller = try source("Sources/DSHChamber/MainWindowController.swift")
        XCTAssertTrue(controller.contains("shellLog(\"[shell] 页面加载失败"),
                      "导航失败必须落盘（双击态白屏的唯一本地考古面）")
        XCTAssertTrue(controller.contains("落首载失败说明页"),
                      "首载失败页必须留一条落盘记录")
        XCTAssertTrue(controller.contains("shellLog(\"[shell] 下载"))

        let updater = try source("Sources/DSHChamber/AppUpdater.swift")
        XCTAssertTrue(updater.contains("shellLog(\"[shell] nativeUpdatePhase"),
                      "更新相位必须落盘")
    }
}
