...
//  DSHChamberPocTests
//
//  钉住视口越界策略的注入契约与装配点（design 25 §5.1 / deviations S-48）。除字符串契约外，
...
final class ShellOverscrollPolicyTests: XCTestCase {
    ...
    /// 注入契约的字面量锁步：改名必须同时改 design 25 §5.1 与自检口径
    ...
    /// 删掉或后移 MainWindowController 的 install 调用，此前所有门禁仍绿而 S-48 原样回归。
    func testMainWindowInstallsPolicyBeforeWebViewConstruction() throws {
        ...
        let construction = try XCTUnwrap(
            source.range(of: "WKWebView(frame:"), "找不到 WKWebView 构造点，装配锁步失效")
