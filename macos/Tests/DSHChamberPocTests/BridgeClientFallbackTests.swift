...
//  T-11：打包态子进程环境从基底与 overlay 双方剔除 POC_*（此前基底合并会
...
final class BridgeClientFallbackTests: XCTestCase {
    ...
    /// T-11：打包态 POC_* 从基底与 overlay 都剔除；DSH_* / PATH 等照常保留。
    func testChildEnvironmentStripsPocKeysWhenPackaged() {
        let base = ["POC_DEBUG": "1",
                    "POC_SIDECAR": "/tmp/stub.ts",
                    "DSH_CHAMBER_CP_PORT": "17500",
                    "PATH": "/usr/bin"]
        let overlay = ["ELECTRON_RUN_AS_NODE": "1", "POC_NODE_BIN": "/tmp/node"]
        ...
        XCTAssertNil(packaged["POC_DEBUG"])
        XCTAssertNil(packaged["POC_NODE_BIN"])
        let dev = BridgeClient.childEnvironment(base: base, overlay: overlay, isPackaged: false)
        XCTAssertEqual(dev["POC_DEBUG"], "1", "dev 态照常传递 POC_*")
        XCTAssertEqual(dev["POC_NODE_BIN"], "/tmp/node")
