...
public enum PackagedLayout {
    ...
    public static func resolveNode(
        ...
        if let explicit = env["DSH_CHAMBER_SHELL_NODE_BIN"], !explicit.isEmpty {
    ...
    public static func resolveSidecar(
        ...
        if let explicit = env["DSH_CHAMBER_SHELL_SIDECAR"], !explicit.isEmpty { return explicit }
    ...
    /// userData 解析：`DSH_CHAMBER_SHELL_USER_DATA` → 打包态同根 → dev 目录。
    public static func resolveUserData(
        env: [String: String],
        home: String,
        isPackaged: Bool
    ) -> String {
        if let explicit = env["DSH_CHAMBER_SHELL_USER_DATA"], !explicit.isEmpty { return explicit }
        return isPackaged ? userDataDir(home: home) : home + "/Library/Application Support/dsh-chamber-dev"
    }

    /// 内置 dsh 工作区解析：`DSH_CHAMBER_SHELL_DSH_PATH` → 打包态 `<Resources>/sidecar/vendor/dsh`
    /// → nil（dev 由调用方按仓库候选查找）。
    public static func resolveDshWorkspace(
        ...
        if let explicit = env["DSH_CHAMBER_SHELL_DSH_PATH"], !explicit.isEmpty { return explicit }
        if isPackaged, let resourcesDir {
