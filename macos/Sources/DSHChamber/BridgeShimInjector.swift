// BridgeShimInjector.swift — A 桥 bridge-shim.js 注入器（design 25 §4.4.1）
//
...
/// A 桥 shim 注入工具：把 bridge-shim.js 作为用户脚本注册到
...
enum BridgeShimInjector {
    ...
    /// 再评估 .defaultClient（WKContentWorld.defaultClient，macOS 11+）
    /// 的隔离语义：design 25 §4.4.1 注明 .page world、documentStart，而
    /// 挂出时机二选一（Swift ready 后注入 vs documentStart 预定义
    ...
    /// 语义不变）。
    /// 原生通道令牌占位符（与 shim 里 NATIVE_CHANNEL_TOKEN 的字面量一致）：
    ...
    /// 已安装标记：注入源码以此为前缀，install 据此幂等——显式标记，
    ...
    /// 该 configuration 是否已注册过本注入器的 shim。
    ...
    /// **幂等**：已安装 → no-op（返回 false），绝不重复注册——重复的
