...
//  启动都在 didFinish 完成；2026-12 二轮独立复核最小复现：run-only 启停输出
//  "WILLFIN → DIDFIN"）。旧注释称手工 finishLaunching()「内部会向 delegate 发
//  applicationDidFinishLaunching」是错的——它只发 willFinish，随后 run() 会再发
//  一遍，一次启动 willFinish 跑两遍；且 ShellLog.configure 在 didFinish 才执行，
//  willFinish 的 W3 启动日志只进 stdout。故删除手工调用。
import AppKit
