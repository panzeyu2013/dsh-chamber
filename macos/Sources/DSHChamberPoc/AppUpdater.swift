...
final class AppUpdater: NSObject, SPUUpdaterDelegate, SPUStandardUserDriverDelegate, NSMenuItemValidation {
    ...
    func start(info: [String: Any] = Bundle.main.infoDictionary ?? [:],
        ...
        if let error = Self.configurationError(for: configuration) {
            ...
            print("[poc] Sparkle 更新不可用：配置错误——\(error)")
            return false
        }
        ...
        if startUpdater {
            ...
            do {
                ...
                print("[poc] Sparkle 启动失败（绝不谎报可用）：\(error.localizedDescription)")
                return false
            }
        }
    ...
    func perform(_ kind: NativeUpdateActionKind) -> NativeUpdateActionOutcome {
        ...
        print("[poc] 原生更新动作：\(kind.rawValue)（交给 Sparkle 标准窗口）")
    ...
    func note(_ event: NativeUpdateEvent) {
        ...
        print("[poc] nativeUpdatePhase \(report.phase.rawValue) version=\(version) error=\(error)")
        onPhase?(report)
    }
    ...
    func presentStandardUpdate(handleShowingUpdate: Bool,
        ...
        case .standardWindow:
            ...
            print("[poc] Sparkle 标准更新窗口：update=\(version) userInitiated=\(userInitiated)")
        case .pageProjection:
            print("[poc] scheduled 更新由原生壳展示（不弹 Sparkle 窗）："
    ...
    func standardUserDriverShouldHandleShowingScheduledUpdate(_ update: SUAppcastItem,
        ...
        print("[poc] Sparkle 询问 scheduled 更新展示权（immediateFocus=\(immediateFocus)）"
