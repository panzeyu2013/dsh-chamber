...
3. Follow the consolidation principles of `docs/design/01-overview.md`: anything the dsh host or its frontend already covers is attached/served by the control plane — **never re-implement an execution surface**; the desktop control plane does not consume host sessions; removed domains must not return to the desktop/control-plane shape. The only bounded exceptions are design 08's in-instance Git plugin and design 17's separately invoked authenticated gateway orchestration; neither exception may leak an execution surface into `packages/control-plane` or the desktop renderer.
...
- `packages/dsh-chamber-client-ui-settings-bridge` — the self-built settings SHELL plugin: replaces the official SettingsRoot registration (sidebar.settings at priority -1, shadowing) — a server dropdown over the selected instance's official settings sections (child cordis context bridge) plus the fixed chamber-global connections nav entry; also hosts the per-server `dsh-runtime` settings.section (id `dsh-runtime`, after agent-presets; local = full runtime management, gateway = proxied `/chamber/runtime`, ssh = version read-only; the runtime projections are a documented monorepo seam — the section imports the shared renderer projection `packages/renderer/src/runtime-management.ts` by relative path, no published dependency; every source gets a restart-dsh action — control-plane `restartLocal()`, `/chamber/runtime/restart`, or `restart_service` systemd IPC — to refresh mounted plugins without restarting the Electron shell — design 18 §3.6/§9) (see 05 §5 sibling design discussion 2026-08).
- `packages/desktop` — Electron shell: single frame (`loadURL` the control-plane origin), transport-manager (generic transport runtime; `transport-provider.ts` interface + the `ssh` provider in `ssh-provider.ts` and the `gateway` provider in `gateway-provider.ts` — tunnels + systemd exec / authenticated HTTPS, v1 kinds `ssh | gateway` (design 17 §7)), instance registry (`<userData>/ssh-instances.json`), IPC, dsh runtime version management (design 18, hosted on the shared `packages/dsh-runtime` core).
...
- `packages/gateway` — separately invoked authenticated server shape (design 17): mandatory public request boundary + single local-dsh proxy + gateway-owned derived orchestration, plus dsh runtime version management (design 18 §9: startup switch phase reusing the shared `packages/dsh-runtime` core + authenticated `/chamber/runtime` surface, S17–S20). It reuses control-plane internals but may bind non-loopback only through the explicit HTTP/WS boundary evaluator capability; the ordinary control plane remains loopback-only.
...
- Unit tests (the exact set CI runs): control-plane `node packages/control-plane/test/protocol.ts`, `storage.ts`, `m1-dsh-client.ts`, `host-logs.ts`, `manager-api.ts`, `instance-proxy.ts`, `gateway-transport.test.ts`, `ws-frames.ts`, `static-serving.ts`, `host-graph-seed.ts`, `restart-local.ts`; gateway `pnpm run test:gateway`; desktop `pnpm run test:desktop` (transport-manager / transport-target / ssh-provider / ssh-config / gateway-provider / renderer-trust / plugin-sync / chamber-settings / notifications / deep-link / open-in / runtime-lockstep / dsh-runtime-controller / scripts/bundle-swap / scripts/after-pack-adhoc-sign); renderer shell `pnpm run test:renderer-shell`; Git plugins `pnpm run test:git` + `pnpm run test:host-git`; other client plugins `pnpm run test:sidebar` + `pnpm run test:settings-bridge` + `pnpm run test:connections` (settings-connections `plugin-diff`) + `pnpm run test:client-web` (web-shell boot-tolerance decision rules — design 09 §3.5 version tolerance) + `pnpm run test:connection` (the chamber base-path patch decision rules).
