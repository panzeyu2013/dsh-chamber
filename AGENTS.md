...
- `packages/dsh-chamber-client-ui-git` — the chamber-bundled Git worktree client plugin (design 08): `sidebar.git` topology plus safe create/remove sagas; facts/actions stay in a page-wide plugin coordinator and never become a control-plane execution surface.
- `packages/dsh-chamber-client-ui-open-in` — the chamber-bundled desktop open-in client plugin (design 17): the per-session header utility negotiates the local desktop's Finder/VS Code capabilities through trusted preload IPC; it has no host plugin/seed and never turns the control plane into an execution surface.
...
- Unit tests (the exact set CI runs): control-plane `node packages/control-plane/test/protocol.ts`, `storage.ts`, `m1-dsh-client.ts`, `host-logs.ts`, `manager-api.ts`, `instance-proxy.ts`, `ws-frames.ts`, `static-serving.ts`, `host-graph-seed.ts`; desktop `pnpm run test:desktop` (transport-manager / ssh-provider / ssh-config / renderer-trust / plugin-sync / chamber-settings / notifications / deep-link / open-in / bundle-swap / after-pack ad-hoc signing); renderer shell `pnpm run test:renderer-shell`; Git plugins `pnpm run test:git` + `pnpm run test:host-git`; other client plugins `pnpm run test:sidebar` + `pnpm run test:settings-bridge` + `pnpm run test:connections` (settings-connections `plugin-diff` + `save-host`) + `pnpm run test:client-web` (web-shell boot-tolerance decisions + real `configureContext` boot seam) + `pnpm run test:connection` (base-path decisions + HTTP/WS/generic-RPC carrier assembly) + `pnpm run test:open-in` (capability/source validation, bounded first-probe recovery, coordinator concurrency, accessible-menu keyboard decisions).
- Client plugin type checks: `pnpm run typecheck:sidebar`, `typecheck:layout`, `typecheck:connections`, `typecheck:settings-bridge`, `typecheck:git`, `typecheck:open-in` (the root `typecheck` program does NOT include the self-built plugins — ambient `declare module` entries shadow them); copied dsh clients `pnpm run typecheck:client-web` + `pnpm run typecheck:connection` (filtered gates over chamber-owned package files; only source-only vendor-graph diagnostics are filtered); host packages `pnpm run typecheck:host-graph` + `pnpm run typecheck:host-git` (both are outside the root `typecheck` program and map the `@deepseek-ai/*` vendor sources through package tsconfigs).
