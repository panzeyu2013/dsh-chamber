...
- Unit tests (the exact set CI runs): control-plane `pnpm run test:control-plane` (protocol / storage / m1-dsh-client / host-logs / manager-api / local-connection / spawn-dsh / instance-proxy / ws-frames / static-serving / host-graph-seed / reaper / rpc-envelope / cordis-inserts); desktop `pnpm run test:desktop` (transport-manager / ssh-provider / ssh-config / renderer-trust / wiring / plugin-sync / cross-package-contract / ipc-surface-mirror / chamber-settings / notifications / deep-link / open-in / updater / bundle-swap / after-pack signing); renderer `pnpm run test:renderer-shell` (pending-open, deep-link activation, eviction/recycle, aggregate refresh, notifications, host graph and shell); Git plugins `pnpm run test:git` + `pnpm run test:host-git`; other client plugins `pnpm run test:sidebar` + `pnpm run test:settings-bridge` + `pnpm run test:connections` (including save-host and host validation) + `pnpm run test:client-web` (boot tolerance, chamber context and real `configureContext`) + `pnpm run test:connection` (base paths and carrier assembly) + `pnpm run test:open-in` (source/capability validation, recovery and accessible menu decisions).
