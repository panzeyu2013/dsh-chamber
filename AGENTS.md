...
3. Follow the consolidation principles of `docs/design/01-overview.md`: anything the dsh host or its frontend already covers is attached/served by the control plane — **never re-implement an execution surface**; the control plane does not consume host sessions; domains removed from scope (walkthrough, notification center/history, MCP, thin-shell chat UI, control-plane session/notification runtime, …) **must not return in any form**. The desktop-native edge notification carve-out in design 19 may only project the renderer's existing per-instance runtime facts through Electron (no control-plane consumer, history, or notification center). One further carve-out (2026-08-16, design 08): git/GitHub stays out of the control plane and the 本体, but may ship as a chamber-bundled client plugin (worktree lifecycle etc., see 08) — the plugin never re-implements a dsh host surface.
...
- `packages/dsh-client-web` — in-repo copy of the official web shell with the rc.8 boot.ts class boot kernel re-based onto the rc.8 module system (incl. the per-instance `extraRows` boot-row merge + `__ModuleLoader__` facade + synchronous `configureContext`, runtimeCtx getter, and awaitable async dispose seams, design 09 module D) (shadows the vendored workspace entry).
...
- `packages/desktop` — Electron shell: single frame (`loadURL` the control-plane origin), transport-manager (generic transport runtime; `transport-provider.ts` interface + the `ssh` provider in `ssh-provider.ts` — tunnels + systemd exec, v1 kind `ssh`), instance registry (`<userData>/ssh-instances.json`), trusted IPC, desktop open-in/deep-link routing, and design 19's Electron-native notification projection (no history/center/control-plane consumer).
...
- The only modifiable dsh sources are our chamber packages: the in-repo copies `packages/dsh-client-connection` (the base-path patch) and `packages/dsh-client-web` (the rc.8 boot.ts re-base: N-ctx module-table sharing seam + `__ModuleLoader__` facade, incl. `extraRows`, synchronous context configuration, runtime getter and async disposal seams), the self-built client packages `packages/dsh-chamber-client-ui-sidebar`, `packages/dsh-chamber-client-ui-settings-connections`, `packages/dsh-chamber-client-ui-settings-bridge`, `packages/dsh-chamber-client-ui-layout`, `packages/dsh-chamber-client-ui-open-in`, and `packages/dsh-chamber-client-ui-git`, plus the host packages `packages/dsh-host-client-graph` and `packages/dsh-chamber-host-git-worktree`; everything under `vendor/harness-packages` is untouched upstream source.
...
- Unit tests (the exact set CI runs): control-plane `pnpm run test:control-plane` (protocol / storage / m1-dsh-client / host-logs / manager-api / lifecycle / local-connection / spawn-dsh / instance-proxy / ws-frames / static-serving / host-graph-seed / reaper / rpc-envelope / cordis-inserts); desktop `pnpm run test:desktop` (transport-manager / ssh-provider / ssh-config / renderer-trust / plugin-sync / cross-package-contract / ipc-surface-mirror / chamber-settings / notifications / deep-link / open-in / updater / bundle-swap / after-pack signing); renderer `pnpm run test:renderer-shell` (pending-open, deep-link activation, aggregate refresh, notifications, host graph and shell); Git plugins `pnpm run test:git` + `pnpm run test:host-git`; other client plugins `pnpm run test:sidebar` + `pnpm run test:settings-bridge` + `pnpm run test:connections` (including save-host and host validation) + `pnpm run test:client-web` (boot tolerance, boot-row merge and real `configureContext`) + `pnpm run test:connection` (base paths and carrier assembly) + `pnpm run test:open-in` (source/capability validation, recovery and accessible menu decisions) + `pnpm run test:cli` (bounded host-log snapshot follow overlap).
- Client plugin type checks: `pnpm run typecheck:sidebar`, `typecheck:layout`, `typecheck:connections`, `typecheck:settings-bridge`, `typecheck:git`, `typecheck:open-in` (the root `typecheck` program does NOT include the self-built plugins — ambient `declare module` entries shadow them); copied dsh clients `pnpm run typecheck:client-web` + `pnpm run typecheck:connection` (filtered gates over chamber-owned files; only source-only vendor-graph diagnostics are filtered); host packages `pnpm run typecheck:host-graph` + `pnpm run typecheck:host-git` (both are outside the root `typecheck` program and map the `@deepseek-ai/*` vendor sources through package tsconfigs).
