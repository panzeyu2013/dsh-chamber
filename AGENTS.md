...
- `packages/gateway` — separately invoked authenticated server shape (design 17): mandatory public request boundary + single local-dsh proxy + gateway-owned derived orchestration. It reuses control-plane internals but may bind non-loopback only through the explicit HTTP/WS boundary evaluator capability; the ordinary control plane remains loopback-only.
...
- Gateway packaging: `pnpm run build:gateway`, then `pnpm --filter @dsh-chamber/gateway pack`; install the tarball into a clean temporary prefix and require `gateway --help` to succeed.
