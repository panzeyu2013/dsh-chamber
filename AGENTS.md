...
- Unit tests (the exact set CI runs): control-plane `node packages/control-plane/test/protocol.ts`, `storage.ts`, `m1-dsh-client.ts`, `host-logs.ts`, `manager-api.ts`, `instance-proxy.ts`, `gateway-transport.test.ts`, `ws-frames.ts`, `static-serving.ts`, `host-graph-seed.ts`; gateway `pnpm run test:gateway`; desktop `pnpm run test:desktop` (transport-manager / transport-target / ssh-provider / ssh-config / gateway-provider / renderer-trust / plugin-sync / chamber-settings / notifications / deep-link / open-in / runtime-lockstep); renderer shell `pnpm run test:renderer-shell`; Git plugins `pnpm run test:git` + `pnpm run test:host-git`; other client plugins `pnpm run test:sidebar` + `pnpm run test:settings-bridge` + `pnpm run test:connections` (settings-connections `plugin-diff`) + `pnpm run test:client-web` (web-shell boot-tolerance decision rules — design 09 §3.5 version tolerance) + `pnpm run test:connection` (the chamber base-path patch decision rules).
