...
- **Push-path classification (2026-09)**: the linux chain in `ci.yml` now differs per event. On a branch push/PR, a **prose-only change** (`docs/**` plus the root prose allowlist, classified by `scripts/dev/classify-ci-changes.mjs`) runs only the file-only gates (workflow action SHAs, i18n, design tokens, the upstream-touchpoint registry, release-workflow policy, tooling unit tests) and skips install / type checks / unit suites / builds / the gateway pack smoke and the Windows leg; a code change runs everything as before. On a **tag push** the linux chain steps aside entirely — `release.yml`'s `validation` is its mechanical superset, and `release-workflow-policy.test.mjs` asserts that contract **derived from ci.yml** (adding a gate to ci.yml without mirroring it into release.yml turns CI red) — while the Windows leg keeps running (release.yml has no win32-semantics leg). The classifier fails safe: anything it cannot prove to be prose runs the full chain, and the prose allowlist is frozen by the policy test, so widening it takes a deliberate edit there.
