...
- `packages/renderer` — 自建 dsh 前端（源码复用）：入口构建（chamber 复合 entry）、纯 dsh 首屏桥接宿主（entry 级 React：本地 auto-start / 注册表 auto-connect / chamberBridge publish & onOpenSession）、N-ctx 多实例编排、启动图清单生成、design 09 模块 C（每实例宿主图合并 + 额外 entry 预加载：`host-graph.ts` + `chamber-covered.ts`）。
...
- `packages/dsh-host-client-graph` — 自研 chamber 宿主包（design 09 模块 A）：经 Typert Remote 只读暴露实例的 `clientModules.graph()` boot 图；提交的 esbuild 产物 `dist/index.js` 由控制面 seed 进 profile（非 vendor 源码）。
...
- lockfile：任何 `pnpm-lock.yaml` 重新生成后，`pnpm install --frozen-lockfile` 必须通过。pnpm 11 从零写入 lockfile 时会裁剪 `vendor/harness-packages/@deepseek-ai/*` 的 importer 记录（符号链接 workspace 包），其自身 frozen 校验随即失败——提交的 lockfile 保留这些记录（生成时须有 vendor 树在场并验证 frozen；勿提交被裁剪的 lockfile）。
