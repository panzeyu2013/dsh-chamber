# 15 · Chamber 设置呈现（v1 平铺形态；统一设置页推迟，设计待评审，2026-08）

> **状态：设计待评审，实现未排期**。2026-08 范围缩减（用户拍板）：**移除 B 项**
...
- `SettingsShell`（`packages/dsh-chamber-client-ui-settings-bridge`）：服务器下拉
  ...
  `__connections`（连接）、`__update`（更新，设计 11）。
...
- divider 下固定入口由 2 个扩为 3 个：`__connections` / `__update` / 新增
  **`__general`**（通用设置）。
- `__general` 视图（`GeneralView`，settings-bridge 壳内）：**设计 14 全部设置
  落点**——
  - 关闭窗口行为（`windowCloseBehavior`：hide-to-tray / quit，可设）；
  - 登录自启（`launchAtLogin`，可设；mac/linux，win 门控置灰）；
  - 保持唤醒（`keepAwake`，默认关）；
  - 退出确认说明（有活动隧道/本地实例时退出需确认）。
