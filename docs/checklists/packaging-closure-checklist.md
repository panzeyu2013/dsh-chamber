...
`clear-only-credentials.ts`、`connection-save.ts`、`control-plane-module.ts`、`credential-binding.ts`、
`credential-identity.ts`、`deep-link-scheme.ts`、`deep-link.ts`、`describe-error.ts`、
`disk-evidence-gate.ts`、`dsh-runtime-controller.ts`、`electron-edges.ts`、`free-port.ts`、
`gateway-http-core.ts`、`gateway-provider.ts`、`gateway-session-refresh.ts`、`gateway-session.ts`、
`gateway-sync-registry.ts`、`host-assembly.ts`、`host-edges.ts`、`host-package-dirs.ts`、
`host-root-lease.ts`、`ipc-events.ts`、`lockfile-facts-memo.ts`、`node-edges.ts`、
`notifications.ts`、`open-in.ts`、`owner-only-secret-file.ts`、`plugin-sync.ts`、
`pnpm-launcher.ts`、`registry-projection.ts`、`renderer-frame-watchdog.ts`、`renderer-trust.ts`、
`runtime-probe-detail.ts`、`runtime-startup-host.ts`、`runtime-tree-check.ts`、`sanitize-error.ts`、
`shell-assembly-ctx.ts`、`shell-core.ts`、`shell-ipc-connections.ts`、`shell-ipc-ctx.ts`、
`shell-ipc-open-in.ts`、`shell-ipc-plugins-gateway.ts`、`shell-ipc-plugins-local.ts`、`shell-ipc-plugins-ssh.ts`、
`shell-ipc-runtime.ts`、`shell-ipc-settings.ts`、`shell-ipc-update.ts`、`shortcuts-bridge.ts`、
`sidecar-console-redirect.ts`、`sidecar-ctx.ts`、`sidecar-entry.ts`、`sidecar-exit-codes.ts`、
`ssh-config.ts`、`ssh-provider.ts`、`store-file-hygiene.ts`、`transport-manager.ts`、
`transport-provider.ts`、`transport-reconnect.ts`、`update-discovery.ts`、`update-headless.ts`、
`updater.ts`、`win-acl.ts`
...
test "$count" = 70 || { echo "STALE: 名单/计数需同步（见上方 70 个）"; exit 1; }
```
