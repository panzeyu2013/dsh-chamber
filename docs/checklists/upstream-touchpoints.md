...
| `@deepseek-ai/dsh-client-locale` + `dsh-client-ui-settings`（**页面语言归属**，design 06 §4.6「页面语言归属」） | `packages/renderer/src/locale-ownership.ts` 只依赖四条 vendor 事实：① locale 服务名 `locale` 与 `slots.installLocale` 面（两扇取面门）；② `LOCALE_SETTINGS_NAMESPACE = 'locale'` 与 `settingsScope.bind({ namespace })` 形状；③ `locale.subscribe(sync)` **先于** apply 里紧随的立即 `sync()`（“vendor 写 → 归属器同栈回写”的前提）；④ 每 namespace scope 的 `status: persistence === 'host' ? 'loading' : 'unavailable'` 与 `derive → ready/unavailable`（归属器的 settled 判据 = `status !== 'loading'`） | **源码文本锁**（不是 vendor 读）`packages/renderer/test/frame-chrome/page-language.test.ts`（剥注释后按标识符/字面量断言 `renderer/src/locale-ownership.ts` 的四处接法；vendor 写时序在该模块里按安装态论证、不在此执行）——2026-12 五轮复核更正：本行此前引用的 `locale-vendor-contract.test.ts` 不存在；pin 升级时命中即重审上述四条——③ 漂移削弱同栈回写，④ 漂移会**静默复活**已修的闪烁（provisional 被当成已敲定） |
...
- **registry 单一来源与两道新门（2026-12）**：`registry.json` 是触点登记的唯一机器来源（生成视图与
  verifier 都读它）；三道命令：
