...
| @deepseek-ai/dsh-client-ui-{layout,sidebar,sidebar-right,conversation,chat,dockkit} + ui-primitives（移动插件锚点的**上游发射侧**；与上一行同一批 DOM 契约） | **锚点保鲜门**（2026-09-14 登记，补上「只自证」之外的上游侧）：`scripts/dev/verify-mobile-anchors.mjs`（纯判据 `scripts/dev/mobile-anchors.mjs`、负例 `verify-mobile-anchors.test.mjs`）把**插件声明的锚点**与**上游真实发射的锚点**做双向差集——方向 A（声明 → 上游）：从 `packages/dsh-chamber-client-ui-mobile/src/**` 的**去注释投影**里抽 `data-*` / `[role=…]` / `data-slot` key / 哈希 class token（不在本表或脚本里手抄第三份清单），逐个要求在产物里有发射点；方向 B（最小集 → 两侧）：门禁最小断言集 19 项必须**既在源码侧声明、又在产物侧发射**——`data-slot="root"`、三列 key `sidebar`/`main`/`rightbar`、`shell.overlay`、`data-conversation-scroll`、`data-composer-seat`、`data-composer-input`、`data-chat-flow`、`data-chat-anchor-key`、`data-phase`、会话头 `conversation.session.header` 及其 `actions`/`utilities`/`corner`/`lineage` 四座、`data-sidebar-collapsed`/`data-rightbar-collapsed`、`data-sidebar-right-panel`。分类例外：`data-mobile-*`/`data-plugin`/`data-tip` 是本插件自打标（不查上游）、`data-git-action` 查仓内 git 插件发射方（`CHAMBER_CROSS_PACKAGE`）；slot key 的证据形是 `renderSlot("<key>"` 这类 **key 发射点**（另有一条 advisory 检查确认渲染器仍以 `"data-slot": <标识符>` 把 key 投影成属性）；`_root_1b2ny_`/`_card_1b2ny_` 这类 build-time 哈希 token 零命中只 advisory（pin 一动必变，属 §7 重锚清单，非本仓可修的漂移） | 语料 = `<anchor-root>/node_modules/@deepseek-ai/**/lib/*.js` + `<anchor-root>/node_modules/@deepseek-ai/dsh-web-frontend/dist/assets/{index-*.js,index-*.css}`（shell bundle 必须在内：`ui-dockkit` 这类未单独发布的 client 包只在它里面，漏了会假红）；锚点根按 `--anchor-root` → `DSH_MOBILE_ANCHOR_ROOT` → 本机 gateway `dsh-anchor` → `packages/desktop/vendor/dsh` 取第一个含 `node_modules/@deepseek-ai` 者，**根缺失或无 client 产物 ⇒ fail-soft 跳过并打印缺什么、exit 0**（不把别的机器/CI 弄红；本门不依赖 `vendor/harness-checkout`，该子模块未初始化也不影响）；`data-*`/`role`/`slot` 零命中 ⇒ exit 1，哈希 token ⇒ advisory；exit 0/1/2 = 通过（含 fail-soft）/ 锚点红 / 用法错误。**与 C8 的分工**：C8 只比对「已提交产物 == src」（盯本仓陈旧），完全看不见上游；本门只判「上游还在不在发射」，是 C8 的正交补充；两者都不覆盖「pin 未动而上游源码已改」的中间态（那属 §7 的 pin 前移循环）。自测负例：`--simulate-rename main=center`（只在内存里改名、不写盘）或把产物复制到临时目录改名后用 `--anchor-root` 指过去；纯函数负例 `node --test scripts/dev/verify-mobile-anchors.test.mjs`。**接入状态**：负例测试已进根 `package.json` 的 `test:upgrade-tools`（`scripts/dev/verify-mobile-anchors.test.mjs`）；**仍待接入 CI workflow** 的是把 `node scripts/dev/verify-mobile-anchors.mjs` 加到 pre-install 腿（fail-soft 保证上游树未物化时不会红）——workflow 文件本次未改；**本门只判锚点、不判几何**：无横向溢出/会话头首行 ≤48px/无单字换行/命中盒 ≥44px 这些**设备模拟下的几何断言**由 `scripts/gui-acceptance/mobile-walkthrough.mjs`（CDP 设备模拟走查，含 WebSocket 帧抓取）负责——两者合起来才等于 design 17 §18.6 的「CDP 设备模拟 + 真机抽检」 |
