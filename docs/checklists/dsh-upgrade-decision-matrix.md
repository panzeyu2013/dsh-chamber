...
| patched-copy 基础设施 ✅ **已落地**（D3：`vendor-patches.mjs` 7 条/7 文件/21 锚点 + C9 + 产物断言） | S7 | renderer/构建面 |
...
| D3 | patched-copy 基础设施 | 现在建（第二例已触发）/ 继续逐例绕过 | **已建并落地（三轮）**：五处同源绝对 URL 全部走 `<basePath>`（见 C9）；剩余 = ssh/http dsh 目标无 cookie 注入 → 401（见 C9） |
