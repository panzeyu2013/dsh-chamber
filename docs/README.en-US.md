...
- **Sleep / background persistence** — close behavior is configurable (hide to tray and keep running, or quit with confirmation); launch at login (macOS/Windows/Linux); immediate reconnect on OS wake; keep-awake toggle
...
- **Windows install is slow / stuck on "Installing"?** — Windows Defender scans the ~33k runtime files one by one. Known tradeoff (design 23 F3): wait for completion, or exclude `%APPDATA%\@dsh-chamber\desktop` from Defender real-time scanning for a large speedup. Install/update progress lines say so.
