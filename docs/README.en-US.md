...
- Windows install is slow / stuck on "Installing"? — Windows Defender scans the ~33k runtime files one by one — known tradeoff (design 23 F3): wait, or exclude `%APPDATA%\dsh-chamber` from Defender for a large speedup; progress lines say so.
