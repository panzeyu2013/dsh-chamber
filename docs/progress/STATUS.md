...
- 实机门禁（未验证；缺真实实例 / 打包态环境）：
  ...
  - 切源后侧栏两个座席图标空白（机制已在WKWebView复现；触发条件与齿轮那半未坐实，2026-12用户报）：切源后左上 `sidebar.brand.mark`（HARNESS字标）与左下 `sidebar.settings`（齿轮）**不可**见但仍可点；其它JSX图标正常。① 这两个图标出自插槽座席，key属 `'sidebar'` children（`dsh-chamber-client-ui-sidebar/src/client/index.ts:125-156`）；② `sidebar.settings` 的 `renderSlot` 无fallback（`SidebarRoot.tsx:1851`），占位者在延迟簇（`renderer/src/chamber-entry.ts:325-330`，同文件308-312），冷启动 ~50ms；③ `single` 槽全部abdicate ⇒ `<div data-slot-error>` 死格、不回落owner fallback（`ui-renderer/src/client/scoped-slots.tsx:799-810`、`settings-bridge/README.zh.md:111` 契约）；按钮图标格可由 `BridgeEntryBoundary` 收口成死格（`SettingsShell.tsx:514/610/872`）。待判（判据）：A) 新boot/重连窗口（控制面日志同刻出现该来源mux `closed/upgrade`）；B) 纯座席派发/贡献丢失（mux未断）。根因（2026-12已定位，WebKit复现）：`opacity: 0` 停在入场动画首帧——`SidebarRoot.module.css` 的 `.wide { animation: wide-in 200ms }`（`@keyframes wide-in { from { opacity: 0 } }`，挂 `SidebarRoot.tsx:1684` 的 `clsx(css.brand, css.wide)`）与 `.railIn .footArea { animation: rail-fade-in 150ms backwards }`（`from { opacity: 0 }`，`.footArea` 包 `sidebar.settings`；`.railIn` 长挂，`backwards` 生效）；同类 `.fading > *`、`.railIn .iconButton/.newSession/.regionArea`。动画只在子树渲染时推进：N-ctx隐藏壳（`styles.css:129` 的 `content-visibility: hidden`）、窗口遮挡/离屏、View Transition快照期停住时间线，元素永久停在首帧 `opacity: 0`。WKWebView实测：`.brand.wide` 计算 `opacity: 0`、动画 `wide-in:running:0`、DOM SVG 182x24完好、快照0墨迹。仍未闭合（2026-12）：①故障态真机取证未做（判据 = 量两个座席的 `opacity`/`getAnimations()` 或一张故障态截图）；②触发条件未坐实——隐藏壳/遮挡/View Transition三条候选都不能单独解释「切源」时刻。基线（4个from-opacity:0退役 + 隐藏壳门）见design 06 §7、design 05 §4；回归锁 `dsh-chamber-client-ui-sidebar/test/visual-lock/`。
