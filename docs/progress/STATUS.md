...
- **T3 openin 插件统一 —— Batch 3 Phase 0 ✅（2026-09）；Phase 2 待执行**：Phase 0 已落地 per-source 视图模型（`shared/open-in-view-model.ts`：official/main 双池 × 来源矩阵 + 显式抑制原因 + channel 去重 + 默认选中；既有 `usableOpenInApps`/`usableAppsForSource` 改为其薄适配层；8 例单测钉规则）。**Phase 2 剩余（本批次核心）**：官方 client 源吸收进 `dsh-chamber-client-ui-open-in`（组件/菜单/css/locale/标签表/选择持久化 + `source-adapter.ts`：per-source 双池选择、`<basePath>/open-in-app/*` fetch/图标重映射、per-entry 通道路由）；desktop 主进程瘦身至 vscode-only provider（finder/stat/openPath IPC 退役，proof/生命周期/队列保留）；测试重写 + lockstep（host-graph、ipc-surface-mirror、cross-package-contract、renderer-trust）+ verify:i18n；**红线修订登记先行评审**（design 05/16/20/AGENTS：本地 launch 信任界由 trustedIpc 迁至实例官方路由）。[UNVERIFIABLE]（须实机）：官方 host 行是否随 a2 默认 profile 进入托管实例（静态上在：`packages/bundle/web-app` 依赖 `dsh-host-open-in-app` + `dsh-client-ui-open-in-app`）、远程无 cookie 下 fence 行为、remote cwd 填充、图标缓存/CSP。
...
- **批次进度**：Batch 0（T1）✅ → Batch 0.5（T4）✅ → Batch 1（T2）✅ → Batch 2（fork 重锚）✅ → Batch 3（T3）Phase 0 ✅ / Phase 2 待执行，每批独立提交与绿门。
