...
- 会话链重构（design 14 §D4）：**仍开放**
  ...
  - **反补丁波次（门禁先行）**：不变量契约（I1–I7）与门禁映射见 design 14 §D4。未闭合：①阶梯决策边界：`planLadder` 持调度半，open-in 的具象 `resync()` 与 producer 的权威执行端仍在宿主（phase 机不进引擎；stage 迁移已随 rc.2 删除，resync 是唯一自动杠杆）；②子代理完整性信号仍是上游依赖（本地已三值化：索引缺席/stale 降 `unknown` 中性呈现，见 upstream-proposals §7）；③载波身份边界仍是请求键（加宽账本/期限装载已归 reducer；per-stream episode token 需上游在 `open` 回调给出或由宿主承担首帧期限，见 upstream-proposals §3/§8）；④P6：上游 `session/follow` 首帧期限/`doOpen` 错误契约仍待落，退役条件由 `verify:upstream-lifecycle-contract`（pin 源两半 + 负控）钉住，落地即删客户端加宽阶梯；移动端 JSC/rAF 与真机语义仍只有实机门禁；G-H 面仍有 23 个聚合/镜像/差分面各自带退役条件登记。
...
- macOS 窗口 chrome 实机验收（待打包 .app；rc.2 官方 `shell.leading` 单席）：折叠态（darwin + 侧栏折叠）红绿灯旁应可见「展开 / 新建」两钮、点按生效，`--dsh-frame-leading-clearance` 计价不压主面板内容；展开态同一条带（ui-sidebar `.topStrip`）的面板开关应停在红绿灯行（28px 控件 11–39、中心 y 25，与折叠座同高）。实机判据（契约 design 05 §2、design 06、design 25 §5.6）：① 灯与开关同一水平线，且缩放/最大化、渲染恢复后的标题变更、跨屏、最小化恢复、进出全屏后都仍在同一行；② 侧栏列可见窗后模糊（浅/深色各一次）、中列仍是不透明底，Reduce Transparency 打开时转 90% 不透明填充；③ 最小化→恢复、隐藏→Dock 恢复时侧栏不瞬时透出桌面；④ 进出全屏时页面切到 `html[data-fullscreen]`（灯带让位、座位不跳）；⑤ 失败说明页在材质腿下可读（自带不透明底）；⑥ 按住顶部带或会话头部能拖动整窗、控件仍可点击。组装：`pnpm run build:sidecar` → `pnpm run build:swift-app`（见 AGENTS.md 的 Swift 壳流程）。
- **Electron macOS 腿未收口**：该腿窗口仍是系统标准标题栏（`packages/desktop/main.ts` 只对 win32 走 hidden 分支），同一条带与 `--dsh-frame-top-clearance` 在这条腿表现为内容区顶部空白；收口 = 窗口转 `hiddenInset`（`data-window-drag` 标记已按上游补齐，app-region 规则与 `window-drag/recall.ts` 随 client-web fork 就位，见 [todo/main-0.1.5-to-0.1.7-upgrade.md](todo/main-0.1.5-to-0.1.7-upgrade.md)）；**材质（vibrancy）、`html[data-fullscreen]` 镜像与页面侧 darwin 透明面（按 `data-window-vibrancy` 门控，该腿不命中、观感不变）同样只有 Swift 腿**，三者随这条腿转 hiddenInset 时一并接（届时该腿 preload 需同落该标记）。
...
- 设计21网关插件能力对齐（写面已随「C 分层」退役）：剩余实机项 = 只读通道（清单/行投影/能力探针）与 seed 供给面在真实 gateway×desktop 的端到端核验（发布前执行；.172 升级因凭据轮换暂停待恢复）；UI 余留：who/when tooltip 未渲染、`pollGatewayReady` 英文串未本地化；archive-pick 双模式对话框为 macOS-v1。

- 产物新鲜度守卫（§6.11 / design 21 §7）：`desktop/dist/control-plane/**`（`packages/desktop/scripts/control-plane-freshness.test.mjs`）与 `gateway/dist/**`（`packages/gateway/test/packaging/build-smoke.test.ts`）是标记守卫，`scripts/gates/verify-artifact-freshness.mjs`（tests/full，经 `ci.yml`）另做重建-比对，C8 覆盖构建期生成物 6 组（清单单源 `scripts/lib/build-artifacts.mjs`）——已落地门的清单不在本记录复述。**仍无守卫**：`desktop/dist/web/**`（仅 scoper 标记由 `packages/dsh-chamber-client-ui-mobile/scripts/assert-scoper-artifact.mjs` 在 CI 断言）、`dist/host-{graph,git-worktree,archive-cleanup,open-in}-package/**`（只断言行序/outDir）、vendor `allowBuilds` 锁步（G7）。未落地的最小守卫建议 G2/G3/G5/G7/G8（G1 已落地 `verify:test-wiring`）见 `todo/refactor-plan.md` §8）不在本记录复述。失效判据 = 每个产物有「陈旧 ⇒ 红」守卫或进入豁免表（G8）；G1=`verify:test-wiring` 已落地（`desktop/dist/web/assets/*.js` 的 scoper 标记由 `assert-scoper-artifact.mjs` 在 ci.yml 断言，本仓无 node_modules 时该步只在 CI 生效）。
...
- **A1状态呈现面（普查；F1已修，四组合由 `packages/desktop/test/plugins/plugin-sync.test.ts:333/:353` 钉住）**：F2/F3/F7 随 2026-09 C 分层写面退役清除（安装结果的重启承诺、`materializeLive`、ssh bundle 断言；`bundleLines`/`dsh.bundle` 消费面已全局消失）；S2/S6 的写-only API（`clientPluginRowLoaded`/`cachedSourceClientGraph`/`notePluginMounted`/`clientRowSignatures`）已无全局引用、随清理消失。仍成立的读面剩余（逐条按当前代码复核实）：F4 `instance-version-conflict` 断言「插件版本不同」而输入建立不了——判据 `packages/dsh-chamber-client-core/src/client-plugin-loader.ts:140-144`（唯一输入=不同 source id+不同 `rev`，`rev` 是 per-process nonce 非内容哈希）→ 文案 `packages/renderer/src/host-graph.ts:487-496`，措辞仍被 `packages/renderer/test/lifecycle/host-graph.test.ts:783-800` 钉住；F5 同源 `restart-required` 报「另一版本」（`host-graph.ts:497-501`：同实例重启一次、字节不变而 nonce 变即触发）；F6 侧栏 recheck 仍可把 boot 会拒收的图治成 `ok`——共享分类器只做 envelope+`id/url/rev` 基门（`packages/dsh-chamber-client-core/src/plugin-graph-classify.ts:134-143`），boot 另校验的 `inject`/`external`/`immediately`（`host-graph.ts:134-139`）不在 recheck 内，可选字段畸形的图仍被判 `ok`（`packages/dsh-chamber-client-core/src/plugin-graph-recheck.ts:100-106`）。S5 写面对话框形态同随退役消失；仅存的 gateway「重新同步」判据已收口：`chamberSyncUploaded` 文案改为「已同步并已请求重启」，不再断言重启成功（restart 非 2xx/异常仍只 `warn`：`packages/desktop/gateway-provider.ts:1157-1178`；如需在 UI 呈现重启结果须给 IPC 结果加字段，暂不接线）。疑似未动：S1 recheck 的 `ok` 没有「行已装载」事实（`plugin-graph-recheck.ts:100-106` 只按通道裁决；对照 boot 真装载行 `host-graph.ts:423-428`；待证据=retry 用尽后通道恢复且未重挂而诊断翻 `ok`）；S3 `writerReasonKey` 未知 token 默认译成「身份无法核对」（`packages/dsh-chamber-client-ui-settings-connections/src/client/writer-diagnosis.ts:67`；原始 token 旁可见⇒低）；S4 `chamberRemoteKey` 家族级匹配任意同族 entry（`.../plugin-inventory-text.ts:140-143`；当前不可达⇒潜伏）。失效判据：其余各条修复或显式裁决（原普查台账已删，本条即权威）。
...
- 渲染进程在 JavaScriptCore 崩溃 → 静默整页重载（实证）：JSC 引擎缺陷（WebKit 22625；EXC_BREAKPOINT/EXC_BAD_ACCESS
  ...
  仓内只能收敛触发面与可见化，不能消除（收敛项 design 14 §D4「第三次排查」、壳侧归因 design 25 §5 E19）。**未闭合**：
...
- 会话可靠性本轮剩余验收：① 用同一份大历史会话在当前 Electron 与打包 Swift 注入丢 status、迟到基线、快速重跑、断流、WebContent 崩溃、sidecar 重启，并复核可信完成的原生投递；纯测试只验证时序，不能代替 OS 横幅回执。② Swift 真机隐藏、完全遮挡、唤醒与 60/120Hz 仍需测；JSC 崩溃若仍复现，发布门保持关闭。③ 流层“消费回调已调用”仍无 React 提交与 WebView 像素绘制的独立可观测游标；画面停顿须用真机帧证据定位后再决定局部重建层。④ 原生通知显示与 renderer 落盘间没有原子事务，宿主进程在显示后、记回执前崩溃的窗口需故障注入验收。⑤ 旧运行时完成仍待投递且新一轮 true/false 均丢失时，两证据通道缺共同运行轮次键，host 新水位无法无歧义归属；需宿主轮次键或端到端注入后裁定重复/漏报边界（design 19 §6）。⑥ Swift 有界写器的满管道、半帧写失败与 sidecar 代际切换仍需在 macOS XCTest / 打包态注入验证；Linux Foundation 压力冒烟只证实调用不阻塞与失败可终止，不能替代 Darwin 管道行为。
- 双帧 status 全丢的证据边界仍开放：无壳观察者仅在 `session/list.updatedAt`（最近用户提示时间）前进且读到**严格更晚**的 host `turn/end.time` 时能恢复新完成；同毫秒、没有新提示水位、持续读不到尾巴均不能据 `running=false` 分类。gateway 的 `SessionTurnEnd.at` 是观察者时钟，当前镜像只能撤销旧完成并保持未知，不能以它同 host 提示水位比较；需要宿主运行轮次键或可信 host 尾时间及端到端注入判定（design 14 §D4、design 17 §10.7）。无壳观察者的 `session/follow` 已改走 remote.mux 逻辑流；剩余验收须用真实宿主核对 opening snapshot、后续 event、cancel 与 2s 无尾期限，旧 HTTP unary 假件不能证明这条协议链。
...
- 打开意图 / 工作区回声两项真机反馈的实机验收（design 05 §2.2.1）：① 本地会话点远程来源会话行：揭幕期只显加载层、无高亮「新会话」行、不切新会话主页；② 远程来源新建工作区（来源头「添加工作区」与workspace行尾
  ...
  临时日志，不提交）**：包装 `uiWorkspace.openSession` 打调用栈+`performance.now()`（rc.2 已无 `sessions.open`）、打印 `runtimeFacts[id].current`、看 `localStorage['dsh.sessions.current']`；期望冷boot先 `session.create` 后目标id、早开臂抢在
...
- 远端完成未读 / 切源体验（design 17 §10.7 只读镜像 carve-out；实现面已闭合，仍开放的是实机/CI 权威）：
  ...
  **实测的精确阻断点**：没有带 CDP 的 Electron 实例——`curl http://127.0.0.1:9333/json/version` → 000，
  ...
  `NODE_PATH=<sidecar>/vendor/dsh/node_modules` 已可解析，不再是阻断。
...
- **结构性重构与清理（未闭合；计划与开放项见 [todo/refactor-plan.md](todo/refactor-plan.md)）**：
  ...
  `ssh:<id>` 别名（design 17 §2.2 深链契约）与旧 dsh/gateway 上游版本探测——经用户裁决明确保留。
...
- docs 证据锚点过期（D15；低–中，未排期）：`docs/**` 的 `文件:行` 锚点写死、位移不均；工具 `check-anchors.mjs`
  （`anchors-budget.json` 只降不升棘轮 + `--report`/`--fix`）已进 `check:static` 与 CI，当前预算与实测口径见
  D15 与 `anchors-budget.json` 的 note（不在本记录复述快照）。退役动作 = 合并分支落地后按符号 grep 分批语义化重锚、
...
- 侧栏行悬停卡片由本仓自持（偏差；上游修掉竞态即可退役）：vendor `ui-primitives` 的 `HoverCard` arm宽限关闭依赖**上一次已提交的`open`**（`onPointerLeave` 每个 arm 由它守卫，rc.2 亦然——相位机只把开卡语句 `setOpen(true)`→`setPhase('open')`，宽限手臂逐字未变，C15 判据已按新形状更新：dwell 定时器按 `openDelayMs` 延迟识别），本仓每实例一大React root⇒pointerleave漏arm、卡片无法关闭。改由`RowHoverCard.tsx`+`shared/hover-intent.ts`渲染；有意偏差与内容差异见design 06 §7（`ResizeObserver`重算等）。退役条件=上游修掉竞态；`verify-upstream-touchpoints.mjs` C15断言竞态两侧形状+时间常数锁步，上游一修升级pin时本门先红。证据：`sidebar/test/session-rows/hover-intent.test.ts`；实机走查W-4b/-race/-swap/-dismiss。
...
- N-ctx原生键盘路由（RC-C4偏差，design 25 §4.4.1 键盘座席）：载体层按「单窗口=单实例」设计——一个 `DesktopShortcutsBridge` 单槽持有 definitions/keys/revision（`packages/desktop/shortcuts-bridge.ts:245,278-282`；main 只装一个门 `packages/desktop/main.ts:695-706,1240-1246`），各实例 boot 时官方 `shortcuts.get(自己的目录)` 后写独占、末位实例目录判 preventDefault 与菜单抑制；投递 input 只带共享 revision（`preload.cts:899-921` 向页面广播；vendor `packages/client/shortcuts/src/client/native.ts:17-18` 各实例只比对 revision，CHANGED 广播又令各实例收敛到同一 revision）⇒ 输入被全部已 boot 实例 accept 并各自 dispatch（隐藏实例也会执行命令）。按活动源路由须同时改 renderer（启动期实例标记 + 活动源广播）、preload（订阅归属/投递过滤）、main（每实例目录 + 活动实例）三层，且 Swift flavor 的 DOM 桥（`macos/Sources/DSHChamber/Resources/bridge-shim.js:604-705`）必须同构，故本轮不实现。证据＝`packages/desktop/test/ipc/desktop-shortcuts-bridge.test.ts` ⑪（单槽行为锁）。实机验收项：≥2来源（local+SSH/HTTP）下焦点在 B 按绑定键只允许 B 动作（在 A 宿主查无会话/侧栏/设置副作用）、A 独有绑定不得按末位目录被误 preventDefault、切源与壳回收重开后判据不变；Electron 与 macOS Swift 两 flavor 均验。收口触发：上游给 selection/命令目录加入口作用域，或 chamber 决定为每实例拆分键盘投递。
...
- 上游对齐轮引入的有意偏差（仍成立）：
  ...
  - **框架失败屏深引 `ui-primitives/src/Button.tsx`（不引barrel）——打包预算决策：深引避免barrel把markdown/CodeBlock家族拉进主图。最近实测**（`build:renderer`写`desktop/dist/web/perf-sizes.json`，门值`check-chunk-budgets.mjs`）：主图 1,434,814 **已越过`mainGraphRaw.warn = 1,350,000`（超约6.3%；距`fail = 1,550,000`余量约7.4%）**；chamber入口 7,595,113 为`chamberEntryRaw.warn = 2,000,000`的约3.8倍（该指标有意只有warn、无硬门）。**待决**：拆出/懒化首屏家族，或上调阈值并把理由写进脚本头注。失效判据：读数回门内或阈值调整与理由落进头注。
...
- settings簇deferred C4（性能审计，已登记chamber-entry.ts/chamber-covered.ts注释）：官方ui-settings保留首屏（locale/ui-theme root-inject `configForms`，defer即瘫痪壳）；后移4个官方settings section + chamber
...
- **第三类 vendor 补丁准入（模块私有状态机的逻辑缺陷；维护者裁决）**：`packages/renderer/scripts/vendor-patches.mjs` 原先只收两类（N-ctx 同源/document-relative URL 硬假设、实测帧成本）；`ui-chat` 的 `chat/use-chat-reading.ts` 采样 settle 属第三类——viewport 把「仅位置变化」记成 reader 移动，位移落在跟随容差（`FOLLOW_THRESHOLD` = 24px）内时旧代码保留残余偏移 ⇒ 最新一行半遮在输入框上方且不出现「回到最新」，直到下一次布局变化（实测：真实前端 12px 非 reader 偏移在 `data-chat-following-tail` 保持的情况下永久残留，任一后续布局变化即回贴）。补丁把 settle 改为「仍在跟随即回贴」，与 `onScroll` 首分支 / `onResize` 在同一容差内的既有行为一致；接受的取舍 = 24px 以内的真实阅读者微调也会在 settle 时回贴。**待办**：上游带上修复（首选形状 = `use-chat-viewport.ts` 按意图归因）后删除该条；触发侧仍未定——直接 bash 调用的 preparing→started 整行替换是相关项，WKWebView 里移动滚动位置的那一步未定位，PTC 子调用不渲染准备行故不复现。登记面：design 09 §3.6 第三类、触点表 §3、`reason` 字段、C9 锚点门与该条的 execution test（含上游负控）。
