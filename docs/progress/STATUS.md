...
- 会话链重构（design 14 §D4）：**仍开放**
  ...
  - **反补丁波次（2026-12，门禁先行）**：不变量契约（I1–I7）与门禁映射见 design 14 §D4。P0 门禁已落、P1 已转绿 G-B/G-C/G-F、P2 已转绿 G-E、P4 已转绿 G-D（`sourceId → {epoch, incarnation, state}` 单世代注册表；指纹变化只在权威 roster 刷新处 `reincarnate`，事件携带捕获的 epoch（错代丢弃），退役经 `retainSourceIds` 出表；删 `incarnationKey`/`retainSources` 键式记录面与 App 的逐事件指纹重算）。G-A 亦已转绿（删除无生产者的 `RecoveryEffect.reconcileFacts` 与 `SourceEffect.note`）。P5-6 已落（包内 bounded forensics ring：cap 256 + 记录时脱敏 + 非有限时间戳保守为 0；reporter 记录 `carrier-rebuild`/`carrier-throttled` 决策事实，探针经 `stream-forensics-request` → `ForensicsSink` 导出；公共面只留 factory/界常量/端口类型）。仍红/待办：G-H = 29/62 运行时导出无生产 importer（P3/P5/P6 收口，阶梯引擎为主）；P3 载波单所有者待先定 episode 身份契约（`RemoteStream`→`RemoteStreamOptions.open`→mux，当前唯一稳定身份是 FNV 请求键）；P5 剩四条阶梯收编（mobile/renderer liveness/open-in chip/sidebar）与子代理 `subagentActivity` 三值化；P6 按计划。退役判据：每阶段同变更删除被替代实现；G-H 不得新增无退役条件的豁免；`run-checks full` 的 Linux 可达面在各阶段末复跑。
