...
- **0.1.6→0.1.7 升级线的取件移植（计划见 [todo/main-0.1.5-to-0.1.7-upgrade.md](todo/main-0.1.5-to-0.1.7-upgrade.md)）**：
  ...
  读面能力门（§12.8）、会话流健康臂的 rc.2 重判（§12.9——臂已在 main 按 rc.2 口径重推导，分支 0.1.6 形态不取件；
  残余真机验收见本文件 ⑫）。
  **未落地**：① §12.6 的 Electron 腿 native theme seat（`data-ds-theme-source` → 新 invoke 通道
  ...
  （`prepare-python-payload` + `primary-runtime-lock.json` + extraResources/发布链）；③ §12.10 启动失败恢复
  （`startup-error.ts` 与 Swift 恢复框）；④ §17-C 上游 patch 通道（`patches/` + `patchedDependencies`）。
