...
- macOS 窗口 chrome 实机验收（待打包 .app；rc.2 官方 `shell.leading` 单席）：折叠态（darwin + 侧栏折叠）红绿灯旁应可见「展开 / 新建」两钮、点按生效，`--dsh-frame-leading-clearance` 计价不压主面板内容；展开态同一条带（ui-sidebar `.topStrip`）的面板开关应停在红绿灯行——28px 控件 11–39、中心 y 25，与折叠座同高（收展不跳位），字标不再贴红绿灯下沿（契约 design 05 §2 / design 06）。**窗口拖拽**（2026-09 补齐，design 25 §5.5）：隐藏标题栏后整窗没有原生可拖区（实测 WKWebView 的 `mouseDownCanMoveWindow` 恒 false，`isMovableByWindowBackground` 对页面无效），拖动面 = 页面标记行（`data-window-drag`：侧栏顶部带、logo 行、会话头部等），壳经独立通道合成 `performDrag`；实机判据 = 按住顶部带或会话头部能拖动整窗、控件（面板开关、按钮、页签）仍可点击、拖拽中不选中文本。**Electron macOS 腿未收口**：该腿窗口仍是系统标准标题栏（`packages/desktop/main.ts` 只对 win32 走 hidden 分支），同一条带与 `--dsh-frame-top-clearance` 在这条腿表现为内容区顶部空白；收口 = 窗口转 `hiddenInset`（`data-window-drag` 标记已按上游补齐，app-region 规则与 `window-drag/recall.ts` 随 client-web fork 就位，见 [todo/main-0.1.5-to-0.1.7-upgrade.md](todo/main-0.1.5-to-0.1.7-upgrade.md)）。
