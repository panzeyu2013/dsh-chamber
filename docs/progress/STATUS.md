...
- 实机门禁（未验证；缺真实实例 / 打包态环境）：
  ...
  - **切源后侧栏座席/字标与设置导航齿轮空白**（首报场景：切换来源后）：根因、修复契约、被拒方案与残余边界见 design 05 §4.2；回归锁 `packages/renderer/test/svg-resource/`（SVG 自足面）与 `packages/dsh-chamber-client-ui-sidebar/test/visual-lock/`（入场动画面）。**开放项**：① 真机验收未做——需在**重新构建的产物**上重放触发序列（切来源数次 + 开设置面板，rail↔wide、字标/座席一并看）确认不再空白；判据与命令见 design 05 §4.2（`node scripts/dev/svg-resource-probe.mjs --expect-artifact`，人工验收工具、不进 CI、需控制面在跑）；② gateway/mobile 独立部署的官方壳未覆盖——chamber 侧可选收口是把 scoper 装进既有打包插件 `packages/dsh-chamber-client-ui-mobile`（需范围决策），否则等上游/runtime 侧修。
...
- 会话打开停滞（「载入历史…」永久停留，2026-09-14实机）：大会话（`session-28e9eb86`）经gateway打开只显示 `chat.loadingHistory`。根因未证实（宿主侧实测健康：经代理 `$events` 就绪 25ms、普通快照 57ms、已结束子代理快照 73ms）；唯一同构状态 = mux socket正常而 `session/follow` 逻辑流永久无首帧，客户端侧 2026-09 起有 fork 首帧期限（30s 起，连续超时 30→60→120→240→300s）+ 开帧发送前校验（design 14 §D4），宿主侧仍无；收口需设备侧帧证据（CDP WS Frames/抓包），入口 `mobile-walkthrough.mjs`（`mobile-ws-frames.json` 落盘前脱敏）。插件侧「停滞提示 + 主动重载」兜底（`session-stall.ts`）判据全为属性锚点，45s阈值未经真机校准；桌面侧同形兜底 = `session-stream-health.ts` `loading` 臂（20s阈值，同未经真机校准），另加 `error` 臂自动stage迁移重开（见 ⑫）；形态取值/误报边界见 `session-stall.ts` 头注与 `README.md`「Anchor baseline」。2026-09 客户端侧已有开帧校验 + 首帧期限（30s 起，连续超时 30→60→120→240→300s；design 14 §D4）；2026-09-21 追加**静默载波升级**（`shouldReplaceSilentSocket`：首帧期限到期而当前 socket 一帧未收 ⇒ 走 `replaceSocket()` 换掉物理 socket，专治「浏览器视角 OPEN 而载波静默」这一类——判定阈值未经真机校准，误报代价 = 多一次载波重启 + 基线重放；`socket-silent` 事实的持久消费面同 ⑩，宿主「socket 活着但 follow 永不回答」仍只能靠座席出口）；另有一条**同症状的 vendor 窄缝**（仓内不可修，仅登记）：`Session.doOpen()` 只把 `isRemoteFailure` 的错误写成 `error`，非 RemoteFailure 抛出、或 `events.open()` 成功返回时 `openGeneration`/`events` 已被推进（dispose/resync 竞争），都会**静默 return 且把 `openState` 留在 `loading`**，而 `followCurrent()` 只在 stage 移动时才再次调用 `open()`——恢复面因此仍只有座席出口（真机未观测到该分支，判据 = 事故后能在 renderer console 里把「loading 驻留」与「open 抛错」对上）。2026-09-21 追加座席**证据门自动重建**：`loading` 且 probe 读到具象 `openPromise` 为空（无在途 open）⇒ 自动 `resync()`（共用同一 cooldown/预算账本），在途 open **绝不打断**（慢宿主）；`loadingFailedMs`=90s 后提示改为「会话内容未载入」并保留重建/重新加载，手动出口不再受账本约束（design 14 §D4「进入会话必须收敛」）。**仍未 100% 的残余 = 宿主侧**：`session/follow` 无首帧期限、`doOpen` 可把 `loading` 静默留下，两条均已登记 `docs/progress/todo/upstream-proposals.md` §4.3/§4.7；阈值 90s/20s 未经真机校准；移动端 `session-stall.ts` 已接同一证据门自动臂（`sessionStallFace` 读具象 `openPromise`；触屏档无 chamber fork ⇒ 载波层升级不覆盖该档，其恢复面 = 该自动臂 + 手动重载）。`loading` 卡死另有座席「重建对话通道」用户出口，不再只有 ⌘R。（同族见上方「会话运行位卡死」与design 14 §D4；本首帧期限与之同缺口——上游提案见 `docs/progress/todo/upstream-proposals.md` §4。）

  **2026-09-21 独立评审（4 个只读 subagent：载波/阶梯/端到端/对抗）后的收口与新增未闭合项**：已修 = ① 移动端自动臂补**loading 门**（只读具象 `openState === 'loading'`；健康 `open` + 首轮慢与停滞同形，仅 `openPromise === null` 会误重建）、`sessions` 服务**每次调用重解析**（apply 早于服务注册不再永久失臂）、时钟回拨视为账本已结算；② 桌面 probe 证据收紧为「own 成员**恰为 null** 才算无在途」（`undefined`/异形一律 unknown fail-closed）；③ `error` 臂手动「重建对话通道」去掉账本门（与 loading 臂对称，兑现「手动出口不受账本约束」）、点击加 1s 双击守卫；④ mux 开帧预算**按逻辑流 episode 结算**（消费者离开且无同键活流 ⇒ 清零，重建后的首帧重新从 30s 起，不再继承最长 300s 的放宽）、`openingTimeouts` 加 256 条上限、「同 task 内 open+close」的永久停放点补 microtask 重排；⑤ 移动端 `lib/client.js` 已重建（109,928 B，含 loading 门/失败文案；`lib/` 入库 ⇒ 必须随源码重建）。新增未闭合 = ⑥ **触屏档无载波层（桌面侧 2026-09-21 已收口）**：桌面侧「已开启会话的载波半死」已由 **teardown 升级**修掉（任意逻辑流 teardown 且该 socket 全程零帧 ⇒ `replaceSocket()`；行为用例 + 变异验证，commit 066c3668；20s abort 早于 30s 开帧预算的场景已覆盖）；但**移动档跑实例自带客户端栈、没有 chamber fork ⇒ 该升级与首帧期限都不存在**，其载波半死仍无页面级恢复；桌面 `open` 臂在该窗口也**没有任何按钮**（静默 socket 不发 carrier-churn 事实）；⑦ chip 的可见恢复面（`setPlan`/ticker/`onClick`/auto 分支）在包内**无行为用例**（无 React/DOM 运行环境），4 个行为反转变异全绿，只靠源码文本锁；⑧ 承载 `loading` 观察的 header 内容在 `session.blank && phase==='blank'` 时上游不渲染 ⇒ 该子形态可能既无 chip 也无出口（静态链路已核，待真机判定）；⑨ 移动端产物缺 source↔artifact lockstep 断言（先例见 `official-hover-card.test.ts`）；⑩ fork 的 M3/M4/M6/M10（首帧成功清键、`replaceSocket` 的 `failAll`、开帧前校验、退避翻倍轨迹）只有源码锁，行为用例缺（评审已证明可补：P12/P13/P14）；⑪ 开帧预算键是 32 位 FNV（约 16 万次抽样一次跨会话碰撞，只共享预算不等价）；⑫ 不可 JSON 序列化的 payload 让 `open()` 抛 TypeError 而非 carrier 类（「载波永不终局」的例外，生产不可达存疑）；⑬ `socket-silent` 事实的持久消费面仍⑮ `presented` 是 document 级近似（renderer 非活动来源靠 CSS visibility 保活 ⇒ 隐藏实例 chip 仍满足 presented，可能对没在看的会话执行 auto-resync/stage 迁移，账本有界 1–3 次/10min）；⑯ **设计未决**：unary 引导通道（`session/control` baseline `asOfSeq` 作合法 `throughSeq` + 冷读 `session/page`）是否新写成兜底内容引导 —— 本轮未采纳（救不了宿主卡死、属非契约窗口写入面），见 design 14 §D4 与上游提案 §4.3。缺。
...
- N-ctx壳常驻语义收窄（2026性能整改偏差，design 05 §4）：「booted壳无限常驻」收窄：local恒留，隐藏壳最多保留`RETAINED_HIDDEN_VIEWS=1`，超限回收「已settle+连续隐藏≥60s」最久者（回收= dispose shell +卸载壳，实例进程/连接不受影响，重开冷boot）；被回收源不再自动预热直到用户点开；运行中任务完成蓝点/通知边沿随壳回收暂停至重开。
