...
- 会话链重构（design 14 §D4）：**仍开放**
  ...
  - **反补丁波次（2026-12，门禁先行）**：不变量契约（I1–I7）与门禁映射见 design 14 §D4。P0 门禁已落，当前红名单即待办，逐阶段转绿：G-A = `RecoveryEffect.reconcileFacts`/`SourceEffect.note` 无生产者（P3/P4 处置）；G-B = 非有限钟可派遣 tiers、`loadIsLate(∞)` 判晚、`withDeadline(ms:NaN)` 0ms、`onExpire` 抛错不结算；G-C = 重建账本与阶梯派遣账本无界；G-D = 同 id 双世代投影合并、错代事件仍可派发（P4 注册表）；G-E = 无 `planVeilTimer`、held 帧可 0ms 重臂（P2）；G-F = Swift `shouldRebuild` 非有限钟与终局态 `isLate`；G-H = 30 个运行时导出无生产 importer（P3/P4 删除、P5 接线、P6 收口）。退役判据：每阶段同变更删除被替代实现；G-H 不得新增无退役条件的豁免；`run-checks full` 的 Linux 可达面在各阶段末复跑。
