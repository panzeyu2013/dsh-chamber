...
- macOS 窗口 chrome 实机验收（待打包 .app；rc.2 官方 `shell.leading` 单席）：折叠态（darwin + 侧栏折叠）红绿灯旁应可见「展开 / 新建」两钮、点按生效，`--dsh-frame-leading-clearance` 计价不压主面板内容；展开态同一条带（ui-sidebar `.topStrip`）的面板开关应停在红绿灯行——28px 控件 11–39、中心 y 25，与折叠座同高（收展不跳位），字标不再贴红绿灯下沿（契约 design 05 §2 / design 06）。**Electron macOS 腿未收口**：该腿窗口仍是系统标准标题栏（`packages/desktop/main.ts` 只对 win32 走 hidden 分支），同一条带与 `--dsh-frame-top-clearance` 在这条腿表现为内容区顶部空白；收口 = 窗口转 `hiddenInset` + 按上游给顶部带与 logo 行加 `data-window-drag`（app-region 机制与 `window-drag/recall.ts` 已随 client-web fork 就位，见 [todo/main-0.1.5-to-0.1.7-upgrade.md](todo/main-0.1.5-to-0.1.7-upgrade.md)）。
