...
- macOS 折叠态 leading 控件实机验收（待打包 .app；rc.2 官方 `shell.leading` 单席）：darwin + 侧栏折叠时红绿灯旁应可见「展开 / 新建」两钮、点按生效，且 `--dsh-frame-leading-clearance` 计价不压主面板内容（契约 design 05 §2 / design 06）。
