...
- 会话链重构（design 14 §D4）：**仍开放**
  ...
  - **反补丁波次（2026-12，门禁先行）**：不变量契约（I1–I7）与门禁映射见 design 14 §D4。P0 门禁已落、P1 已转绿 G-B/G-C/G-F（时间纪律、账本上界、Swift 非有限钟与终局态 `isLate`；`src/time.ts` 为唯一时间所有者）。仍红/待办：G-A = `RecoveryEffect.reconcileFacts`/`SourceEffect.note` 无生产者（P3/P4 处置）；G-D = 同 id 双世代投影合并、错代事件仍可派发（P4 注册表）；G-E = 无 `planVeilTimer`、held 帧可 0ms 重臂（P2）；G-H = 运行时导出无生产 importer（P3/P4 删除、P5 接线、P6 收口）。退役判据：每阶段同变更删除被替代实现；G-H 不得新增无退役条件的豁免；`run-checks full` 的 Linux 可达面在各阶段末复跑。
