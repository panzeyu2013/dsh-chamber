...
> S-48（ProMotion 实机）、S-49（窗口高度单侧对齐裁决）、S-50（根级回弹实机）、S-52（Sparkle quit 与在飞
> 关窗相撞）、T-28（corner-shape 单点裁决）、D15（文档锚点过期）、G19 与 G43（CI 内打包 .app 启动）。
...
|S-49|初始窗口视口高度不对称：Swift内容区1280×786（外框 ~814）vs Electron外框1280×800（视口1280×772）——折中后两端仍差14pt（~1.8%），侧栏/工作区列可见高度差一档（折中前28pt）|折中：不单侧对齐，原生内容区取786使两端各偏 ~14pt；单侧对齐（原生取772，或Electron开`useContentSize`后两端同取800）仍未裁决|E `packages/desktop/main.ts:870-872`（无useContentSize）↔S `macos/Sources/DSHChamber/MainWindowController.swift:57-69,350,375`（+ `ShellStartupTests.testInitialWindowGeometryMatchesTheRecordedCompromise`钉住786与两个消费点都走`Self.windowSize`）|open（待单侧对齐裁决）；裁决后按结论改accepted/resolved或退役本条|
