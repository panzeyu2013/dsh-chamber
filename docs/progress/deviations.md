...
|T-30|窗口拖拽实现|`-webkit-app-region: drag` 由 Chromium 原生消费（`packages/dsh-client-web/src/base.css:55-58`；几何盒序合成，标记行清单 = ui-theme `app-region-styles.client.spec.ts` 的 `CHROME_ROWS`），页面侧零 JS|WKWebView 不认该属性且 `mouseDownCanMoveWindow` 恒 `false`（本机实测）⇒ 壳以 WKUserScript（documentStart、仅主 frame）注入 mousedown 监听：命中标记行（`data-window-drag`）且未命中控件时 `preventDefault` + 经 `dshChamberWindowDrag` 通道交回，用当前鼠标位置合成 `leftMouseDown` 调 `performDrag`（`macos/Sources/DSHChamber/ShellWindowDrag.swift`）|同一批页面标记、同一「控件不拖」选择器（`regions.ts#INTERACTIVE_SELECTOR` 逐项锁步）；命中口径 = DOM 真实命中 vs 几何盒序（可见且被按下的那层决定结果，取舍见 design 25 §5.5）。accepted；退役判据 = WebKit 放出 app-region 等价 API（或 Electron 腿 `hiddenInset` 落地后只余几何盒序差异时复核本行与 S-51）|
...
- S-24 菜单入口（Cmd+? → 项目页外部打开预算；File/Force Reload/Substitutions响应链）与「检查更新…」并存观察；打包态菜单无DevTools项（检查器改由设置页开关开启，T-10）。
