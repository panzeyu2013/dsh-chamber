...
| 4 | [branch-plan-v0.1.3-alpha1.md](branch-plan-v0.1.3-alpha1.md) | v0.1.3-alpha1 分支执行计划 T1–T4：追踪 alpha.2 / 命名统一 / fork 重锚 / open-in 统一 / 上游触点清单（方案已定稿，用户批准） | 执行中：**Batch 0（T1）✅ + Batch 0.5（T4）✅ + Batch 1（T2）✅ + Batch 2（fork 重锚）✅ 已完成 + Batch 3（T3）✅ 已完成（Phase 0 + Phase 2）**（记录见 STATUS）；实机验证项待实机（绿门见文内 §8） |
