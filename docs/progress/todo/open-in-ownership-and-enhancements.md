...
5. **远程来源**：ssh 目标只有 VS Code（新窗口/复用两态都与既有深链语义一致），
   ...
   该条目整体不渲染；
6. **插件管理页**：本地目标列出该行（4 行）且显示已注入；ssh/gateway/http 目标**不列**该行
   （行集 = 该目标适用行，3 行；绝不是"未注入"，也不再有已退役的「本地形态专用」badge），
   且远端 `~/.dsh` 下**没有**该包的目录与 loader 行；远端三行都注入齐全时 ssh 目标
   **不显示**「注入」按钮（合成的 `localOnly` 探测行不得参与该门），缺行时点「注入」
   **不再**报 `open-in 的 dist/index.js 缺失`（预检/远端日志/桌面侧 gateway 上传源清单一律读 portable
   列表；网关自有派生白名单按设计仍含该行，见 design 20 §9），
   且远端已注入未生效时出现的是「重启生效」而不是「注入」；
