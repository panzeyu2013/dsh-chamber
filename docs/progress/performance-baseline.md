...
| 实机 dsh | 内建 vendor runtime `@deepseek-ai/dsh@0.1.5-alpha.2`（本地实例自动就绪；host 插件已播种）。**本次实测的载体就是该版本**——源码线/运行时线此后已整体重锚到 `0.1.5-rc.2`（见 CHANGELOG 发布节），本文 §2 起的数字仍属 alpha.2 代，未在新锚上复测 |
...
落点与语义偏差登记见 STATUS.md 与 design 05 §4 注记；这里只记方法、定案与
...
| 空闲 15s 主线程停顿 | 无 >100ms 长任务 | `idle.over100msCount`；**2026-12 注**：首启基线收割窗口（每源一次后台 boot，串行于全局 boot 链）**不计入"空闲"**——测量须从收割全部完成后开始，否则该行必然失败；收割窗口自身成本（boot 次数/时长、峰值节点数）作为独立测量面登记，尚未纳入本表 |
| 合成输入帧间隔 | 无 >500ms（官方侧残余另计） | `input.clicks[].worstFrameMs` |
| 预热并发 | 1 壳、仅前台 | `dom.perInstanceNodes[].pending` 计数（预热中壳）；前台性由代码门控 + 空闲长任务间接佐证；**2026-12**：基线收割与预热**共享**该唯一槽位（收割期间不启动温壳） |
