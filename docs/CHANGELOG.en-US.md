...
- **Open-in unification · Phase 2 (Batch 3 core, redline revision)** — the single header entry now consumes the per-source view-model (Phase 0) and absorbs the official client for local sources:
  - **Local sources (official channel)**: the instance's own official host catalog (`dsh-host-open-in-app`, shipped in the default a2 web bundle) serves the full local application picker through the per-instance proxy `<basePath>/open-in-app/{apps,icon/<id>,open}` — the catalog protocol (`shared/open-in-app-protocol.ts`, byte-lockstep test against the vendor `shared.ts`), real bundle icons (404 falls back to a neutral square), the official `app.*` label table and button copy (merged into the single chamber locale NS), persisted choice (official key `dsh.open-in-app.choice`, in-memory fallback when storage is unavailable) and the busy/error dress (250 ms delayed busy, 2 s error decay) are all absorbed. The desktop main process only adds the VS Code override (the official catalog wins a shared id).
