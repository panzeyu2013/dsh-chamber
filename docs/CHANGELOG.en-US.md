...
- **Fixed four same-origin absolute URLs under the N-ctx shell (D3 ruling: build-time vendor patch set)** — the official client assumes it is served from the dsh origin, but under chamber's single-page multi-instance shell the page origin is the control plane, which only proxies `/api/i/<id>/*`. A round-3 repo-wide scan found **four** sites (round 2 had found one): (1) `ui-chat`'s `/api/file` (Markdown-local images — broken images); (2) `client-file-upload`'s `/api/session/uploadFileBinary` (**composer attachments 404**); (3)(4) `ui-deliverables`' `/api/present.host|open` (delivery-card open/reveal 404). Ruling (upstream-first, minimal intrusion): do **not** fork the whole `ui-chat` client (82 files / ~11.3k lines) for a few URLs — register a **declarative build-time patch set** in `packages/renderer/scripts/vendor-patches.mjs`, applied by the renderer's `deepseekSource().transform` against exact upstream anchors (4 patches / 5 files / 18 anchors) with zero writes to the vendor tree; a drifted anchor fails the build. `ui-chat` reads the new root standard prop `chamberFileApiBase` (provided by the chamber layout fork via `ctx.slots.provideRoot({ props })`, value = this entry's `ctx.chamberBasePath`); `client-file-upload` reads the same fact from its own service ctx; the `ui-deliverables` controller receives it at construction; when the base path is absent (official-layout deployment) the patched code falls back to upstream behaviour. So the patch can reach the upload client, `client-file-upload` becomes **composite-covered** (covered/factory 54/26 — an extra-row bundle is served by the instance and never passes our build), which also removes that extra-row dependency (the probe set shrinks to `sidebarRight`). Freshness gates: touchpoint **C9** (every anchor must match exactly once; hard failure) plus `scripts/vendor-patches.test.mjs` (anchors, patched behaviour, id forms) and `verify-vendor-patch-applied.mjs` as the last `build:renderer` step (the emitted bundle must contain the patched shape, so a silently no-op transform cannot ship). **Known boundary**: local and gateway sources work (the control plane injects their cookies); ssh/http dsh targets still get no cookie injection (instance-side 401) — an existing auth-surface item.
