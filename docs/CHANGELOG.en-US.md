...
- **Upstream touchpoint registry + freshness gate (T4) and design-token compliance gate (S1–S7)** — new `docs/checklists/upstream-touchpoints.md` (per-file purity registration [pure]/[patch-add]/[patch-mod]/[patch-comment]/[own-divergent]/[own]/[dropped], the dropped table, the deep-import/roster section, the contract mirror table, the artifact registry, and the 8-step maintenance loop per tag) plus `scripts/dev/verify-upstream-touchpoints.mjs` (C1 pure byte-equality / C2 cross-tag replay report / C3 completeness / C4 roster sentinel / C5 anchor scan / C6 excluded-directory existence / C7 seed-domain lockstep / C8 artifact staleness / C9–C10 anchors and live literals), the two maintenance faces mirroring each other and both CI legs running right after the vendor bootstrap; also `verify:styles` (the upstream design-token compliance gate: hairlines / overlay shadows / namespaces, S1–S7) and a touchpoint self-check section in `PULL_REQUEST_TEMPLATE`.
...
- **dsh re-anchored to 0.1.5-rc.1 on both lines (source + runtime, same generation)** — the build-time vendor source (submodule pin) and the bundled runtime both advance to `dsh-v0.1.5-rc.1` (183f08e9c6dd; npm `@deepseek-ai/dsh@0.1.5-rc.1` is published, so no split commit is needed — preflight reports "runtime published on npm: true"). This is a **version-line upgrade**: across 17 upstream commits / 373 files (+1209/−722) the three forks' upstream counterparts (`client/connection`, `client/web`, `api/gateway`) changed **only their `package.json` version line** (`src/` untouched; preflight measured pure 0 / needs replay 3 = the three version lines / dropped 0), so the fork replay is three version numbers — no non-version field to mirror, and no `tsconfig` `files` addition. The upstream workspace member set is unchanged (vendor links stay **284**), **no client row was added and no `dsh.client` metadata changed** (the official client-row set is **57** entries, byte-identical with identical metadata on both sides) ⇒ covered/factory stays **57/26** and the typert remote contract stays **15**; all **7 vendor patches / 21 anchors still match exactly once, zero drift**; the chamber deep-import seams (`ui-layout`/`ui-renderer`) moved only in `package.json`. **The first-screen coupling probe is unchanged**: `ui-sidebar-right`'s `provide('sidebarRight')` and `ui-chat`'s inject are byte-identical, so `REQUIRED_EXTRA_ROW_SERVICES` stays `['sidebarRight']`.
  - **All 23 substantive upstream source files land on surfaces chamber does not own**: the `ui-sidebar-{files,right,documentpreview}` rows' guide/preview polish (extra rows, served by the instance's own bundle; including the in-package `definition.ts`→`.tsx` rename and the guide's optional `description?` addition), `ui-chat`'s StatsPills no longer rendering the zero-valued cache-write row, `ui-primitives`' `CodeBlock` gaining an optional `contentRef` and wrapping its body in `[data-code-block-content]`, `ui-dockkit`'s two `z-index` additions, and `cordis-client-runner`'s slot-catalog doc pointer; the `sidebar.right.tab.document` slot props gain a **required** `scrollportRef` (chamber implements no renderer for that slot — no action).
  - **Lockfile**: the only non-version manifest change in the whole upgrade is `dsh-llm-deepseek` gaining `@deepseek-ai/dsh-attachment-local` (a host-half dependency chamber's build never consumes); pnpm did **not** prune the vendor importer block during `update-vendor`'s step-6 frozen verification, so that record was updated in place and both the frozen install and the before/after hash-stability assertion passed (the conditional risk is now written into the upgrade checklist §4). The same regeneration also moved two unrelated transitive edges, `@types/node` 26.2.0→22.20.1 (peer links of `buffer-image-size`/`happy-dom`; both versions were already in the lockfile, the root still pins `^26.2.0`, and this is type-side only).
  - **Six runtime anchors + 3 fork markers**: the `bundle-dsh` fallback constant, the `vendor/dsh` lockfile (`bundle:dsh --force --refresh-lockfile`; after regeneration the package population is unchanged — the same package names in `packages:`/`snapshots:` — and beyond the dsh version strings only each package's new tarball integrity, pnpm's peer-resolution suffixes and a handful of keys re-serialized by pnpm (including one `tarball:` field) differ, which a republish implies), the release.yml env, install-gateway.sh, the gateway `dshAnchorVersion` and the release-preflight `FORK_VERSION` all move to 0.1.5-rc.1; the `bin.js --version` smoke prints 0.1.5-rc.1. Native builds still go through the existing `allowBuilds` single-source list (`packages/dsh-runtime/src/allow-builds.mjs:14-21`, six allowed entries: node-pty / koffi / **fs-ext** / protobufjs / @google/genai / @deepseek-ai/dsh-subprocess-local — `fs-ext` is **retained as allowed**, not denied: its consumer is the 0.1.3-alpha.2 rollback target and the rc.1 runtime closure no longer contains it; the same-named `false` in the root `pnpm-workspace.yaml` means "this development tree does not build it", which is not the runtime tree's allowlist) — **no new native dependency**.
  - **Operator-visible upstream change reaching managed instances through the runtime line**: the base bundle's default model moves `deepseek-v4-flash` → **`deepseek-flash`** (V41 Flash; the catalog grows 3→4 entries, keeping V4 Flash / V4 Pro / V4 Flash Vision Exp), and that entry declares `systemPromptUpdate: 'in-history'` (the V4 ids declare nothing and keep their behaviour); upstream's own caveat applies — requests can fail with `INVALID_REQUEST` until the gateway enables that id, and the operator can switch back to a V4 model in settings.
  - **Version-skew registration**: the instance's document-preview code view now depends behaviourally on a same-generation `ui-primitives` (`CodeBlock`'s `contentRef` is its only scrollport/line-navigation anchor), so an older-generation composite serving a newer instance loses that view's own scrollport and line navigation — registered under the platform-word deviation C3 in STATUS.
  - **Validation**: `verify-upstream-touchpoints` C1/C3–C10 all green (C5 on the new anchor, C8 rebuilding and comparing 5 artifact groups, C9 with all 21 anchors unique, C10 with the six anchors + 3 forks consistent and no unregistered live version literal); the C2 report confirms the fork face between the tags is exactly three version lines; the root-script regression passes as 40 green steps (19 `test:*` — the 17 on the checklist §6 list plus `upgrade-tools` and `release-workflow` — 16 typecheck entries, `build:renderer`, `verify:i18n`, `smoke`, plus the touchpoint gate and the workflow policy script), and the full `build:desktop` chain passes with a byte-identical runtime lockfile when `bundle:dsh` re-runs (reproducible).
  - **Test-portability fix in the same batch (a pre-existing defect this regression exposed, unrelated to the pin)**: `test:host-archive-cleanup`'s uppercase near-miss case cannot hold on a **case-insensitive filesystem** (macOS APFS default / Windows NTFS, where `session.v3.JSONL` folds onto `session.v3.jsonl` as one directory entry, so the directory only ever holds the canonical name) and used to fail there unconditionally; it now probes the filesystem at runtime, asserts the folded reality on such platforms (single entry, purge reclaims it) and keeps the original refusal assertion on case-sensitive ones (the Linux CI main leg).
  - **Unverified ([UNVERIFIABLE])**: the alpha.2 batch's on-device gates (multi-source sleep/wake and hidden restore, gateway-shape regression, packaged-app visual inspection, …) remain outstanding under STATUS; this upgrade added no new on-device evidence.

- **Fixed five same-origin absolute URLs under the N-ctx shell (D3 ruling: build-time vendor patch set)** — the official client assumes it is served from the dsh origin, but under chamber's single-page multi-instance shell the page origin is the control plane, which only proxies `/api/i/<id>/*`. Round-3/4 repo-wide scans found **five** sites (round 2 had found one): (1) `ui-chat`'s `/api/file` (Markdown-local images — broken images); (2) `client-file-upload`'s `/api/session/uploadFileBinary` (**composer attachments 404**); (3)(4) `ui-deliverables`' `/api/present.host|open` (delivery-card open/reveal 404); (5) `session-log-export`'s `/api/session.export` (**the /export download 404s**). Ruling (upstream-first, minimal intrusion): do **not** fork the whole `ui-chat` client (82 files / ~11.3k lines) for a few URLs — register a **declarative build-time patch set** in `packages/renderer/scripts/vendor-patches.mjs`, applied by the renderer's `deepseekSource().transform` against exact upstream anchors (7 patches / 7 files / 21 anchors) with zero writes to the vendor tree; a drifted anchor fails the build. `ui-chat` reads the new root standard prop `chamberFileApiBase` (provided by the chamber layout fork via `ctx.slots.provideRoot({ props })`, value = this entry's `ctx.chamberBasePath`); `client-file-upload` reads the same fact from its own service ctx; the `ui-deliverables` controller receives it at construction; when the base path is absent (official-layout deployment) the patched code falls back to upstream behaviour. So the patches can reach the upload and export clients, `client-file-upload` and `session-log-export` (the latter in the deferred cluster) become **composite-covered** (covered/factory **57/26** — an extra-row bundle is served by the instance and never passes our build), which also removes the `fileUpload` extra-row dependency (the probe set shrinks to `sidebarRight`); `dsh-client-ui-mobile` and the official `ui-directory-picker-native` are registered as **page-own skips** (the former is the gateway single-shell surface, the latter is mutually exclusive with chamber's pinned `browse` surface). Freshness gates: touchpoint **C9** (every anchor must match exactly once; hard failure) plus `scripts/vendor-patches.test.mjs` (anchors, patched behaviour, id forms) and `verify-vendor-patch-applied.mjs` as the last `build:renderer` step (the emitted bundle must contain the patched shape, so a silently no-op transform cannot ship). **Known boundary**: local and gateway sources work (the control plane injects their cookies); ssh/http dsh targets still get no cookie injection (instance-side 401) — an existing auth-surface item.
- **dsh re-anchored to 0.1.5-alpha.2 on both lines (source + runtime, same generation)** — the build-time vendor source (submodule pin) and the bundled runtime both advance to `dsh-v0.1.5-alpha.2` (b2e3b2a01258, upstream master HEAD; vendor links 271→**284**, upstream +17/−4 packages: `apps/desktop`, `apps/desktop-host`, the `native/system` family, `api/workspace-files`, `client/resources`, `ui-dockkit`, `ui-sidebar-{right,files,documentpreview}`, `session-format-v2-to-v3`, `fs/tool-present`, `util/chunked-list`; the landlock family is gone). Upstream rewrote the client shell's slot model twice in this range (`details`→`rightbar` with scope session→root; the centre column `conversation`→**keyed `main`**; a new `sidebar.panellist` global-panel axis, the `usePanelInfo`/`useResource` global seats, and `ctx.layout.selectPanel`/`beginNavigation`), so every chamber-built surface replays:
  - **layout fork (P0)**: `children` becomes `sidebar` / `main` (keyed, root) / `rightbar` (single, root) / `shell.overlay` — without the keyed `main` slot the official ui-conversation's `slots.inject('main')` never resolves and the whole conversation surface stays unregistered. The store nests as `{panelInfo:{activePanelId}, layoutInfo:{…}}` and gains `selectPanel`/`retainMainPanels`; `apply` mints the instance eagerly and constructs `LayoutController(actions, hasMainPanel)` (upstream deleted `attachPanels`) plus `provideRoot({hooks:{panelInfo}})`. Both chamber value-adds stay: shared sidebarWidth persistence (an explicit `trackLayoutInstance` registration replaces the now-dead `handle.create` patch) and the single document theme projection; `layoutFacts` gains `getCollapsed()` so consumers stop restating the vendor breakpoint constant.
  - **sidebar fork**: the `sidebar.brand.mark` / `brand.name` / `panellist` declarations and rendering are restored (the brand row falls back to the chamber wordmark; panel rows render the alpha.2 `{id,order,label}` metadata and forward clicks through a direct `ctx.layout.selectPanel` (both the chamber fork and the alpha.2 official ui-layout declare the method, so a missing one is a misconfiguration that must fail loud instead of probing and degrading)), with a new `panel-source.ts` projection (locale thunks resolved at read time, notify only on change) and five unit tests.
  - **mobile plugin**: column anchors move to `ROLE_SLOT_KEYS` (conversation→`main`, details→`rightbar`) and `layoutFacts.getCollapsed()` replaces the local derivation; two mechanisms are retired because upstream solved or duplicates them — the session-log capsule label stamping (upstream now renders a 28×28 icon button plus menu) and the self-drawn right-column overlay (upstream auto-fullscreens below 768px); layering moves to 74/75/76 above dockkit and the official fullscreen panel, the dead `[class$=…]` selectors are fixed, and dockkit split chrome is hidden on touch.
  - **settings bridge**: the child-ctx declaration ledger gains the brand/panellist keys and the kit gains an empty `usePanelInfo` seat.
  - **three fork copies**: connection's pure files are copied verbatim (`src/index.ts` is fork-pure — the host-half webServer optional injection needs no hand merge) plus the version line; client-web gets the version line and the dockkit deviation note (**not seeded**); api-gateway is version-only.
  - **renderer**: `ui-dockkit` is answered by a covered factory (seeding it would pull the docking kit into the main-graph eval, the same C3 reasoning as ui-primitives), covered/factory **57/26** (round 3 adds `client-file-upload`; round 4 adds `session-log-export` + two page-own skips); the C4 typert remote-assembly contract moves **13→15** (+`command-feedback`, +`workspace-files`) with the lockstep test updated; and a new bounded `assertRequiredExtraRowServices` probe (`required-extra-rows.ts` + `chamber-entry.ts`) reports within 5 s — naming the instance and the services — when the extra rows fail to provide `sidebarRight`, which the first-screen `ui-chat` cordis inject set requires (a diagnostic, not a boot gate: the gateway/mobile shapes may legitimately run without those rows, so the probe only removes the silent-disappearance mode).
  - **design 24, four real-machine defects fixed**: (1) `sessionPersistence.list()` has returned snapshot records since 0.1.3-alpha.1 while the binding treated them as headers, so preview/purge were broken on real hosts (the test fixtures masked it) — the binding now reads `snapshot.header`; (2) the existence probe `inspect(id)` is gone — it uses the official `stat(id)` (every project directory, every generation; `undefined` means no content, any throw still fails closed); (3) the purge removed only the current generation — it now removes **every** generation file plus `session.lock`, and refuses the whole operation on an unrecognized entry, symlink or subdirectory instead of half-deleting; (4) the consumerless `isPersistenceNotFoundError` classifier is deleted. Design 24 §2 and AGENTS now name `stat(id)`.
  - **six runtime anchors**: the `bundle-dsh` fallback constant, the `vendor/dsh` lockfile (`bundle:dsh --force --refresh-lockfile`), the release.yml env, install-gateway.sh, the gateway `dshAnchorVersion` and the release-preflight `FORK_VERSION` all move to 0.1.5-alpha.2; `bin.js --version` smoke-prints 0.1.5-alpha.2.
  - **Not verified ([UNVERIFIABLE])**: real-machine multi-source sleep/wake and hidden-recovery, the gateway deployment shape, right-sidebar loading order against a real profile, the `provideRoot` timing for `useResource`/`usePanelInfo`, and session v3 migration against real storage.
...
- **dsh source line upgraded to 0.1.3-alpha.2 with the runtime line closed (dual lines same-generation)** — the build-time vendor source (submodule pin) advances to dsh-v0.1.3-alpha.2 (82a5fd61a7, vendor links 267→271, three new packages: `client/ui-open-in-app`, `host/open-in-app`, `util/package-manifest`); the fork copies replay accordingly — connection adopts the upstream recovery-config extraction (reconnect/readiness timing defaults move into a shared schema: a 3 s slow-handshake warning plus a 15 s hard deadline that aborts the generation, continuous retries at the cap replace the terminal `disconnected` tier; the chamber loopEpoch generation guards and the `CONNECTION_BACKOFF_MAX_MS` export survived at that point (**retired by the Batch 2 re-anchor**, see the next entry), `basePath` is now a chamber apply-config member), api-gateway and web are version-only; the three copy markers move to 0.1.3-alpha.2. **Runtime line closure**: `@deepseek-ai/dsh@0.1.3-alpha.2` is published on npm, so the four anchors — the bundle-dsh fallback constant, the desktop vendor lockfile (`bundle:dsh --force --refresh-lockfile`), the release.yml env, install-gateway.sh and the gateway `dshAnchorVersion` — move from rc.1 to alpha.2, and `bin.js --version` smoke-prints 0.1.3-alpha.2 (dual-line gate closed). chamber-covered gains the `@deepseek-ai/dsh-client-ui-open-in-app` row (the official open-in client row stays covered once it appears in the host graph; the official button's availability probe fails inside the chamber shell and it self-hides — the T3 double guard).
- **Chamber package naming unified (Batch 1 / T2, one atomic batch)** — naming is now "directory == package name minus the scope": the six client plugin names `@dsh-chamber/dsh-client-ui-*` → `@dsh-chamber/dsh-chamber-client-ui-*` (directories unchanged), and the three host seed packages' directories and names → `dsh-chamber-seed-<loader-id>` (`@dsh-chamber/dsh-chamber-seed-client-graph` / `-git-worktree` / `-archive-cleanup`; loader ids, activation-probe domains and the release package count unchanged). `dsh-client-ui-mobile` (the client-kind exception), the three fork copies (shadow mechanism) and the infrastructure packages are untouched. In the same batch: the `.gitignore` committed-dist negations move with `git mv`, every reference is replaced once (scripts / renderer tables / the gateway sync list / the desktop seed / test fixtures / lockfile importers / docs), and both seed registries gain a fail-loud naming assertion (`kind === 'host'` ⇒ `@dsh-chamber/dsh-chamber-seed-<loader-id>`: the control-plane registry at start and at every per-spawn resolution, the gateway syncable list at module load). **Remote transitional exception**: a pre-rename `cordis.patch.yml` row is folded once (the `@dsh-chamber/dsh-host-*` name under the same loader id is rewritten in place; only the exact row bytes the seed writer produces — hand-written flow/inline variants still fail loud) — without it the remote seed would hard-fail on an id-bound conflict forever. The committed host-package `dist/index.js` artifacts are byte-identical (the package name is not embedded in the bundle), and the regenerated lockfile is frozen-stable with the vendor importer records intact.
- **Fork re-anchor on alpha.2 with patch minimization (Batch 2)** — the three fork copies were re-reviewed file by file against dsh-v0.1.3-alpha.2: what can be pure is restored byte-identically, what must be patched is reduced to the minimum:
  - **connection**: `src/client/connection.ts` is reduced to "upstream content + the erasableSyntaxOnly explicit-field rewrite + a header note" (`src/browser-auth.ts` returns to verbatim upstream) — the loopEpoch generation guard and the stop()+start() reconnect path are retired (upstream's native `reconnect()`/`setNetworkAvailable()` cover the same semantics without the second-pump-loop race), the `CONNECTION_BACKOFF_MAX_MS` export is gone, and the liveness-trigger debounce value becomes the internal `DEFAULT_MIN_RESTART_INTERVAL_MS` in `liveness-triggers.ts` (10 s == the recovery schema default `backoffMaxMs`) with an offline gate (upstream already suspends retries while offline, so triggers are ignored); `basePath` converges to `apply(ctx)` reading `ctx.chamberBasePath` (symmetric with api-gateway; chamber-entry no longer passes plugin config) and `createWebConnectionRpc` drops its compatibility overload for the chamber options object only. A new `client-apply` behavior gate pins ctx → carrier/handle, and the test loader gains fixture/recovery-config stubs.
  - **client-web**: `src/base.css` returns to verbatim upstream and the five ui-theme token sheets move to the renderer entry CSS (`packages/renderer/src/styles.css`) — the head CSS order is unchanged, tokens still precede plugin CSS; the seed/platform/index rebase prose is condensed to "invariant + pointer", and the five irreplaceable seams (module-table host / extraRows / configureContext / boot tolerance / async dispose) are untouched.
  - **api-gateway**: `apply(ctx)` reads `ctx.chamberBasePath` directly (the `ClientRemoteOptions` parameter is gone); the stream-carrier patch is unchanged.
  - The upstream-touchpoint registry follows (pure: connection 15→16, client-web 4→5, api-gateway 6; C1 byte-identity now covers the newly pure files), and designs 05/14/20 are amended accordingly.
- **Open-in unification · Phase 0 pure gates (Batch 3)** — new per-source view-model `packages/dsh-chamber-client-ui-open-in/src/shared/open-in-view-model.ts`: it folds the two app pools — the official host catalog (`official`) and the desktop main-process provider (`main`) — through the source matrix (local = both pools in full; `dsh-*`/`gateway-*` + ssh = remote-capable `main` entries only; http/malformed = nothing) into one decision surface, reporting every rejected candidate with an explicit suppression reason (`unknown-source` / `transport-not-ssh` / `source-not-local` / `app-unavailable` / `app-not-remote-capable` / `duplicate-app-id`), plus channel-priority dedup and the default selection. The existing `usableOpenInApps`/`usableAppsForSource` helpers became thin adapters over that view-model (behavior unchanged, one decision surface). Unit tests pin the matrix, dedup and suppression reasons (8 cases). Phase 2 is the next entry.
- **Connection-recovery hardening (2026-09, Batch 2 follow-up)** — responding to the dedicated investigation into "upstream connection management assumes a local single-instance host, while chamber faces tunnels, slow links and OS wake":
  ...
  - **Responsibility boundary registered**: the connection layer is the ONLY re-opener of the push channel (`/api/remote.mux`) — every other chamber layer only detects, tears down or re-registers transports (the mux client has no backoff of its own); same-instance `stop()+start()` is forbidden (reintroducing it would require restoring a generation guard).
  - **Upstream trend (read-only, no pin bump)**: the latest tag is `dsh-v0.1.5-alpha.1`, but the three forks' client recovery model is UNCHANGED (only fixture, the host-half `webServer` optional-inject refactor, and a new platform word `dsh-client-ui-dockkit`) — so this hardening has no upstream equivalent to adopt and uses only upstream-supported seams; the upgrade notes are recorded in the touchpoint registry §2.4.
- **Open-in unification · Phase 2 (Batch 3 core, redline revision)** — the single header entry now consumes the per-source view-model (Phase 0) and absorbs the official client for local sources:
  ...
  - **Not verified on a real machine ([UNVERIFIABLE])**: the official host row reaching the managed instance through the default a2 profile, fence behavior without a remote cookie, remote cwd population, icon caching/CSP.
- **Upgrade tooling + connection-recovery follow-up (W1/W2/W3, 2026-09; source/runtime pin still 0.1.3-alpha.2)** — three pieces of version-independent follow-up work for the 0.1.5 upgrade, all validated at the current pin:
  - **Pre-bump pin preflight (new `scripts/dev/preflight-vendor-pin.mjs`, read-only)**: diffs the target tag against the current pin and reports, in one pass, the three fork copies classified as pure / needs-manual-replay / dropped, the deep-imported vendor seam files, upstream package additions and removals, new client rows, and whether the runtime is published to npm; supports `--offline`/`--json`/`--fail-on-replay` (advisory tool: it never writes the worktree and never moves the submodule HEAD). Measured against `dsh-v0.1.5-alpha.1`: 2552 changed files → pure 5 / replay 6 / dropped 6, **seam 16 (all under `packages/client/ui-layout/*`)**, packages +15 / −4 (landlock), 5 new client rows. This turns the trap hit during 0.1.5 (bumping the pin first and only then discovering the three-column rewrite → `typecheck:layout` red immediately) into step 0 of the process: read the list before touching the pin.
