...
> **状态：现行（archiveCleanup 宿主域 + 归档管理器 + purge 后会话列表收敛 + 常驻保留，2026-13）**——本文是
...
archiveCleanup/purge({})          → domain { ok, value: {
                                       ...
                                       residentRetainedRoots?: string[] // 2026-13 常驻保留修正：内容已删除（或本就
...
// purge 亦可带可选保护集（2026-09 保护修正）：protectSessionIds 命名的 id 及其
...
- 入参：`preview` 与 `probe` **零参**（envelope `payload: { args: {} }`——
  ...
  与**可选** `protectSessionIds` JSON 参数（保护集，2026-09 保护修正新增）。
  客户端照 §5 恒发 `{args:{sessionIds:[…], force:true, protectSessionIds:[…]}}`。
  可选参仅限唯一标识符（无解构/默认值/rest——gateway SRC 签名约束，方法签名
  不得含默认值）。
- **可选子集过滤语义**：`undefined` = 全量（旧形状，向后兼容）；数组 = 只把
  ...
  同一收尾批量写移除）。
- **可选 `protectSessionIds` 语义（2026-09 保护修正）**：与 `sessionIds` 同规校验
  （非空串数组、≤ `MAX_PURGE_SESSIONS`，畸形 → `invalid-request`，先于任何权威
  读取）；未知/陈旧/未归档 id 是无操作。**方向性**：只会**缩小**本次删除集
  （命中子树闭包即整棵跳过），永不放宽任何安全守卫。命中计数进
  `skippedProtected`，分类**优先于** running/loaded/force：既运行又受保护的树只
  记 protected（客户端由此得到可操作事实——"这棵树里就是你在看的会话"）。
  同一集合还排除出孤儿清扫与收尾集合移除（纵深防御：**只读查看中的会话既不在
  running 也不在 loaded 集合里**——客户端打开会话是只读观察
  （`sessionQuery.observeSession`），不会 attach 进宿主 sessions store，所以
  宿主自带的 loaded 守卫**兜不住**它，protect 是它唯一的保护）。
- **可选 `force` 语义**：缺省 = 既有 fail-closed 行为，逐字节不变；
  ...
  （已加载未运行）/`forcedLoaded`（本次真的删掉了内容的强制删除树根）拆分，
  `PreviewResult` 同步带 `skippedLoaded`。
- **常驻保留（2026-13 修正，用户报告「删完反而被重新加载回 workspace」）**：
  会话内容删除后**只有归档集合成员关系在遮住它**，而官方会话列表是 live 优先的
  （`sessionQuery.listSessions()` = 持久化重扫 ∪ `ctx.sessions.list()`，见 §10⑤）
  ——本进程仍持有该会话时，行会继续被供出；此时清成员关系 = 把用户刚删掉的会话
  以普通行重新推回侧栏（F2 权威探针也会列出它，幽灵行抑制按 §12 主动放行）。
  因此：**删除瞬间仍常驻本进程（liveness `loaded`，或根自己在删除瞬间被 binding
  报告 resident）的树，其归档成员关系一律保留**，整棵树（含被覆盖的 archived
  ...
  `residentRetainedRoots`
  （id 标识符，非内容投影），`forcedLoaded` 收紧为「本次真的删掉内容」的根数。
...
3. **运行保护**：任一节点是 live agent（`ctx.agents`）→ 整棵子树跳过并计数
   ...
   **运行是唯一硬底线**：`force` 也**不**放过 running（活跃 writer 会在删除后
   经 `open(path,"a")` 重建无头残档）——客户端因此在删除前先 `session/cancel`
   停回合（§5），归档动作本身也停一次（见下条）。**live facts 读取面 fail-loud
   （2026-13 review）**：agents 条目/状态、以及 live store（`ctx.sessions.list()`）
   的**数组形状与条目 id** 任一漂移都拒绝整次读取（`registry-unreadable`，一个
   字节都不删），与 `listSessionStates` 的「did not answer an array」同政策——
   静默丢弃一个条目会同时把 loaded 守卫与常驻保留判据推向**不安全方向**
   （fixture 见 binding 测试）；
3b. **保护（2026-09 保护修正，先于 3 分类）**：候选根的**子树闭包**与运行期
   `protectSessionIds` 相交 → **整棵跳过**并计 `skippedProtected`（优先于
   running/loaded/force，见 §3 语义）。`core.resolvePlan` 先算闭包、再判保护、
   最后判 liveness（因此"既在跑又受保护"只记 protected）；删除循环开始前**再查
   一次**（plan 出错时也绝不部分剪枝——前缀删除不可回滚）；`binding.
   deleteSessionContent` 拿到同一集合，命中即 `protected` 拒绝（最后一道不变量
   守卫）；清扫候选与收尾 `clearIds` 同样排除 protect。**该集合由客户端提供**
   （它知道自己在显示哪个会话），宿主用它做**全量语料**上的闭包判定——这正是
   客户端自证闭包（旧 pre-flight 门）被整体删除的原因；
...
8. **逐会话隔离与复检**：单会话失败记入 `errors` 不 throw、不阻断其它树
   ...
   守卫是唯一 mid-window 门**并承载中止语义；同一次读取同时产出**删除瞬间的
   常驻位**（`SessionContentDeletion.resident`，2026-13），供步骤 9 的保留判据
   使用；幂等（内容已不在 = `missing`
...
9. **集合成员移除**：官方**无公开裁剪原语**（registry 仅 `archiveSession`
   ...
   **一次纯 `workspaceRegistry.setState({initialized, workspaceIds,
   ...
   `typeof setState === 'function'` 运行时守卫并 pin 版本。**收尾批量写
   （2026-13 常驻保留修正后）**：完成树根、完成树覆盖的已归档后代、孤儿在同一批
   官方 setState 写中清除（clearIds 去重；core.ts `purge()` + binding
   `removeArchivedSessionIds`，测试固化）——**唯一的例外是常驻保留树**（§3
   「常驻保留」）：该树根与它覆盖的 archived 后代**不进 clearIds**，成员关系
   原样保留，id 列进 `residentRetainedRoots`。理由：归档集合在 vendor 的定义即
   **隐藏集**（`dsh-workspace` `archivedSessionIds`: "sessions hidden from every
   grouping surface"），而官方会话列表 live 优先——本进程仍持有该会话时，清掉
   成员关系就是把刚删掉的会话以**普通行**推回侧栏（2026-13 用户报告），所以
   「内容已删」不再构成「必须摘标记」的充分条件，「会话是否仍常驻本进程」才是
   判据。该例外不改变红线段「no unarchive」的读法：本域只在其内容删除完成后
   清除**该已删内容自身**的集合成员，不提供任何恢复/浏览/反向操作；A-区
   （todo 12 方案 A，已归档浏览）若实现，其数据面即当前 archived 集合（仅含
   未清理、运行中留存与常驻保留项）。上游 unarchive/delete wire 落地后本域退役
   并按官方语义收敛（§11）。
...
- **常驻保留字段的解码与文案（2026-13）**：`purgeArchivedSessions` 额外解
  `residentRetainedRoots`（`optionalIdList`：缺键/非数组 → 缺席；条目只收非空
  字符串、**去重**（列表同时喂"计数"与"标注集合"），**绝不伪造 id**——它只驱动
  行标注，计数仍是权威结果）；note 装配
  （`archivePurgeNote`）在该字段非空时输出 `archive.purge.note.residentRetained`
  （「其中 {count} 项内容已删除，但会话仍在本实例内存中——继续保持隐藏，重启该
  实例后彻底消失」），**该文案取代**旧的 `archive.purge.note.forcedLoaded`
  （「强制删除」读起来像"列表里也没了"，正是这次误解的来源）；旧宿主不发该字段
  时（版本歪斜）回退旧键，不静默。**为什么由宿主回传 id、而不是客户端自己算**
  （2026-13 review 记录，防止后人"化简"掉这个字段）：客户端能拿到的近似式是
  「本次提交的根 ∩ run 后仍在归档集合」——但**中止/被保护/运行中跳过的树同样
  留在集合里**，按那条近似式会被错误地标成"内容已删除"。只有宿主知道哪棵树是
  **完成**的，所以 id 列表必须来自宿主；
...
- **「先停止、再删除」编排（force 路径）**：管理器删除前用官方
  ...
  集合清空（有界），再 `purge({sessionIds, force:true, protectSessionIds})`。
  - **闭包 = 选中根 + 全部 subagent-origin 传递后代**：血统只沿
    ...
    （`fetchSessionRunningLineage`），对**冻结选中集合**的**闭包内每个成员**发
    cancel（不只 observed-running 的——维护相位对外报 idle，见下条），再等待闭包内
    不再有运行成员；`stillRunning` 报闭包内仍在运行者。闭包计算对畸形/自环父链有界
    （`seen` 守卫），父子边索引每趟只建一次（`sessionChildrenIndex`，O(V+E)），
    cancel 有界并发（4）且按输入序记账。
  - **保护代替拒绝（2026-09 保护修正）**：**运行期不再有任何"整体拒绝"腿**。
    客户端把**活** `runtime.current`（vendor `SessionListSnapshot.current`）作为
    `protectSessionIds` 交给宿主；宿主在**全量语料**上做闭包判定并整棵跳过
    （`skippedProtected`）。该 id **在请求时**从 bridge 的活投影解析
    （`chamberBridge.getServers()`，不是渲染快照——`current` 可以在上一次 publish
    与点击之间移动），缺失时回退到该次渲染的 prop。于是：
    - **已知 viewed**：其整棵树绝不触碰（连它的 subagent 兄弟都不剪）；
    - **未知 viewed**（未选中 / 壳被回收 / 瞬时列表缺口）：**照删**，只是没有
      protect——这是**保护降级**，不是能力丧失；
    - **为什么不做 sticky 记忆（2026-09 review 定案）**：vendor 的公开列表快照
      **无法区分**"掩码"与"清空"——两者都表现为 `list.current === undefined`
      （持久化 selection cell 在两种情况下都被清，`service.js` 的
      `persisted` 分支；请求态 selection 与 `watched` 都是私有的）。而
      `ui-workspace.clearArchivedCurrent()` 恰好在**当前会话被归档**的瞬间调
      `sessions.clear()`。因此"记住最近一次 current"会把**用户刚归档的那个会话**
      一直保护下去，直到切换会话或重启应用——正是本轮要消灭的死端（只是更窄）。
      取舍：保护**活事实**，把掩码窗口登记为残余（§13⑭）。归档后的短暂窗口内
      首次删除可能收到 `skippedProtected`（选中尚未清空，通常一次 store 更新即
      自愈），文案已写明"切换会话后重试，或稍后重试"。
  - **血统只服务"停回合范围"，不再服务拒绝**：vendor 列表会跳过无 cwd 的冷记录
    （`list.ts` 过滤 `record.header.cwd === undefined`），中间层祖先行可能缺失。
    客户端因此只用血统决定**取消范围**：`exclude`（viewed）里的 id 永不取消；
    `upwardChainComplete` 不成立时**整趟停回合都不执行**（无法证明哪些选中根
    含 viewed，取消一棵"宿主根本不会删"的树会白杀活回合），随后**照常 purge**。
  - **读失败 → 只跳过停止，不拒绝删除**：初始 `session/list` 读失败 →
    `unavailable: true`，不发任何 cancel，直接进入 purge；宿主 running 守卫是
    安全网（真在跑的树被跳过并如实计数）。
  - **停回合范围（2026-09）**：对选中闭包内**每个成员**发 `session/cancel`
    （不只 observed-running 的）：维护相位（compaction/schedule）对一切观察者
    报 `idle`（vendor agent loop 只有 running/maintenance 两相），却仍在追加写
    日志，cancel 是客户端唯一能中止它的手段。取消按**有界并发**（4）扇出、
    结果按**输入序**记账（大树的远程实例因此付 N/4 次往返而不是 N 次，文案
    计数仍确定）。**顺序保证**：扇出被完整 await，返回时**每个 cancel RPC 都已
    resolve**（维护相位的确认就是它的 resolve——维护相位对观察者报 `idle`，
    running 集合见证不到它），随后才发 purge。用户可见计数只算**观察到的
    running** 成员；**失败**则对 observed-running **与** listed 成员都上报
    （listed 者可能正处维护相位，其取消失败不可吞），无列表行（cold）成员的
    机会性 cancel 是文档化 no-op（`session/not-found`）。
  - **归档即终止（user motion「已归档的对话应该终止」）**：chamber 的归档动词在
    `workspace.archiveSession` 成功后**就地**执行同一趟停止
    （`stopArchivedSubtree` = 无排除的闭包停止，含全部 subagent 后代）。归档会
    清空"正在查看"的选中（vendor `clearArchivedCurrent`），卡在提问/权限的回合
    因此永远等不到回答——不终止就是永久 running 的僵尸，之后任何一次删除都会被
    running 守卫整树跳过。停止是 advisory（归档已生效，失败只告警、不回滚），
    删除侧还会再停一次兜底（别的客户端归档的会话也走这条）。
  - **停止未生效 / 子代理仍在跑** → 宿主照旧整树跳过（fail-closed），note 提示
    可稍后重试。
  - **版本歪斜：不做兼容回退（2026-09 决定）**：客户端**恒发**
    `{sessionIds, force:true, protectSessionIds}`，**没有**"去掉新字段重试一次"
    的旧宿主腿——去掉 protect 就等于在无法保护 viewed 的情况下强删，去掉 force
    就等于回到"已加载的删不掉"。宿主包随应用 seed 并随实例重启生效，歪斜窗口
    的处置口径是**重启该实例的 dsh**（`gateway/arguments-invalid` 原样上抛，
    文案由 §6 的错误行呈现）。`session/cancel` 是官方 wire，不受本域版本影响。
...
- **工作树侧判据（design 08 §5.2 同源）**：工作树删除**不停、不取消、不隐式归档、
  ...
  **fork 边终止链条，fork 永远不因其来源会话被归档而 INERT**（fork 是独立
  ...
  唯一「先停止运行中的回合、再清理已归档内容」的入口**（2026-09 起归档动作本身
  也停一次，见 §5"归档即终止"；工作树侧仍然没有任何停止编排），本域也不因工作树
  删除而新增任何执行权。
- **i18n**：`shared/archive-purge.ts` 只返回
  **字典键 + 参数**（`PurgeNoteLine { key, params }`，`archivePurgeNote` 返回
  `{ kind, lines }`），**不内联任何文案**；对话框
...
- **无 UI 预检门（2026-09 保护修正）**：删除控件只在 `busy`/武装确认/未选中时
  禁用——**未知当前会话不再是禁用理由**（它只是"保护降级"）。列表可见时若
  `server.runtime?.current` 缺失，对话框在列表顶部渲染一行可见说明
  （`archive.manager.unprotected`，role=status），而不是把控件置灰并只挂在
  `title` 上：用户至少能看到"这次不排除任何会话（运行中的仍不会被删除）"，
  并且**仍然可以删**。
...
- **运行结果内联呈现**（`role=status`/`role=alert`，zh 硬编码）：完成摘要
  ...
  `skippedLoaded`/`skippedProtected`（"已跳过 N 个归档树：其中包含你正在查看的
  会话"）与列表顶部 `archive.manager.unprotected`（无法点名 viewed 时的诚实
  降级说明）的如实呈现。**错误绝不静默**；
  **成功无系统级横幅**（与「归档」动作一致——动作对象不可见），成功/空态为
  中性 `cleanupNote` 样式。
- **常驻保留行的标注（2026-13）**：run 返回的 `residentRetainedRoots` 存进对话框
  本地状态（`residentPurged`，在对话框生命周期内**取并集**——后一次不报告不代表
  前一次删的内容回来了），命中行在标题后渲染 `archive.manager.residentPurged`
  （「内容已删除，待实例重启收敛」，`role` 中性、非控件、非错误）：这类行**必然
  继续列在这里**（宿主保留了成员关系，官方列表也还供着这一行），标注是"删掉了、
  等重启"，而不是让用户怀疑删除失败。标注随行集合被动 prune（实例重启后行消失
  ⇒ 标注一起消失）；标注不进任何持久层、不改选中/计数/删除语义。
  **a11y（2026-13 review 补）**：标签是独立 `span`，只在浏览模式被读到——聚焦
  复选框时读屏只会听到标题，因此命中的行把状态并入复选框可访问名
  （`archive.manager.rowAriaResidentPurged`，`rowAriaLabel()` 单点生成），
  读屏用户不会把"这行还在"误读成"没删掉"。**布局（同轮）**：列表行是 nowrap
  flex 线，标签必须**先让位**——`.archiveManagerRowTag` 带 `max-width: 45%` +
  ellipsis + `title` 全文，避免窄卡上把会话名挤没（§13⑰ 的窄卡残余因此不再被
  新元素放大）；
- **勾选清理的判据（2026-09 review）**：只有本次 run **真的移除了内容**
  （`purgeRemovedContent`：删除的会话/子代理或清扫的孤儿成员 > 0）才清空勾选。
  纯跳过/纯保护的 run 一行未动，清空勾选会把"切换会话后重试"变成"重新勾选再
  重试"；行集合真正收敛时那条被动 prune（`pruneSet`）照旧收敛勾选。
...
- **locale 键**（en/zh；对称由 `locales.ts` 的 `SidebarKey = keyof typeof zh`
  ...
  计数键、`degraded`、`listUnavailable`、`loading`、常驻保留标注
  `residentPurged`）、复用既有
...
- **版本歪斜 = 重启实例（无兼容腿，2026-09 决定）**：`sessionIds`/`force`/
  `protectSessionIds` 任一不被旧宿主描述符承认，泛型网关就以
  `gateway/arguments-invalid` 整体拒绝——客户端**不做任何形状回退**（回退要么
  失去 protect 保护、要么失去 force 能力，两者都是本轮要消灭的降级），
  按错误行原样呈现并提示重启该实例的 dsh（宿主包随应用 seed，重启即同版）；
  **结果字段方向的歪斜（2026-13）**：旧客户端 + 新宿主安全（`residentRetainedRoots`
  是可选加字段，旧客户端忽略之，行照样保持隐藏、也不产生 shrink/墓碑）；
  **新客户端 + 未重启的旧宿主会复现本修正前的症状**（旧宿主仍摘标记 ⇒ 已加载
  会话的行回流，客户端按 §5 回退旧文案、F2 探针按 §12 主动放行）——这正是
  「重启该实例」口径要覆盖的窗口，**不做客户端兜底修补**（sticky 抑制会引入
  新的死端与客户端持久状态，见 §5/§13⑭ 的同类取舍）；
...
- `core.test.ts`（纯 fixture，不依赖 vendor，同 git core 模式）：候选集 /
  ...
  零读取）/ 树中止与 rerun 收敛 / **常驻保留（2026-13）**：loaded 树 force 删除
...
- binding：真 binding/RunGate 单测（无装饰器直测）——`assertHeaderShape`
  ...
  facts 拆分、`stat`/`locate` 面缺失三态、**删除瞬间常驻位报告**（idle agent /
  attached session / 无 live facts 三态 + **真 binding×真 core 的端到端保留**
  ——成员关系一笔不写）、**live facts 漂移 fail-loud**（`sessions.list()` 非数组 /
  条目无字符串 id / agent 状态未知一律 `registry-unreadable`，2026-13 review）；
  **protect 全套**（受保护根/受保护
  subagent 后代各整棵跳过、优先于 running/loaded/force 的分类、未知 id 无操作、
  受保护孤儿不清扫不摘集合、畸形与超限在权威读取前拒绝、空集合逐字节等价、
  plan 出错时删除循环前的二次保护复查不产生前缀删除、binding 删除原语
  `protected` 拒绝）；
- `retention-properties.test.ts`（2026-13 收尾补，确定性生成：固定种子 xorshift32
  × 2,000 轮）：在 fixture 之上覆盖**组合空间**，断言五条不变量——I1 任何"删除
  瞬间仍常驻"的根绝不离集合（用户要求本体）、I2 上报的保留集不 phantom、I3
  `forcedLoaded` 只计真的删了内容的保留根、I4 force 从不跳过 loaded 树、I5 重跑
  不删内容/不报 item 失败/**绝不摘 live 成员**/只摘无记录成员；失败消息带迭代号
  与全量状态快照，且种子固定 ⇒ 不引入 flake（同轮随机 harness 曾抓出"非 live
  leftover 会在下一次 run 收敛"这一文档错误，已回写 §4 step 9）；
...
- 客户端 wire：`instance-api.test.ts`（client 对象 stub + global fetch stub）：
  ...
  无重试腿）、旧宿主 args 拒绝原样上抛、`session/cancel` 请求形状、
  `stopArchivedSubtree` 的 advisory 语义、**`residentRetainedRoots` 解码**
  （正常列表 / 缺席 / 非数组 / 空数组 / 混入非字符串条目一律不伪造 id / **重复 id
  收敛**）；
...
- 实机 E2E（vendor 树就绪 + 打包态，§13）：本地实例归档若干会话（含 subagent）
  ...
  **常驻保留链（2026-13 用户报告回归）**：归档一个**本进程打开过**的会话 →
...
在 vendor/harness-packages（pinned submodule，当前 pin dsh-v0.1.5-rc.2 fb2c4b9e698e；
...
6. generic gateway：零参 Remote 的 envelope 要求（chamber 侧先例：
   ...
   `unexpected "sessionIds"` / `unexpected "force"` / `unexpected
   "protectSessionIds"`）——新客户端请求到旧宿主绝无「静默全量删除」分支，也
   没有形状回退（§5/§8）：`gateway/arguments-invalid` 原样上抛，UI 按错误行
   呈现并提示重启该实例的 dsh；
...
- **F1 墓碑抑制**（`shared/purged-rows.ts` 纯函数 +
  ...
  **与常驻保留的关系（2026-13）**：常驻保留的根**从不离开集合** ⇒ 对它们
  根本不发生收缩、不布防墓碑、也就没有"probe 放行"这一步——它们的行由归档
  过滤（`sessionVisible` / 官方导航同款）继续遮住，这正是本修正要的结果；
  F1/F2 仍然覆盖非常驻的幽灵行（清标记 + 官方 summaries 滞留）。
...
1. **实机门禁**（打包版 UI 目检，无自动化基建可替代）：purge → 切会话/任务
   ...
   清理成功）；**归档即终止**（归档动作后就地停止含 subagent 闭包；之后删除
   不再出现"仍在运行"跳过）；**保护修正三态**（无会话打开时照删 + 顶部降级
   说明行；正在查看的会话所在树被 `skippedProtected` 跳过；被回收来源可删）；
   工作树侧「只对 INERT 会话
...
11. **最低宿主版本**：存在性权威 `stat` 与产物定位
    ...
    **能力门响亮失败 + 运行期 fail-closed 降级**的分工：域的正确性依赖 `stat`，
    但绝不因为面缺失而误清成员。当前支持基线（0.1.5-rc.2）与回滚目标
    （0.1.3-alpha.2）都满足该面；
12. **force 路径与维护相位（不声称已解决）**：dsh 在**维护相位
    ...
    **REJECTED 备选**：读 vendor 的私有 phase 字段来检测维护相位——依赖上游内部
    ...
    wire 落地后本域退役（§11）；在此之前的三条缓解：归档即终止（归档动作就停
    一次）、删除侧对**闭包全员**发 cancel（维护相位报 idle，只有 cancel 能中止
    它）、管理器的 viewed 保护（`protectSessionIds`）——是最佳缓解，不是保证；
13. **force 删除后残档机制**：`force` 删除的是「本进程已加载」的会话档，而宿主
    ...
    (a) 归档时就停一次 + 删除前再停一次（闭包全员 `session/cancel`）；
    (b) 宿主按 `protectSessionIds` 整棵跳过**正在查看**的会话所在树（2026-09
    保护修正：判定已下沉到宿主，覆盖全量语料）; (c) 已归档会话没有任何 UI 路径
    开始新回合。**running 不能裸删**的理由同此机制（`force` 也不放过 running）;
    (d) **常驻保留（2026-13）**：这类树的成员关系不再被摘掉，所以即便残档被重建、
    甚至迁移竞态把整份内容写回（§4 step 10 / STATUS 清理残留②），行也仍然被
    归档集合遮住——本修正顺带封住了那条内容面残留的**可见症状**（磁盘泄漏本身
    仍需单独裁决）；
14. **保护的边界（两条同族残余）**：保护输入是本页各来源壳上报的**活**
    `current`，因此
    (a) **多客户端**——另一个窗口/另一个客户端正在查看的会话不在保护集内，删除
    仍可能剪掉它所在的树；
    (b) **掩码窗口**——vendor 把瞬时离列表的选中 `current` 掩码为 undefined
    （`SessionListSnapshot.current`；`manager.current` 私有、`watched` 私有），
    与"清空"在公开面上不可区分，本页在该窗口内也不会保护它（见 §5 的取舍）。
    两者都与两步编排（先读 `session/list`、后 `purge`）同族：旧口径用"整体
    拒绝"回避（代价是死锁与"归档后删不掉"），新口径选择"照删 + 宿主全量保护 +
    如实报告"。彻底闭合需要上游把「当前被查看」暴露为宿主/客户端公开事实
    （vendor 无公开的观察面枚举 API，已核对）；**不**采用 sticky 记忆，理由见
    §5（它会把用户刚归档的会话永久保护住，是更硬的死端）；
15. **无挂载基线的来源在降级窗口内无法清理**（v1 能力回归，用户决策接受，§8）；
16. **desktop 激活恒全量 vs seed 产物门**为设计内取舍（构建期 preflight 兜底），
    不修；
17. **归档管理器 a11y/视觉残余**（模块级偏差，视觉腿复核；原「行删除钮 20px 命中
    < WCAG 2.2 2.5.8」一条已于 2026-09 阶段 2 的 B-2 收口——视觉盒仍 20px，命中区用
    不可见 `::after { inset: -2px }` 扩到 24px，行高与布局不变：该行按内容撑高
    = 20px 按钮 + 上下 5px 内距 = 30px，直接换 24px 视觉盒会让它变 34px。**同类偏差
    至此全部收口**（2026-09 阶段 3 G1-4）：本表的行删除钮、侧栏 16/18/20px 图标钮、
    git 的两个 20px 图标钮都改为"视觉盒不变 + 不可见 24px 命中盒"，见 design 06 §7 与
    design 08 §3.4；本项**不再登记 20px 命中**，仅保留下面这些残余）；
...
19. **可选增强（未排期）**：PluginDialog 归档清理三态行（§7 F）、rowError 本地化
    ...
    `RiskConfirmation`。
20. **常驻保留的四条残余（2026-13；均为"不可见、自愈、不影响正确性"）**：
    (a) **无内容的归档 tombstone**——保留的成员不再有会话记录，需要孤儿清扫
    （F4/G3 `stat` 证明无内容）才会摘除；**时点分两类**：被覆盖但从未 attached 的
    后代在下一次 run 即可收敛，仍常驻的成员自身要等实例重启后（live 成员永不被
    清扫）。在此之前它一直
    留在集合里但**不可见**（管理器列"行 ∩ 集合"，行已随重启消失）——代价是集合
    体积与一次额外的清扫候选，无用户影响；**重跑的文案面（2026-13 review 登记，
    待目检）**：对这些已无内容的常驻成员，宿主既无树可删（plan 无记录）又按 live
    排除清扫，run 会落到空态文案（`archive.manager.empty`）而**不报错**——行与
    标注保持原样（幂等），但"没有可删除的已归档会话"与列表里仍有的行一起出现，
    需打包版目检确认读起来不困惑；
    (b) **管理器标注只在对话框生命周期内**——`residentPurged` 是对话框本地状态
    （不持久化，遵守"客户端不新增持久状态"的纪律）：关闭再打开对话框后，那行
    仍在列表里但没有标注（内容确已删除，只是少了说明）；若需要跨开关持久，须
    另行设计（可能方向：宿主把"内容已清理"做成可读事实，那属于本域"无读取面"
    边界之外的动议，需重新走 §2 例外评审）；
    (c) **多客户端各自为政（仅标注面）**——保留发生在宿主、对所有客户端一致（这
    正是选宿主方案的理由），但**标注**只出现在执行删除的那个客户端；另一个窗口
    里那行同样保持隐藏，只是没有"待重启"说明；
    (d) **`residentRetainedRoots` 无独立 cap（2026-13 收尾决定：不加）**——
    上界由请求自身给出：数组只装**本次候选根**（⊆ 归档集合 ∩ 调用方 `sessionIds`），
    即响应规模与调用方已提交的选择同量级；病态上界 = 手写 wire 的全集 force purge
    × 数万同时常驻会话（≈ 每 id 47 B，65,536 时 ≈3 MB），而 UI 路径永不带全集
    （§6 无「删除全部」）。加 cap 需要**额外的诚实计数键**（静默截断会让文案与
    标注数量对不上），收益只在上述病态场景 ⇒ 记录为已接受的边界，不引入新字段。
