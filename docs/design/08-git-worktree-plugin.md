...
> **状态：现行（v1 实现，Design 17 迁移期保留）**——Git 执行在每个 dsh
...
- occupant 渲染进 workspace 头部行内（title 与 rowActions 之间）。**行内不渲染分支
  ...
  （`+` ↔ kebab）一致同为 4px（修订——此前用官方 `Rows .rowActions` 的 12px，
...
- **行内动作揭示 pointer-safe**：动作按钮的样式钩子是 **`data-git-action` 属性**
  ...
  `[data-git-action]:disabled`；按选择器锚定——行号随文件增长漂移 复核时旧引用
  ...
  **不用字面量类名**：属性选择器不被 CSS Modules 哈希，跨包模块才能匹配同一钩子（本仓既有
  规则见 `packages/dsh-chamber-client-ui-mobile/src/client/styles.ts:10-20`；upstream-alignment，原 `git-ws-action` 全局类名已退役）。它与**折叠字形交换**（workspace
...
- **未注册工作树按仓库分散到 repo 组末尾**（名称=目录 basename、与派生 workspace 一致的行
  样式：26px 行 / r8 / 名称 14px-600-次级色 + 20px 行内动作钮——batch 1 G1 收口，
  其中 20px 命中 < WCAG 2.2 2.5.8 的 24px 属模块图标按钮语言的既有权衡）。
  **（用户指令「按照 v0.2.4 恢复」）**：命中盒 pass（`33238ffe`）的两层
  ...
  填充同值，指针悬停时整块消失 upstream-alignment）。无已注册 workspace 的仓库
...
- **dirty**：删除图标不再禁用（仅 dirty），点击进对话框显示醒目警示（"该工作树有未提交的更改，将被
  ...
  （`RemoveWorktreeDialog.tsx:411` upstream-alignment；单手势与撤销语义按 review-fix F1 校正）：对话框内无勾选框，点「移除」时尚缺授权则先弹官方风险确认（警示图标 + 同上
