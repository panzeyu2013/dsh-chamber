...
> **本地实例托管与部署形态**——控制面以 dsh 内置 web profile
...
> - **移出的旧体系（不得回流）**：slim profile 生成与维护、glue 插件、旧业务补丁层
...
- **同端口仲裁**：同一起始端口时，先成功就绪者占住端口；后来者探测到
  ...
  进程**（先注册先托管）。
### 2.6 chamber host 包的 seed 与单一 loader overlay（设计 08/09/20/24；四个 host 包）

官方 web profile 仍是宿主组装权威；chamber 只追加自身拥有、边界明确的 host package（四个：
...
8. **手工启动代次**：候选端口预检和 TCP 就绪轮询的
   ...
   信号、销毁在途 socket 并清理 detached 尝试），等待旧代终结后释放该代的
...
- **控制面自身日志落盘**（取证缺口修复）：`createControlPlane` 无条件给注入的
  ...
  - **写入面纪律**：①日志行**同步**追加（每行一次 `writeSync`，无 fsync）
