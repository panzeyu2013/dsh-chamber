...
> **ssh 后端插件编排；范围 = `{kind:'dsh', transport:'ssh'}` 目标**。
...
> 决策 19 的修订口径：`P = B₀ ∪ S ∪ F`，**装面保守**——远端无 family 事实源，官方
...
- **行派生为纯函数**：行派生是纯函数 `deriveChamberRows`（`plugin-inventory-text.ts`，locale-free：
  ...
  专用的本地清单读取；此前 ssh 的本地列恒为「未知」、探测失败还会清空表格），ssh 在远端
...
- `desktop_ssh_plugin_apply` 的 **registry add/remove** 同样须主进程确认——远端安装是持久执行面，与
  ...
  plugin_apply 触发无变更重启。**当前实现状态**：确认链位于 gateway
