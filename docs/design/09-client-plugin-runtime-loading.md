...
- **反向依赖（现行仅 1 条；名单自 2026-09-11 起为派生）**：覆盖集解决「复合行不需要宿主图」，但复合内
  ...
  **W3 边界（2026-12）**：② 覆盖**除 `not-injected` 之外的所有通道失败**（503 预算耗尽、502/504、
  ...
  `shouldReportGraphUnavailable`；`not-injected`（HTTP 404 或通道答 method 缺失）仍是 gateway/mobile
