...
- **逻辑流开帧丢失与首帧期限（2026-09 ui-chat 卡死排查的根因修复，chamber fork）**：
  ...
  - **第二次排查（2026-09-21）：把期限升格为物理载体重建** —— 本条此前的缺口正是「期限只失败 inbox，重试道只能在同一物理世代上重发同一个请求」：`stream-client.ts` 的开帧期限回调在同一请求连续第 2 次超时（30 s + 60 s 已放宽预算）且 socket 仍为 OPEN 时调用新的 `rebuildSilentCarrier()` —— 与车道下令的重连同形：退休当前 socket、以普通 `RemoteStreamCarrierError` 失败全部待决流（重试道照常重开）、从基线节奏重连，并留一条 `opening-stall-escalation` 取证事实；`shouldEscalateOpeningStall()`（`remote-retry-policy.ts`，纯函数）把「第一次超时永不升级」与「60 s 冷却内不重复重建」钉死在真值表里。它覆盖的正是**宿主侧无期限**的那类卡死（`session/follow` 无首帧、宿主 fiber 挂住、静默半死 socket、`$events` 世代卡住）——chamber 不改宿主，于是把期限权威放在客户端载波层；宿主一旦作答即删除该请求的 streak，慢而健康的宿主（本次实测开局帧 14–273 ms，含并发/大会话/缺 maxMessages 各形态）永不被升级。行为用例 `mux-self-heal.test.ts`「an opening that is never answered rebuilds the physical carrier and recovers」与「a slow-but-answering Host keeps its carrier」，并已变异验证（关掉升级 → 前一条红）。
  ...
  - **第三次排查（2026-09-21，渲染进程崩溃归因轮）：两条新事实 + 三处收敛**
    ...
    3. **仍未闭合**：JSC 崩溃本身是引擎缺陷（WebKit 22625），仓内只能降低触发概率与把状态可见化，不能消除；vendor 侧自身的 rAF 循环（chat/conversation/layout/trajectory）仍在同一批次里；页面事实的持久消费面仍是 STATUS ⑩；10:19 那次没有崩溃报告的流层卡死仍属本 D4 前三段的范围。
  - **未闭合**：抖动触发源未钉死（ready 握手已证快速：`$events` 25 ms）；取证事实的**持久消费面**仍是 STATUS ⑩（新的 `opening-stall-escalation` 同此缺口，仍只在页面事件里）；升级阈值的真机校准；宿主侧首帧期限（upstream-proposals §4.3）；首帧期限与探针阈值未经真机校准；`AbortSignal.any/timeout` 恰好压在文档基线的 macOS 14.4 / Safari 17.4 门槛上（仓内 sidebar 已同款无守卫使用），需一次真机冒烟；错误臂在「刚恢复后的 cooldown 窗口」内只显示「正在恢复…」无按钮（≤120 s，cooldown 到期自动重试），是否再给一个手动出口待真机观感。
