...
- **会话运行位活性守卫（2026-12，Swift 原生版「深度求索中」永久卡死修复）**：
  ...
  **状态机不变量**（2026-12 二轮复核后定稿）：时段按**每会话计时**（`runningSince`）
  ...
  ① **L1 只读对账**：某来源持续 running ≥ 120s ⇒ 经既有
     ...
     滚动窗口 10 分钟 ≤3 次。
  ② **L2 有界 reconnect**：仅当**对账回执**证明**对账通道**已坏（**非 `unknown` 的**失败回执 `ok:false`，或请求后 150s（**第一次 `unknown` 会把期限从它自己的时刻一次性顺延**——见下）
     ...
     一致才结算 `converged`；探针**正面证伪**（官方说 running 而权威说没在跑）或 refresh 相位自身失败/超时结算 `stale`（允许升级）；**探针自己失败/超时结算 `unknown`**——它走控制面 HTTP 代理，与被守卫的 WS 事实通道是两条载体，只推进水位、不升级也不清「等回执」计时；但**第一次 `unknown` 会把等回执期限从它自己的时刻重新起算一次**（第二次起不再顺延）——生产常量下等回执期限 150s < coalesce 200s，若不顺延，"探针无法裁决"必然抢在下一次 L1 之前被判成"对账通道已坏"，一次 502/代理重启就会制造假 L2 并吃掉该 running 时段唯一重连预算（2026-12 独立复核的时间线仿真）。持续无结论仍由 `outcomeMissing` 在顺延后的期限超期收口（有界）。
  ...
  **被否替代（Rejected alternatives，2026-12）**：
   ...
   - **把 `unknown` 当失败立即升级**：辅助探针走控制面 HTTP 代理、被守卫的是 WS 事实
     ...
     水位、并**一次性**顺延等回执期限，持续无结论由 `outcomeMissing` 在顺延后的期限
     收口（不顺延则期限必然抢在下一次 L1 之前到点，等于"立即升级"）。
