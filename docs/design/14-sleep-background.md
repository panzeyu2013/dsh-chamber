...
- 设置项 `desktopMinimizeToTrayEnabled`（win32/linux）；关窗事件里 `shouldHideMainWindowToTray(browserWindow)`
  为真 → `event.preventDefault(); browserWindow.hide()`——窗口只隐藏，Electron 主进程 + 进程内 web 服务器 +
  其 sidecar/SSH 子进程全部继续运行（`main.mjs`）。macOS 关窗默认 hide（`window-all-closed` 在 darwin 直接
  return），Cmd+W/红点不退出、Cmd+Q 才退出（`before-quit` + 退出确认）；窗口全关后应用保持运行（托盘常驻），
  `activate`/`second-instance`/
  托盘点击 → `show()` + `focus()` 恢复。

### 2.2 托盘控制器（常驻入口 + 状态投影）

- `tray.mjs` `createTrayController`：idle / breath（呼吸动画）/ unseen 图标帧 + 会话/审批快照菜单；点击 → `onAction` 回调回主进程路由。
  渲染端 `useTraySync.ts` 经 `desktop_tray_update` IPC 把**非秘密快照**（会话数、审批数、runtime key）推给
  主进程，点击路由到**快照所属 runtime 的窗口**（防会话 id 串台）。托盘三平台全启用；macOS 菜单栏项默认开、
  可在 General 设置关闭（`main.mjs` / `electron/README.md`）。

### 2.3 后台启动（无窗口常驻，登录自启）

- `--background` 启动参数 + 登录项（`shouldStartInBackground`：argv 含该参数或
  `wasOpenedAtLogin`/`wasOpenedAsHidden`，`main.mjs`）；无窗口启动时进程内 web 服务器照常启动
  （`resolveInitialUrl` 等）、`startupResolved` 置位，**窗口后开时重新探测远端**而非信任登录时刻的
  reachability。登录自启：darwin/win32 `setLoginItemSettings`（带 `--background`）、Linux XDG autostart
  （`linux-autostart.mjs`）。

### 2.4 唤醒恢复 + 防休眠 + 退出保护

- **唤醒恢复**：`powerMonitor.on('resume')` → 广播 `openchamber:system-resume` → 渲染端 event-pipeline
  （SSE 事件管线）监听该事件**立即重连**，不等心跳看门狗超时（`main.mjs`；
  `packages/ui/src/sync/event-pipeline.ts`，另有 visibilitychange/online 兜底）。**防休眠**：
  `powerSaveBlocker.start('prevent-app-suspension')`（设置项 `desktopKeepAwakeEnabled`，
  `setDesktopKeepAwakeActive`）。**退出保护**：`quitRisk`（活动隧道 / 运行中的定时任务）→ 退出确认对话框
  （"Quitting now will stop sidecar/background processes…"）；确认后 `shutdownBackgroundServices`（kill
  sidecar + `sshManager.shutdownAll()`）；SIGINT/SIGTERM 硬信号同样走后台服务清理。渲染设置面：General 设置
  页 `DesktopNetworkSettings.tsx`——keep-awake / minimize-to-tray / mac menu bar / launch-at-login / LAN
  access 开关，写 app 自身配置（`/api/config/settings`）。

### 2.5 移植要点（差异）

OpenChamber 的 sidecar 与 web 服务器在**主进程内**；dsh-chamber 控制面同样是主进程内对象
（`createControlPlane`），传输层与 dsh 子进程由主进程管理——**hide 窗口后无任何东西需要额外保活**，模型
直接成立。差异：dsh-chamber 无「进程内服务器自启」（控制面由 `whenReady` 显式 start），无需
`startupResolved` 语义；v1 不做会话级托盘（会话业务归各实例前端 runtime，01 §5 / P2 纪律）。
...
- 关窗分支（`browserWindow.on('close')`）：设置 = hide-to-tray 且非显式退出 →
  ...
  继续运行。显式退出（托盘「退出」/ Cmd+Q / 应用菜单）走现有 `will-quit` cleanup single-flight：先阻止新 runtime
  启动并中止在途 operation，再并行等待 plugin-sync/本地插件子进程、transport、control-plane、runtime
  installer 与在途 transaction；有界超时 fail-loud，不留孤儿进程。
...
- **隐藏窗口节流 = 保持 Chromium 默认（2026-12 修订；实测换判，取代旧「不节流」定案）**：
  `createMainWindow` webPreferences **不再**设置 `backgroundThrottling: false`（旧判对齐 OpenChamber main.mjs）担心「隐藏后计时器被节流 → D4 唤醒重连被拖慢」。2026-12 在 Electron 43.4.0 / Chromium 150 /
  M5 Pro 120 Hz 实测：关闭节流会让 Electron **永久抑制隐藏态**（`disable_hidden_ = !backgroundThrottling_`，
  `shell/browser/api/electron_api_web_contents.cc`）——隐藏后 rAF 仍 120/s、帧照画、
  `document.visibilityState` 恒 `'visible'`、`visibilitychange` 0 次，`retention.ts` 的
  `shouldRunBackgroundPhase` 六处门控**从不生效**，隐藏期 renderer CPU 最高 28.1%（上游常驻动画 19.0% +
  发布 19.4%）；恢复默认后隐藏期 rAF 0、CSS 动画暂停、CPU 0.0–0.1%，**SSE 是网络流不受影响**（隐藏 9s 收
  9/9 条、`maxGap` 1005ms、0 错误），唤醒即时重连走 `powerMonitor.on('resume')` →
  `dsh-chamber:system-resume` 的 IPC 推送（非计时器）；被节流的只有 <1s 定时器（100ms→1Hz；1s 保持 1Hz），
  3s/15s/30s/60s/120s 看门狗节奏不变。**Rejected alternatives**：① 保留 `backgroundThrottling: false` +
  新增「窗口隐藏」IPC 推送由渲染侧暂停动画/发布——多一条宿主桥面（preload / bridge manifest / 锁步测试）而
  收益已被恢复节流覆盖（隐藏期已是 0.0%），不采纳；② 运行时 `webContents.backgroundThrottling = true`——
  实测对**已隐藏**窗口不生效（rAF 仍 120/s、`visibilityState` 仍 visible），只在建窗时有效。
...
- **单飞**：`quitRequested` 置位后不再重复弹确认（防连点/双路径）；异步 will-quit 清理也有独立
  single-flight，第二次 quit 事件继续 `preventDefault`，不因首轮已清空 controlPlane holder 而提前退出。
- renderer 崩溃/无响应恢复 timer 由窗口生命周期持有（新导航/恢复/close 清旧 timer），quit 在途禁止
  reload/show/loadURL 失败路径复活窗口或重入 teardown。

### D3 托盘增强（可选）

- 状态 tooltip：`dsh-chamber · 控制面 http://127.0.0.1:<port> · <connectionState> · 连接 N/本地实例运行中`
  （非秘密投影，来自 transport-manager status push + control plane `/health`）。
- 菜单：显示窗口 / 退出（现状）+ 可选「N 个远程连接活动」只读行。
- 保持防御式构造（沿用 `maybeCreateTray` 的 try/catch：无图标资源/
  失败 → 跳过并日志，绝不阻塞启动）。
...
- **会话运行位活性守卫（2026-12，Swift 原生版「深度求索中」永久卡死修复）**：ui-chat 运行指示器由官方
  session 的 `running` 位驱动（`dsh-client-ui-chat`: `running = useSession(s => s.running)` → `TurnStatus` →
  `chat.deepDiving`），
  该位只由 mux `$events` 上一条 **emit 型转发事件** `api-session/status` 递送（`dsh-api-session-controller`: `handleSessionStatus` → `handleRunning`）；
  emit 无重传、`$events` 开场帧不重放会话状态（2026-12 独立复核逐行核实）。官方另有两条**非周期**写 running
  的路径——物化时按缓存 summary 播种、`refreshList()` 回灌权威 summary（`connection/reset` → `handleConnected` 及首次 apply 期）
  ——但**无任何周期性触发点**（全树无官方 `sessions.refresh()` 调用者、无 session.list 轮询）⇒ 丢一帧或
  carrier 静默半死时该位永久为 true（客户端零超时、零出口）。**故障类精确命名**：传输层已有周期性
  liveness（宿主 mux 2s×2、控制面浏览器腿 30s×1），覆盖「socket/心跳死亡」；本守卫针对「**socket 与心跳
  全健康、而宿主侧转发事件源停摆**」（fiber 被 dispose / 源静默；emit 无重传也无游标），无任何现成触发器；
  宿主侧无责（agent loop 在 `finally` 必写 `turn/end`），且 §D4 的 liveness 触发器全部要求 OS 级事件
  （唤醒/网络/可见性），窗口可见且机器未睡时一个都不响。修复 = 三级阶梯（决策半
  `renderer/src/session-liveness.ts` 纯模块，执行半 `dsh-chamber-client-ui-sidebar/src/shared/session-fact-reconcile.ts` + producer 既有 refresh seam）：
  **状态机不变量**（2026-12 二轮复核后定稿）：时段按**每会话计时**（`runningSince`）起算——短会话反复
  开始/结束**不得**重置真正卡住的长会话的时段；只有来源代际（registry fingerprint）变化、或当前运行集合
  与上一 tick 完全不相交时才重起算（挡住「A 结束、B 开始」继承旧配额与旧提示）。**L3 门 = 未收敛证据 +
  宽限**：(`outcomeFailed`（最近一次**非 unknown** 结算 ok:false，且结算时刻晚于最近一次真实重连——重连前
  的 sticky 失败不算新证据）或 `outcomeMissing`（等回执超过 150s = 对账链最坏回执 2×(20s+35s)+1.5s
  ≈111.5s 之上）) ∧ **梯子到顶**（真实 L2 预算用尽 **或** 连续 no-op 派遣达 `maxNoopReconnects`——「杠杆
  一直不可用」的第二条出口）∧ 距**梯子到顶那一刻**（`ladderAnchorAt`，只记一次）≥ `noticeAfterMs` ∧ 本轮
  尚未提示。健康回执清除未收敛证据并撤下提示，**新的**失败证据可再次武装——不会被守卫自己的 L1 请求无限
  推迟，也不会被一次健康回执永久 latch，更不会在「配额间隙没发请求」时误报（只用时间锚点的版本正是这样
  产生周期性假横幅，2026-12 二轮复核修复）。
  ① **L1 只读对账**：某来源持续 running ≥ 120s ⇒ 经既有 `chamberBridge.requestSessionListRefresh` 请挂载
  ctx 重跑官方 `ctx.sessions.refresh()`（single-flight；其 `refreshList()` 把权威 summary 的 running 回灌
  到已物化会话 ⇒ 卡死的位自然掉落）；节拍 = `refreshCoalesceMs` `200s`（**= refreshWindowMs /
  maxRefreshRequests**：配额铺成均匀节拍——用 60s 会在 120/180/240s 爆发用完窗口、之后失明 8 分钟；相等时平均成本不变而最坏未探测时长 = 200s），滚动
  窗口 10 分钟 ≤3 次。
  ...
  ③ **L3 可见提示**：有未收敛证据且距梯子到顶（`ladderAnchorAt`）≥ 120s ⇒ 顶部非模态横幅（重连 / 重载
  应用页面 / 忽略），**绝不自动重载**（与 mobile `session-stall.ts` 同纪律）；横幅只在有证据时出现，通道健康时不会因探测间隙反复亮灭。
  **核心取舍**：升级的唯一依据是「拿不到权威结论」，不是「沉默很久」——长工具/长推理的合法静默与真卡死
  在 App 层不可区分，误升级（每次 reconnect 重放全部会话 baseline）会引入比原缺陷更糟的风暴；L1 是读
  操作、可廉价重复。实测：活跃 turn 期间宿主 durable 进展 5–21s/次（median 11s，161s 13 次），合法静默
  可达 75s（TTFT）到数分钟。
  **被否替代（Rejected alternatives，2026-12）**：
   - 把 L1 放进 mobile 的 `session-stall.ts`：该插件只有 gateway/mobile flavor 加载，desktop 与 Swift 原生
     壳不挂它 ⇒ 打不到缺陷现场；且自带未校准 45s 阈值，会造出第二套「停滞」概念。
   - 只做 L3 横幅、不做 L1 对账：`running` 位不收敛，用户唯一出路是整页重载（丢页面状态），而收敛动作
     只是一次只读 `session.list`。
   - 用 disconnected/close 事件驱动升级：缺陷本体是「carrier 静态半死、socket 不 close」，事件根本不发。
   - 决策机放进 client-plugin 直接盯官方 running 位：拿不到 App 的 reconnect 杠杆与 registry 代际，且每个
     挂载页各持一份预算（多 ctx 放大成 N 次对账）。
   - 改 vendor 在宿主侧加日志/心跳：pin 升级即丢，违反「不改上游」边界；诊断价值已由 P1 的两处落盘
     （02 §3.8 与 25 §3.1）拿到。
   - 把 local 也纳入既有 S2 重连臂：S2 判据是「mounted 快照静默」，而本缺陷里工作区/会话列表推送照常活着
     ⇒ 源看起来「新鲜」、该臂永不响；必须按**运行位本身**的收敛证据判定，不能复用快照静默。
   - 改用 App 每 30s 兜底 unary pull 的权威 running 行直接对表（STATUS ⑪ 下轮首选）：可删
     reconcile/verify/回执整条链（≈450–500 行），但① `aggregates` 会被 producer push **整块覆盖**、与
     runtimeFacts 同源于官方 store（两份事实不独立）；② push 会作废在途 pull；③ 该 pull 只在源 stale 时
     发生，推流存活的源根本不拉 ⇒ 要覆盖「丢一帧而流仍活」必须另加旁路采样面，收益不敌 push/pull race。
   - 把 `unknown` 当失败立即升级：辅助探针走控制面 HTTP 代理、被守卫的是 WS 事实通道，一次 502/代理重启
     就会拆流并重放 baseline；改为 unknown 只推进水位、忽略期限直到下一次 L1 发出（第一次 unknown 被吸收）。
   - refresh/verify 共用一个尝试计时器（或沿用 90s 总预算）：慢宿主上「refresh 十几秒 + 探针未回」会被判
     「拿不到权威结论」⇒ 假 L2；改为两相位各自计时（20s/35s）、等回执期限抬到 150s，并由接线测试锁住三条
     不变量（最坏回执 < 等回执期限、verify 预算 ≥ 探针自身 30s 上限、生产构造点不得 override）。
  **未闭合**（权威清单与逐条判据见 `docs/progress/STATUS.md`「会话运行位卡死」，编号以 STATUS 为准）：
  ① transcript 与运行位可能分别收敛（STATUS ②，需上游逐流交付统计/游标）；② 官方 `session.list` 单飞悬挂
  时「重新连接」也无效，只剩「重新加载」（STATUS ③）；③ 子代理会话不在事实通道（STATUS ④）；④ 隐藏期不
  tick，恢复补偿 tick 按**累计** running 时长判定（STATUS ⑤）；⑤ 三级动作目前只落 renderer console
  （STATUS ⑩，真机取证缺一条落盘链）。L1 把卡住的 running 位压回 false 时，官方完成通知/完成蓝点边沿照常
  触发（`syncCompletedNotifications`）——修复的副产品，不需要额外机制。
...
- **对话流健康臂（2026-12，纯 chamber 代码）**：本节阶梯覆盖不到的那类真机卡死的收口。
  - **缺陷本体（已复现）**：连接代际仍 ready 时连续载体丢失 ⇒ 上游 `waitForRemoteStreamRetry` 把
    `RemoteStreamCarrierError` 抛成终局 ⇒ `Session.failEventStream()` 把 `openState` 锁成 `'error'`（窗口
    保留、流不再重开），官方聊天面只在列首渲染一行 `chat.loadError`。headless 复现：同一 mux socket 在
    1.2s 内被拆 4 次（`closeAll` + `killNext(3, 25ms)`），页面此后不再重发 `session/follow`，DOM 冻结在
    213 行 / 92 turn；对照跑（同一抖动、快照被接受）journal 正常重开 ⇒ 是 race 非必失败。真机同形证据：
    `13:35:47.689Z local closed (upstream close, 368803ms)`（用户报障前 91s）与该源两个子代理会话同时冻结。
  - **唯一杠杆与硬边界**：公开 `ISession` 无 `open()/resync()`；`clear()` 不动 stage（`followCurrent()`
    在 `current === undefined` 时直接 return），连接代际 reconnect 也不重开 journal。可用杠杆只有 stage
    迁移：`open(neighbor)` → `open(target)` 同步两步，让 `followCurrent()` 在 `Session.open()` 上重跑
    `doOpen()`——但只对 `openState !== 'open'` 生效；`'loading'`（`openPromise` 在途）与 `'open'`
    （状态短路）都不可重开。
  - **落地**：`packages/dsh-chamber-client-ui-open-in`（既有 per-instance 头排座席宿主）新增
    `src/client/session-stream-health.ts`（纯决策：error 满 8s 自动 stage 迁移重开，冷却 120s、滚动窗口
    10 分钟 ≤3 次；loading 满 20s 或阶梯无杠杆时给「重新加载」提示）、
    `src/client/session-stream-health-probe.ts`（stage 迁移与面形状读取，全部 fail-closed）、
    `src/client/SessionStreamHealthChip.tsx`（`conversation.session.header.actions`，list/session 作用域：
    只显示、绝不自行重载）、`src/client/session-stream-health-seat.ts`（座席注册 + **按会话持有的阶梯状态**
    + 非抛错 vendor 面读取）。两条 review 修正写进契约：①**阶梯状态在座席而不在组件 ref**——会话子树按
    绑定 key 挂载、切会话即卸载，ref 会让同一会话每次访问重获 grace/冷却/预算、storm bound 失效；②stage
    迁移有**前置条件**：target 必须"仍是 current 且在列表里"，否则拒绝（address-only 子代理首步失去
    eligible 被 `pruneScopes()` 拆 scope 并 release 输入壳附件；masked gap 让回程 `open(target)` 抛错把
    用户留在邻居会话；未 flush 的旧 effect 把 stage 抢回已切走的会话），退化为提示臂。控制面只保留一条
    取证日志：握手完成前下游腿离开时记
    `WebSocket upgrade <id> abandoned (downstream close before upstream handshake, Nms)`（归因边界：控制面
    自身 `revokeTransportTraffic`/`closeAllStreams` 拆 socket 也走这一行，不可据此判定"浏览器主动离开"）。
  - **触发面：做完又被否掉的一条（2026-12 review）**。原设想让控制面在上游腿**代答宿主心跳 pong**（被动
    `PingScanner` 扫宿主 ping → 自己写 pong），把宿主 2s×2 判死从浏览器网络进程解耦；被两处协议级证据
    否定、已整体回退：①上游腿里代理是 **client**，RFC 6455 §5.3 要求 client 帧掩码，pinned `ws` 对未掩码
    帧直接 1002（`WS_ERR_EXPECTED_MASK`）——代答会在首个心跳（~2s）后杀掉每条 mux；②pong 插在
    `socket.pipe(upstreamSocket)` 字节流**中间**，浏览器帧跨 chunk 被劈开，宿主按 TCP 顺序读到
    `[帧前半][pong][帧后半]`（实测 `invalid UTF-8 sequence`）。正确实现需要"浏览器方向帧边界感知的写入
    路径 + 掩码"，代价与风险落在被保护的传输层、收益只是降频。**残余选项**留 STATUS「连接稳定性」：托管
    实例 patch overlay 调宽 `websocketHeartbeatIntervalMs`（宿主 schema 有该键，`api-gateway` 插件
    `Config.websocketHeartbeatIntervalMs`，默认 2s；只动宿主自己的判死容错（~4–6s ⇒ 如 30s×2），不注入
    任何字节；代理下行腿心跳仍是 30s 周期、回答后才重新计时，最坏约 60s 才 onDead，`ws-heartbeat.ts`）。
  - **拒绝替代**：整页自动重载（丢页面状态，只能由用户点——与移动端 `session-stall.ts` 同纪律）；
    `session/follow` 盲空闲超时（合法长静默：TTFT 75s 起、工具可数分钟，必然误报）；fork `ui-chat` 改
    错误行（82 文件，design 09 已拒）；代理注入合成 `end`/未知帧逼客户端重试（未知帧会让客户端 `failAll`
    并关 socket，且空闲会话被周期性重放）；代理透明重放客户端流开帧（无流上下文，触发 `RemoteJournalStream` 的「多次 opening
    cursor」协议错误）；上游腿代答宿主 pong（见上段）。
  - **未闭合**：`openState === 'open'` 但静默的半死形态仍无自动杠杆（无 applied cursor 水位时与合法长静默不可区分）——宿主侧流级
    keepalive 仍未做；「让 carrier 失败永不终局」已落地（见下条）；自动重开的**实机验收未做**（判据 = 真机
    抖动后自查恢复，且 `local closed (upstream close, …)` 频率不再随浏览器卡顿波动），阶梯阈值
    （8s/20s/120s/≤3）与 `presented` 判据的近似（document 级 `[data-chat-flow]` 存在性，非「实际可见」）
    待真机校准。

- **载体故障的用户可见面（2026-12）**：治因 = `packages/dsh-api-gateway/src/client/remote-stream.ts` 在活世代下用
  有界退避（250ms 起翻倍、10s 封顶，与连接车道 `backoffMaxMs` 同值）重开而不逃逸为 `gateway/internal`；
  信号面 = 该包 `$stream` 工厂组合 `carrierFailed` 成有界页面事实 `dsh-chamber:stream-carrier-failed`
  （`stream-carrier-fact.ts`：计数 + 时间 + 实例 id + 截断消息，dispatch 抛错被吞，绝不打断重连），由 open-in 健康臂座席按
  实例过滤后喂进纯决策模块，chip 以 `role="status"` 显示「对话流正在重新连接…」（信息性、不给「重新加载」
  按钮，超过 `carrierChurnMs` 自行消退）。
  - **拒绝替代**：（a）不做可见面——去掉终局逃逸后用户分不清「流在重连」与「会话本来就安静」；（b）打包期
    import 该 fork 的事件常量——client plugin 不应加深进 fork 的 import 路径，故字面量复制并由
    `stream-health-wiring.test.ts` 钉住两处拼写（vendor-lockstep 先例）；（c）ctx service seam——fork 探针面会被
    chamber 插件的存在绑住，而页面事实在座席缺席时无害；（d）让 carrier 失败重新终局——正是本次要修的根因。
...
`test/session-rows/workspace-membership.test.ts` 的回执投影边界）、`test:control-plane`（`test/log-file.test.ts`、
`test/host-lifecycle/lifecycle.test.ts` 的落盘/reopen 端到端）、`test:open-in`
（`test/session-health/session-stream-health.test.ts` 阶梯真值表 + `stream-health-wiring.test.ts` 座席接线锁 +
`vendor-heal-contract.test.ts`：三条 vendor 事实 lockstep，pin 升级语义变化即红）、`verify:test-wiring`（**全部
test manifest 必须登记在案**）、`test:swift`（ShellLogTests 全部用例，含 sidecar 文件名/轮转名/落盘三条 +
