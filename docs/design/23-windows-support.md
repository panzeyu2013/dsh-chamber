...
- **托盘与 preload**:托盘图标候选收敛为只留真实打包资源(两条永不随包的候选路径
  ...
  静默回退源码 `.cts`。
- **登录自启**:win32 走 `app.setLoginItemSettings({ openAtLogin, name: WINDOWS_APP_USER_MODEL_ID })`
  (Electron 43.4.0 在 win32 以该 name 写 HKCU `...\Run`,当前用户、无需管理员;`getLoginItemSettings`
  的读回键同为该 name,故 P-20 读回校验一致——传其它 name 会让读回永久失败),`supported` 恒 true;
  NSIS 卸载段 include(`packages/desktop/scripts/nsis-uninstall-cleanup.nsh`)清
  `dsh-chamber`、`@dsh-chamber/desktop` 与 AUMID `com.dshchamber.desktop` 三个 Run 值
  (前两个覆盖旧版本/双 flavor 残留)。
