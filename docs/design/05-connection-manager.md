...
- **会话行标签 = 官方链**：`durable title → cwd 目录名 → 会话 id`，在侧边栏单点解析
  （`packages/dsh-chamber-client-core/src/derive.ts` 的 `sessionDisplayTitle`， 两个快照构造都经它接线，并作为必需字段 `SessionRow.displayTitle` 携带）；该字段进
...
- 会话行悬停操作（v1 最小集，走该来源自己的 API）：重命名/**fork**/归档（**kebab 菜单
  ...
  重命名、删除（kebab 菜单）。**workspace 删除用应用内官方 `Modal` 确认**（upstream-alignment T2b)：outline 取消 + outline 危险确认 + 一句说明 + `role="status"`
...
- 未挂载来源的**工作区集合**仍只有"回声 + 挂载 push"两个来源：**别处**创建 / 改名 / 删除的工作区与工作区**顺序**仍要等该来源被挂载（用户点开）才收敛（§2.3 已登记的降级面）；本修订刻意不引入"每次变更付一次后台 boot"的收敛臂。**应用内**发起的工作区变更已由唯一出口覆盖（第二入口修订），不属于本残余。

**被否方案（Rejected alternatives 第二入口修订）**：
- **逐调用点各发一次回声**（修复的原形态，只挂在侧栏对话框上）：正是缺陷成因——Git 的 create / adopt / 两类 recovery 四个调用点漏发，未来新入口也可能再漏；由单一出口取代。
...
`(instanceId, sourceFingerprint)` 隔离的有界本地状态中。首升级没有历史墓碑时，只清理宿主列表缺失且
非运行、非空白、非近期活跃的旧摘要；运行中、新空白或近期活动会话不会因一次创建竞态被隐藏。过滤在
侧栏快照与运行事实发布前完成，官方列表收敛后撤销墓碑。状态只含有界 session id，不含内容或凭据。
**Rejected alternatives**：只保留页内归档墓碑会在 renderer 重启后复现幽灵行；直接改上游私有 summaries
会和官方 store 分裂；无条件隐藏所有首轮 host scan 缺失项会误伤刚创建或未落盘的会话。选择持久化有界
墓碑并对首次迁移候选做活跃态保护。
...
- **切换实现（即时隐藏 + View Transition + 骨架屏，content-visibility）**： 非活动视图用 `visibility:hidden + opacity:0 + pointer-events:none` 即时隐藏，
  ...
  **骨架/框架文案进 typed 字典（T16 upstream-alignment）**：框架（App / InstanceView /
...
**被否方案（P0/P1/P2）**：①把遮罩 z 提到壳内所有值之上、或让官方 composer 降 z——数值军备竞赛，且要 fork 官方包（越界）；②顶层（popover/dialog）幕布——实测能压过 body portal 的 z=1100，但它是**文档全局层**，会同时盖住所有视图与 chamber 自己的 chrome（含 `.boot-gap` 非模态降级横幅）；③每个视图实例各自一个 `view-transition-name`——旧/新快照名字不同，配不成组，等于没有命名过渡；④`View Transition types`（`:active-view-transition-type()`）——WebKit 支持面不确定，改用宿主 `data-vt-intent` 属性达到同样作用域；⑤租客 **body portal**（ui-chat 4 处 + chamber 2 处，z-index ≤1100）渲染在文档 body 上，**在任何 stacking 边界之外**——边界内不可消除，作为残余登记（STATUS）。 **失败呈现**：失败不由各 InstanceView 自绘（旧 `.instance-fatal` 仅重试、无导航），而由 App 在
  ...
  dispose 该 entry，重试干净重 boot)。**失败报告内容与官方同源（T15
  ...
  按文档语言解析)+ 官方 `Button variant="primary|outline"`，控制面不可达块同样是 `role="alert"`。**框架的 `Button` 走深路径导入**（`@deepseek-ai/dsh-client-ui-primitives/src/Button.tsx`
  ...
  方案（同样无闪烁）（styles.css `.instance-view.instance-hidden`）。**追加纪律
  （「看不到但能点」的空白图标）**：CSS 动画/过渡只在子树被渲染时推进，隐藏壳里创建的入场动画
  被钉在首帧（`opacity: 0`）——元素仍可命中、除重挂载外不自愈（WKWebView 实测见 STATUS）。**更正**：动画退役后同一症状仍复现，当前根因（文档级重复 SVG 资源 id × 隐藏壳丢绘）与本条纪律的
  关系见 §4.2；本段只覆盖入场动画特有的风险。故
  ...
  **降级呈现**：boot **成功**但已知缺口（来源在启动窗口内没有提供客户端插件图／图到了
  ...
  自己写一套会漏掉最后那条探测臂。
  **降级事实的第二批座位（同一修订）**：事实还经**既有投影通道**过桥——
...
- **每 entry Context 私有注入**：`AppWebEntry` 提供 `configureContext(ctx)` seam；shell.ts
  ...
  不同 entry 不通过 `window.__DSH_BASE_PATH__` 或页面级 `chamber-knob.ts`交换 boot
...
### 4.1 未 settle boot 的呈现与逃生（boot 死区收敛）
...
- **通道失败进入降级事实（W3）**：图通道失败（非 `not-injected`：502/504、网络
  ...
  **boot 表面的可见性 + 自愈**，不是"从零到一"（复核 M3 更正了口径）。
...
- **残余边界**（有意接受，改动面越小越好）：① 样式面**尽力而为且只读安装期见过的内容**——跨源样式表、`adoptedStyleSheets` 读不到，仍在加载中的 `<link>` 要等它 `load` 后才会被重读（**本批已经改名的图标无法回退**），「已存在 `<style>` 之后被改写文本」也不会重读；被样式表或 a11y 引用到的 id 一律**保留原名**（安装期决定、不回收）⇒ 那些图标仍可能重复（保守优先不破坏引用；**该图标的缺陷可能因此继续复现**）；**反过来**：若某 id 已经被改名，之后才出现的 `<style>` 再引用它的**作者 id**，那条引用会悬空（改名前本可解析）——真实上游图标不用 svg 内嵌 `<style>`，当前无此形态；按 id 的 CSS **选择器**（`#clip0_…{…}`）不在保留面内（改名会让它失效，当前树无此用法）；② 外部定义副本走传递闭包但**有预算**（≤8 份、深度 ≤2），副本内部**全部** id 都换进本 svg 的命名空间（不会因复制再添同名定义），且只复制「永不直接渲染」的定义元素：`<use>` 指向可渲染元素（`<g>`/`<path>`）时保持原样、绝不复制（避免重复绘制）⇒ 该引用仍按文档解析，可能落到隐藏壳里的同名定义（当前无此形态）；超出预算/深度的引用仍解析到定义者（不悬空，但不自足——可能仍受隐藏壳影响）；③ 直接改资源属性（不改 childList）不触发重扫；根 `<svg>` 自身的 id 豁免不改（外部代码/CSS 更可能按它寻址，当前上游图标不起根 id）；④ gateway/mobile 独立部署的官方壳由实例自带 bundle 渲染，本模块不覆盖（chamber 侧可把它装进既有打包插件 `packages/dsh-chamber-client-ui-mobile`，属范围决策，见 STATUS 开放项）；⑤ 模块记忆（改名表/保留集/已 scoped 身份/link 监听）按 realm 记账，生产是一 realm 一文档（disposer 会清空，测试用 `resetSvgResourceScopeMemory`；token 前缀登记刻意不随 disposer 清，避免重扫时 id 形状增长）；⑥ 与我们的 token 形态**同名**的既有 id（`chamber-csvg<数字>-…`）在改名时会被当作旧前缀剥掉（引用仍成对，只是丢了字面前缀；改名目标做同批去重，不会撞成重复 id）——当前树无此形态。
  - **裁决与落地（W8/R15③）**：范围决策取「把 scoper 装进既有打包插件 `packages/dsh-chamber-client-ui-mobile`」（import 自 `packages/renderer/src/svg-resource-scope.ts`，单一实现不复制），`lib/client.js`（构建期生成、不提交；clean checkout 由 `pnpm run build:artifacts` 自举）由 `scripts/artifact-scope-marker.test.mjs` 守卫（缺标记/缺文件即红，含负控），CI 在 `build:renderer` 之后由 `packages/dsh-chamber-client-ui-mobile/scripts/assert-scoper-artifact.mjs` 断言 `desktop/dist/web/assets/*.js` 携带 scoper 代码；真机验收（`svg-resource-probe.mjs --expect-artifact`）仍为发布门。
- **被拒方案**（用户裁定「不改上游，chamber 内修」）：① 改上游 primitives（删三处矩形=viewBox 的无用 clip、真实 clip/mask 改 `useId`）——本轮明确不动上游，且只能覆盖已知图标，未来任意图标/第三方插件仍会重复；② 文档首部放「id 锚」渲染一份带 defs 的图标——实测无效：锚在重复子树**之前**时新图标照样空白（WebKit 解析取最后注册的定义），把锚反复追加到末尾则是与插入竞态；③ 隐藏壳改用 `display:none` 换取资源可解析——实测可避开触发，但丢掉 §4 依赖的渲染缓存（切回变慢）且只是绕开；④ 事后自愈（属性回写/重建资源型 svg 节点）——属性回写实测无效，重建只会掩盖回归；⑤ 按实例壳命名空间（而非按 `<svg>`）——省改名量，但漏掉**同一壳内**的重复（rail/wide 变体、面板行 + 座席），而按 `<svg>` 正好是自足的最小单位。
## 5. 连接设备页
...
- chamber 自研插件包 `packages/dsh-chamber-client-ui-settings-plugin-manager`：复用官方插件管理页面、控制器与配置 ledger，作为 `settings.plugins.tab` occupant 注册在「设置 → 内置插件」；不提供侧栏 panel。
  - **Rejected alternatives（容纳层，2026-09）**：上游 `PluginManagerPage` 是主面板整页——`align-items:center` 的居中列、`clamp(24px,4vw,48px)` 侧内边距、`28px + 窗口顶带` 的两条头部行，外加自有 h1+intro；官方兄弟 tab `PluginInventorySettingsTab` 的 `.section`（`width:100%` / `max-width:760px` / 无内边距、无自有标题）才是 tab 体契约。chamber 以 `EmbeddedPluginManagerPage.{tsx,module.css}` 包装（只转发 slot props），按上游自有钩子 `data-plugin-panel`/`data-window-drag` 把根覆盖为 tab 体并隐去重复标题（工具栏行保留）。被拒：① fork `ui-plugin-manager`（页面 63KB + CSS 28KB + controller/store，复制面远超收益，违背本包「复用官方页面」的裁决）；② 改 vendor 源或请上游加 embed 变体（本轮不动上游；提案归 `docs/progress/todo/upstream-proposals.md`）；③ 在 settings-bridge 里对「插件管理」特判（壳不应认识某个分节的内容形态，跨包泄漏）；④ 摘掉 `data-window-drag`（上游 `ui-theme` app-region 清单把 `.pageHead`/`.detailTop` 定为 chrome 行，且 Electron `titleBarStyle:'hidden'` 下它是弹窗打开时唯一的窗口拖拽面）。脆弱面（上游改属性/改结构即静默回归）按 `docs/checklists/upstream-touchpoints.md` §3 登记、§7 复核，宽容层偏差见 STATUS。
- 「dsh 运行时」段（design 18 §3.6/§9，per-server）：chamber 自研 `settings.section`（id `dsh-runtime`，order 31，由设置壳 settings-bridge 的 `registerRuntimeSection` 注册）挂在**该来源自己 boot ctx 的 ledger**（完整桥接修订；随 `chamberBridge` roster 投影 reconcile），紧随 agent-presets 渲染。local = 完整运行时管理面；gateway = 经反代触达该 gateway 的 `/chamber/runtime`；**dsh 直连（ssh/http）= 不挂载**（无 `/chamber` 面、无 ssh exec 管理通道，不渲染该分节；18 §3.6 / AGENTS / design 17 §3 同口径），也不再位于 chamber 全局「客户端」视图（design 15 的 `__general` 控制组不含运行时块）。
- **「重启 dsh」动作只在本地与 gateway 两源**（design 18 §3.6 项 8，刷新插件挂载）：local = 控制面事务接口 `restartLocal()`（design 18 §9.3，与健康状态机重启单飞行串行化——**不是**连接页裸启动/停止的组合）；gateway = `POST /chamber/runtime/restart`（经 `/api/i/gateway-<id>/chamber/*` 反代，202 + status 轮询）+ `POST /chamber/runtime/start`（design 21 决策 12 停机恢复：stopped/error/restart-exhausted，卡片「启动实例」入口）。二次确认 + 状态行，与健康状态机 `restarting` 单飞行互斥、applying 期间禁用。**确认门只有一个**（upstream-alignment T2；accept 复验与时限见 review-fix F2/F4b）：本地重启与**全部** gateway 变更动作共用应用内官方 `Modal`(`RuntimeConfirmDialog` + 纯机器 `confirm-machine.ts`)——武装不跑任何东西，取消在 runner 调用前丢请求，**accept 先按 live 事实复验**（每个 request 带 `stillValid`，与武装同一谓词、读每次渲染重写的 `liveFacts`；失败即 `outcome:'dropped'`，由本段错误行如实报「已过期」而非静默，因对话框跨过 `~3s` 状态轮询）；**gateway 侧动作另有 12 分钟墙钟上限**（`REMOTE_ACTION_TIMEOUT_MS = REMOTE_STATUS_POLL_TIMEOUT_MS`(11 min) + 60s，与轮询预算同源），否则 pending 期忽略取消/Escape/遮罩的对话框会被永不 settle 的动作锁死；**本地腿（`restartLocal()` 经 IPC 到主进程事务）没有 abort 句柄**，缺口登记在 STATUS。ssh（dsh 直连）的 systemd `restart_service` 属**连接管理面**（重启 gateway/dsh 服务本身，非插件模型动词，design 21 §3 目标语境差异登记）；dsh（ssh/http）直连不挂载 dsh-runtime 段、无插件模型重启动作（http 直连来源无任何重启动作，design 17 §3）。
- **写者静默通知与「清理并接管」（02 §3.4 / 04 §3.2）**：本地卡在 `GET /api/connections/local/writers` 报非静默时，卡片内点名阻塞写者（`pid <n> · <原因>`，附机器 token）并给「清理并接管」按钮（`POST …/reclaim`）：清除**本状态目录自己的**陈旧/孤儿托管写者记录后启动本地实例；owner 仍活的另一应用实例永不被影响（控制面拒 409，原因带 `detail` 返回、卡片原样呈现）。写入期终止失败（`sticky`）只提示重启应用、不给按钮（无证据可依）。判定是纯函数 `writer-diagnosis.ts`（有单测），卡片只渲染。
- **职责划分**：连接页本地卡的 启动/停止 = 连接生命周期（开机常驻与否）；「dsh 运行时」段的 重启 dsh = 运行时维护（刷新插件挂载、恢复服务）——两者不合并、文案不混用（起停不改运行时事实，重启不改指针/版本）。
- **每来源设置面 = 该来源自己的设置面（权威口径 完整桥接修订）**：设置壳不再装配「缩小版前端」，而是**渲染该来源自己 boot ctx 的 `settings.section` 台账**，条目用**该 ctx 渲染器绑定的标准座**渲染（实现见 `packages/dsh-chamber-client-ui-settings-bridge/src/client/`）：
  ...
  - **归因不上面**：`StoredEntry.registrant`（cordis fiber-name 戳）**不渲染**——上游官方壳也只渲染 `navIcon(row.id)` + 分节标签，该戳在上游是纯诊断字段。本仓因此退役曾经的「插件」来源标记（用户拍板）：插件提供的分节与官方分节在 nav 上完全同形。
...
注册表现有四行 client-graph / git-worktree / archive-cleanup / open-in，其中 open-in 标 `localOnly`：**该行只列在本地目标**，非本地目标的行集 = 该目标适用行（local 4 行 / ssh·gateway·http 3 行；裁决，详见 design 21 §6.6）；另有 gateway 源才出现的
  移动端 client 行，随发行物注入）→ 第三方插件区（已安装列表 + 逐行卸载 + 添加：spec 输入 + npm 搜索 +
文件夹导入；**行集 = 该目标 profile 的依赖表**（local/ssh/gateway；http 直连没有 `rows` 面，只有 Loader 清单分类）——安装自带组合与 chamber 播种物不在此列，它们分别属于运行时基线与上面的 chamber 内建组件表，详见 design 21 §6.11.5 的 行集修订))
...
- 自研插件包（`@dsh-chamber/*` 前缀，替换/扩展官方插件注册）：
  - `packages/dsh-chamber-client-ui-sidebar/`——**自研侧边栏插件**（包名 `@dsh-chamber/dsh-chamber-client-ui-sidebar`，照官方 ui-sidebar 结构改造：保留几何/折叠/孔位声明，会话区改多来源统一列表）+ `packages/dsh-chamber-client-core/src/aggregate-store.ts`(chamberBridge)+ `packages/dsh-chamber-client-core/src/instance-api.ts`（每实例 unary 客户端，App 层与插件共享一份，vite 共享 chunk）；
  ...
  - `packages/dsh-chamber-client-ui-settings-plugin-manager/`——内置插件分页：在 `settings.plugins.tab` 注册上游完整管理器，不接回 `sidebar.panellist`，设置入口与会话导航分离。
...
- 前端入口复用 `packages/renderer/`：vite 构建时把 workspace 包 alias 到源码；`chamber-entry.ts` 复合 entry 挂整棵 dsh 客户端树（connection→typert→
  ...
  **open-in 插件**→settings cluster（general/models/plugins/inventory/chamber plugin manager）→conversation→…→全量 ui-*）。
- **启动图清单 = 单 entry + 每实例宿主图额外 entry（设计 09）**：
  ...
  - **每实例宿主图额外 entry（设计 09 落地）**：boot 时经反代（`/api/i/<id>`）调 chamber host 包 `@dsh-chamber/dsh-chamber-seed-client-graph` 的 Remote `clientGraph/graph` 取该实例宿主组合的客户端插件 boot 图，按 `CHAMBER_COVERED_IDS`（`packages/renderer/src/chamber-covered.ts`：复合已覆盖 + 页面自有 id）去重，预加载剩余 bundle（`/api/i/<id>/plugins/<pkg>/client.js?rev=…`），经 boot.ts `extraRows` seam 合并进 boot rows（详见设计 09）。
  - **host 包与 seed（设计 08/09/20/24）**：`packages/dsh-chamber-seed-client-graph`、`packages/dsh-chamber-seed-git-worktree`、`packages/dsh-chamber-seed-archive-cleanup` 与 `packages/dsh-chamber-seed-open-in`（后者 `localOnly`：只 seed 进本地 profile，不进远端 seed、不随 gateway 上传，插件页仅在本地目标列出该行；非本地目标的行集 = 该目标适用行，design 20 §6）的 esbuild `dist/index.js`（`@deepseek-ai/*` external）为构建期生成、不提交，clean checkout 由 `pnpm run build:artifacts` 自举；控制面 `host-graph-seed.ts` 幂等 seed 所有已构建包进 `$DSH_HOME/profiles/web/node_modules/@dsh-chamber/*/`，把 `client-graph` / `git-worktree` / `archive-cleanup` / `open-in` insert 合并到单一 `<stateDir>/dsh-chamber-graph.patch.yml`。每次 spawn 注入同一 `--patch`（`webProfileArgs(port, patchPath?)`）；任一产物缺失只跳过对应行，不产生悬空 insert。远程 ready-time seed 同样一次探测/一次 overlay 合并写，见设计 13。

**Rejected alternatives（构建产物移出 git）**：继续提交产物可以保住「clean checkout 零构建即可 typecheck/static/test/打包」的便利；代价是仓库跟踪约 1.57 万行生成物（四个 seed `dist/index.js`、`dsh-runtime/dist/index.js` 与 mobile `dist`/`lib`），每次重建都产生与源码改动不成比例的 diff 噪音（长 bundle 行与二进制级 churn），也让「产物是否陈旧」的审查面模糊。选择移出：`.gitignore` 不再反选这些产物，clean checkout 首次验证前由 `pnpm run build:artifacts` 自举（`run-checks` 与相关包测试自动前置），产物新鲜度继续由 C8 重建-比对盯住。
...
- `dsh-chamber:info`；`desktop_ssh_instances_get`（spec v2：kind/transport/insecureHttp、sshPort、serviceName 与 remoteDshHome，03 §2.2）、`desktop_ssh_instances_health`（只读 invoke；返回注册表**加载**健康位终态 `{degraded: boolean, reason?, rosterIncomplete: boolean, droppedCount?}`——`reason` 为非秘密失败文本、仅 degraded 时出现，`droppedCount` 仅随 `rosterIncomplete:true` 出现，旧生产者缺 `rosterIncomplete`/`droppedCount` 时 renderer 按 false/0 读。三态：degraded `{degraded:true, reason, rosterIncomplete}`（旧 `{degraded, reason?}` 面的扩展）、健康完整 `{degraded:false, rosterIncomplete:false}`、行级丢弃 `{degraded:false, rosterIncomplete:true, droppedCount:N}`。与 `instances_get` 正交：degraded 期间空数组 roster 不是权威；rosterIncomplete 期间合法行照常安装/可见，但该维与 degraded **同档**否决 durable 剪枝门（19 §3.2.4）；成功加载或 **authoritative** `saveInstances` 原子重建后清零，`compensation` 回滚绝不清零，见下条 F13/F16/F19/V5-A 契约）、`desktop_ssh_save_connection`（元数据 + SSH password + gateway token/password 的 crash-safe 主进程原子/补偿事务；write-only 旧值仅主进程快照，renderer 不可读）、`desktop_ssh_delete_connection(id)`（精确 id-addressed 主进程删除事务；不存在 id 即幂等 no-op）；三个单项 credential setter 仅接受显式 clear；add/edit/delete/非空凭据写不得绕过各自主进程事务；`desktop_ssh_config_list`（`~/.ssh/config` 非秘密投影）、`desktop_ssh_connect/disconnect/status/logs/logs_clear`、`desktop_ssh_start_service/stop_service/is_active/restart_service`（固定参数数组 `systemctl <action> -- <serviceName>`，serviceName 白名单 `^[a-zA-Z0-9][a-zA-Z0-9_.-]*$`、首字符必须为字母或数字）；
- **注册表健康位与写 provenance（F13/F16/F19）**：`saveInstances(next, provenance?)` 的 `provenance` 取 `'authoritative' | 'compensation'`（缺省 `'authoritative'`）——`authoritative` = 真实用户动作（add/edit/delete/导入）提交的新注册表真相，成功即重建 live 文件并清零加载降级位；`compensation` = 事务失败后的回滚重写，只恢复主进程事务前快照，**不是**关于 live 文件内容的新事实（降级启动时 `before` 是内存空集，不是未知的磁盘内容），**绝不清零**降级位。**F19 重启残留闭合**：降级态（`registryLoadFailure !== null`）下的 `compensation` 写**跳过落盘**（live 文件保持缺失/原状——一次「成功的空快照写盘」会让下次启动把未知读成权威空注册表，renderer 随即放行 durable 剪枝），内存仍接受入参以维持 `connection-save` 的「返回列表 == 事务前快照」rollback 判定，跳过时 warn；`authoritative` 与非降级 `compensation` 的落盘行为不变。于是**下一次启动**因 live 缺失 + `.corrupt` 副本仍在仍判 degraded（不是健康空注册表），只有一次成功的 `authoritative` 保存写回 live 文件并治愈（回归：`packages/desktop/test/transport/transport-registry-health.test.ts` 的 F16/A4+F19 与 F19 regression 用例）。**roster 行级丢弃（V5-A）**：一次**成功**的 `loadInstances` 在合法 JSON 数组内丢弃条目时置 `registryIncomplete`/`droppedCount`——成因三类：`validateSpec` 拒绝、null/非对象（或数组）行、重复 id（首胜丢弃）；合法行照常进入内存 roster（连接可见、renderer 照常结算），但磁盘文件含本进程无法表示的内容，roster 只是**部分权威**。健康通道报 `rosterIncomplete:true` 并附 `droppedCount`；renderer 侧该维与 degraded **同档**优先否决 durable 剪枝门（合法行可见、门关死），否则被丢弃来源的 durable 键会被当退役来源删除。**V5-A 落盘纪律**：incomplete 期间 `compensation` 回滚同样**跳过落盘**（把「丢弃后的部分快照」写回磁盘会让下次启动把「部分」读成「完整」并重新放行剪枝），只做内存回滚以维持 `connection-save` 的 rollback 判定，跳过时 warn；一次**无丢弃**的成功 load 或 **authoritative** 保存原子重建后才清零。renderer 按 `rosterIncompleteDiagnostic(droppedCount)` 单源文案 warn-once（与 degraded、health-unavailable 三角可区分）。**健康探针不可用**（旧 preload 缺 `instances_health` 方法 / invoke 抛错）同样 **fail-closed**：renderer 不安装 roster、不置 `remoteRosterSettled`、也不把不可用折叠成 degraded 位，按 `healthProbeUnavailableDiagnostic` 给出与 degraded / 无桥可区分的 warn-once 诊断（F16/A4；19 §3.2.4）。**计数口径**（以 `bridge-manifest.json` 与 preload 为准）：manifest 61 invoke（含顶层 `dsh-chamber:info`）+ 8 push = 69；preload 命名空间成员口径 = 60 invoke + 8 push；其中 `desktopSsh` 面 32 invoke（含 `instances_health`）。
...
- registry 编辑以 transport + exec generation 隔离旧异步工作：`serviceName` 与 `remoteDshHome` 同属 live transport fields 与 exec identity（字段集单源 = `packages/desktop/credential-identity.ts` 的 `liveTransportIdentityChanged`/`sshCredentialEndpointChanged`/`gatewayCredentialTargetChanged`），变化时先提升 generation/`execEpoch`，撤销旧隧道/直连尝试与所有 exec child（SIGTERM→SIGKILL），
