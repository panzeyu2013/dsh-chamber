...
真机问题：远程来源未就绪时点开会话，活动视图停在全屏遮罩上。遮罩盖住整个窗口，
而多来源导航（侧栏）在**壳内部**、App 级失败覆盖层（见本节上文「失败呈现」）只在
已 settle 的失败时出现——于是"boot 不 settle"这段窗口曾经没有任何导航：唯一的自动
兜底是收割的绝对放弃臂（`HARVEST_ABANDON_MS` = 135s，只对未 settle 的挂载生效），
在此之前用户只能整页刷新（两 flavor 的默认菜单都有 Reload）。本轮把这段死区收敛成
五条契约（实现：`renderer/src/source-readiness.ts` 纯决策 + `App.tsx` /
`components/InstanceView.tsx` 接线，另涉 `host-graph.ts` 的通道失败上浮、`locales.ts`
的新键与 `styles.css` 的遮罩动作样式）：

- **遮罩可操作化（W1）**：遮罩在反馈窗（`VEIL_ACTIONS_AFTER_MS` = 10s）后从"纯转圈"
  升级为可操作态——已等待时长 +「重试」+「切换到其他服务器」行（与失败覆盖层的
  `.fatal-servers` 同款、chamber 级、不依赖任何 shell 挂载）+ 一句 ⌘R/Ctrl+R 兜底
  提示。它**不是失败声明**：失败呈现仍由覆盖层专有（结构互斥——`error !== null` 才画
  覆盖层，遮罩只在未 settle 时给动作），慢 boot 因此不会被谎报成失败。动作出现时
  `aria-busy` 同步翻假（"纯忙"区域会把更新播报压后，正好盖住要用户看见的动作），
  动作行本身 `role="status"` 播报一次；队列事实（见下条）不受反馈窗门约束，一成立就出现。
- **返回/切换的顺序（W4）**：遮罩的「切换来源」只登记"用户放弃"意图并切视图，**等
  切换落地**再由 App 回收该视图（`reclaimView` 拒绝活动/待开视图，`selectView` 的
  apply 又经 View Transition 单槽队列可能延迟）。顺序反过来会拆掉用户正在看的壳；
  先拆后切则会在切换失败时留下没有内容的窗口。已知取舍：从未注册成功、永不 settle
  的 boot 其 ctx/容器仍只能等它自己 settle 时才被拆除（本节既有结论），本轮不改变该
  边界——被拆的是**视图**与挂载位。
- **相位感知的就绪门（W2）**：`waitForServing` 仍以 `BOOT_TIMEOUT_MS` 为绝对上界，
  但**终态相位持续 `SERVING_TERMINAL_GRACE_MS`(1.5s) 后立即判"不可服务"**，不再烧满 60s
  （宽限是给点击触发的即时重连留出把相位翻到 `connecting` 的机会，避免把正在恢复的来源
  误报成未连接）。终态词汇 = `error`（快速重试耗尽）+ `stopped`/`restart-exhausted`
  （托管 dsh 停机；门读到的是**合并后**的 `phase`，本地实例的托管停机同样落在这一词汇，
  SSH 形态不会出现这两相），与 sidebar 姊妹门 `shared/serving-gate.ts` 的
  ...
  事后冷重挂"。`idle`（手动断开）**不启动 boot**：
  遮罩直接呈现「未连接」+「连接」（显式用户意图，与设置页 Connect 同语义——idle 的
  "不被自动触碰"纪律正是靠这一点守恒）；相位离开 idle 后按正常路径 boot。**被推迟的
  视图从不参与放弃臂计时**（它从未起过 boot），但**必须回收**：后台挂载（设置面板选
  来源）不经过 active 变化臂，所以它在**挂载时刻**起隐藏计时，过保留宽限（60s）由
  推迟回收臂拆除——否则一个离线来源的挂载会永久占住挂载位、并让"隐藏壳数"把用户的
  温壳挤出保留位（2026-12 复核 F1）。裁决的边界（`isDeferredReclaimDue`）：**只接管
  从未 settle 的推迟挂载**（已 settle 的推迟壳是真壳，回 retention 常规候选窗），且
  **设置面板正在编辑的来源绝不回收**（design 05 §5 的面板 hold，缺这道守卫会把面板钉死
  在"正在启动该实例的前端"）；活动/待开视图同样不回收。来源被手动断开前就已经起过的
  在途 boot 仍由放弃臂看管（起表只跳过、绝不删除）。
- **重试的诚实（W4）**：同 id 的新 boot 要先等同 id boot 尾（绝对上限两个 boot 预算，
  `INSTANCE_TAIL_WAIT_CAP_MS`），静默点击会被用户当成"按钮没反应"。遮罩只在**前一次
  尝试尚未 settle**（才会真的排队）时播报"排队 + 上限"——以失败 settle 后同 id 尾已
  释放，按"第几次尝试"播报就会在最常见的"失败后重试"里撒谎（2026-12 复核 F3）。
- **通道失败进入降级事实（W3）**：图通道失败（非 `not-injected`：502/504、网络错误、
  图形非法等）现在同样上浮 `ShellState.degraded`（kind `graph-unavailable`），由 App 的
  非阻断 `.boot-gap` 横幅给出解释，并沿用该 kind 的「每个 ready 世代自动重挂一次」。
  旧契约只把这一事实放进连接页的 `pluginDiagnostic` 一行（侧栏早已不渲染该诊断）：
  事实并没有"零出口"，但用户停在 boot 表面时既看不到解释、也拿不到自愈——W3 的增益
  是 **boot 表面的可见性 + 自愈**，不是"从零到一"（2026-12 复核 M3 更正了口径）。
  豁免 = `not-injected`（HTTP 404，或通道答 method 缺失）：gateway/mobile 形态合法地
  没有图端点，不是降级。

**Rejected alternatives（本轮权衡）**：⓪ 以"进度信号"判前台停滞（帧/心跳/rAF 计数）——
死区里没有任何可信进度源（官方启动页不发心跳，图请求可能合法地慢），错判的代价是把健康
启动判死，与本轮"只呈现事实、不猜"的纪律冲突；① 取消在途 boot（让重试真正打断前代）——
根因层最彻底，但要动 `shell.ts` 的 boot 世代/teardown 契约（同 id 尾等待、一容器一 root
不变量、ctx 只在 settle 时拆除），成本与风险都超出本轮收敛窗，登记为后续项；② 只下调
`HARVEST_ABANDON_MS`——它是收割账本的停用/释放槽位判据（退避是另一个常量
`HARVEST_RETRY_BACKOFF_MS`），且完全不解决"死区内零导航"；③ 把来源切换器搬到壳外
（全局 chrome）——根因层最彻底，但推翻 §1 的 N-ctx 与
"导航在壳内"契约，成本远超收益；④ 60s 墙钟硬判失败——仅"graph 4.5s（9×500ms 重试预算）
+ 60s serving 门"就已超过它，**必然误杀合法的慢启动**（其上还有 30s bundle 加载与 15s
mount 上界），正是本节"截止/放弃值必须高于 boot 预算"要防的形态；⑤ 复用既有
`.fatal-overlay` / `markAbandonedShellFailed`，在反馈窗直接判失败——实现最省，但把"慢"
谎报成"失败"（且放弃臂会按新一次尝试重新起算预算），违反失败呈现的"不撒谎"纪律；
⑥ 让遮罩点击穿透以露出壳内侧栏——死区里根本没有已 boot 的壳（侧栏随壳 boot），穿透只
会把点击送给被覆盖的启动页；⑦ 新增原生桥面事实让 Swift 侧监督 boot——代价是 A 桥通道
+ manifest 生成 + shim/ipc 锁步测试，而它唯一能覆盖的残余场景（页面 JS 主线程冻结）
**并未被既有看门狗完整覆盖**（看门狗需 15s 无输入 + 3 次探测，用户点按钮即重置；且
`didFinish` 前完全未武装，而窗口此时已呈现）。该残余因此不靠新桥面解决，改为登记为
原生侧开放项（见 STATUS），本轮的页面级逃生已覆盖用户面。
...
- **每来源设置面 = 该来源自己的设置面（权威口径，2026-12 完整桥接修订）**：
  ...
  - **来源必须在挂载中**：面由该来源自己的壳发布，所以面板打开期间 App 层保证该来源
    ...
    未挂载则后台挂载（**不切 active view**），已挂载则**排除出一切回收路径**——不只
    保留策略候选，唯一拆除入口 `reclaimView` 与推迟回收臂都拒收该来源（否则隐藏 60s、
    135s 放弃臂、收割失败/放弃任一臂拆壳都会让正在编辑的设置面消失）；面板关闭即撤除。
    来源退役的卸载走注册表删除臂（不经 `reclaimView`），所以这条 hold 不会拦住退役。
