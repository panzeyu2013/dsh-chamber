...
- **顶带（`--dsh-frame-top-clearance` / 76px 拖拽带 / `data-animating`）不采用**（2026-12 裁决，
  ...
  字标贴在红绿灯下沿）。该带与 logo 行同时按上游打 `data-window-drag`（窗口 chrome 行标记，必带清单见 ui-theme 的 `CHROME_ROWS`）：Swift 壳据此把它们当窗口拖动面（design 25 §5.5），Electron 腿的 `base.css` app-region 规则读同一批标记。
