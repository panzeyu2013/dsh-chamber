...
| goal 活性事实 | 运行时事实行 `goal?: GoalFact \| null`（三值：缺席 = unknown / `null` = 明确无 goal / 对象 = 有 goal；`{goalId, revision, phase, activation?, updatedAt?}`）。P1 来自挂载壳的会话投影 `projectionValues.goal`，P2a/P2b 来自只读 `session/list` 投影 + `$events` 的 activation 事件 | `packages/dsh-chamber-client-ui-sidebar/src/shared/derive.ts`、`packages/control-plane/src/session-state-protocol.ts`（06 §4.1、17 §10.7） |
...
- subagent 会话不产生事件（事实通道不含 subagent 行；父会话的 `runningSubagents`
  只驱动子代理计数徽标）。
- **2026-12 单入口收口（P3 移交 + goal-aware v5 落地）**：上表与 `notification-edges.ts`
  的纯函数仍是**事实输入**，但生产裁决的唯一入口已是
  `packages/renderer/src/notification-projection.ts` 的 `reconcile(state, observation)`：
  `packages/renderer/src/completion-observation.ts` 把壳运行时 report 与 facts 快照两条
  独立到达的通道按 §3.2.1/§3.2.3 的权威规则合并成每 (source, session) 一份可序列化观测，
  `reconcile` 在 `complete-ledger.ts` 的账本状态（三张 durable 表 + 五项易失状态：
  `armed`/`armedFloor`/`settleFence`/`generation`/`goalKnown`，见 §3.2.3）上就地收敛
  （水位吸收、一次性标题、settleFence）；`App.tsx`（facts 轨）与 `use-bridge-subscriptions.ts`（壳 report 轨）都
  只经 `observeSource` + `applyObservationBatch` 进入（接线锁
  `test/wiring/session-authority-wiring.test.ts`），通知组装与 IPC 仍是 App 的单一
  `emitSessionNotification` 出口。`planRuntimeNotifications` / `planFactsNotifications`
  只保留导出面给既有语义用例（生产调用点已全部迁走）；通知路径的 `usableFacts` 抑制分支
  与 App 内的第二完成循环已删除。**勿混同**：`App.tsx` 仍保留同名局部量 `usableFacts`
  （`recomputeSourceUnread` 内，只服务未读派生并经 `factsVerified` 闸门），它不是被删除
  的那条分支，也不参与通知裁决。

#### 3.2.1 goal 三值事实与两个门（P1/P2a/P2b）

运行时事实行新增 `goal?: GoalFact | null`（`packages/dsh-chamber-client-ui-sidebar/src/shared/session-row-state.ts` 是唯一类型家）：
...
- **解析（P1，挂载壳）**：`shared/derive.ts` 的 `parseGoalFact` 读会话行
...
- **两个门（刻意不同条件）**：零依赖叶模块 `shared/session-row-state.ts` 是**呈现门**的
...
转移记忆（`shellRunning`/`shellCompleted`/`shellPending`）承担。running 权威：同代
...
- **通知**：唯一组装/发送出口仍是 App 的 `emitSessionNotification`（`kind = 'complete'`），
  ...
  **刻意分叉**：goal 转 `paused` 后呈现门不再压制（完成未读出现）而通知按 #3 drop——呈现
  与通知的门本来就不是同一个。
- **六面单源**：`shared/session-row-state.ts` 的 `sessionRowState` 是唯一派生源，六面
...
- ⑥ **跨批围栏重复（V5-B 已登记边界，2026-12）**：触发链——facts 不可用期间壳边沿
  ...
  `[sourceId, sourceFingerprint, sessionId, kind, watermark ?? null]`，首条壳边沿
  （无水位 ⇒ `null`）与重复的 facts 水位 100 键不同，因此不被其吞掉。**为何不做跨批
...
- ⑦ **facts 输入缺席（纯壳批）不触发复位补偿（F30 登记，开放边界）**：触发链——来源
  ...
  （壳 1 条 + facts 1 条，TOTAL=2；两条 claim 键一为 `watermark ?? null` 的 `null`、一为
  实际水位，主进程 5s 去重不吞）。与 ⑥ 的差别：⑥ 的栏被跨批 running 清掉，本项的栏还在，
...
- **身份与去重键**：主进程 claim 键 = **五元组** `[sourceId, sourceFingerprint,
  sessionId, kind, watermark ?? null]`（`packages/desktop/notifications.ts` 的
  `NotificationClaimWindow.claim`；`watermark` 缺省序列化为 `null`，即不含水位的四元组身份）
  ——`kind` 与 `sourceFingerprint` 必须保留，否则同一会话的
  ask/complete 互吞、或 same-id 换宿主继承旧 claim；旧调用方不带 `watermark` 时请求逐字节不变
  （镜像钳制见 `packages/renderer/src/global.d.ts`）。
...
- `maybeShowNativeNotification(payload)` 裁决链（设置权威在主进程内存，随
  ...
  6. **去重 claim**：key = 五元组
     `JSON.stringify([sourceId, sourceFingerprint, sessionId, kind, watermark ?? null])`
     （与本节「身份与去重键」同形；无水位壳事件序列化为 `null`，带水位的 facts 同事件
     键不同 ⇒ 仍各自发一条，同键重放才被吞），5s TTL、64 条硬上限，Map + 时间序列只
     清理过期前缀（防同一事件双路径/重放双发，claim 在裁决之后；'test' 不走 claim）；
...
两面永不分叉，vendor completed 兜底并入同一并集（`ledger || vendor completed`），断连窗口的呈现边界见「计数语义」）
...
- **呈现边界（既有窗口，徽标投影同一并集）**：窗口内完成点 = App 账本 ∪ vendor
  `completed` 兜底行（`mergeRuntimeFacts`），徽标投影**同一并集**——`badge-count.ts` 的
  `armed = ledger || row.completed`（`projectBadgeCount`）——vendor-only 完成的会话
