...
> **状态：现行（v1 侧边栏增强契约，2026-12）**——本设计定义 chamber 自研侧边栏的
...
- **状态（共享控制器）**：每来源搜索状态与防抖 job 整体
  ...
  DOM ref（outside-click 包含判定与 focus）。任一来源侧边栏发起的搜索在视图
  切换后仍存活（可见侧边栏换 shell，共享状态不换）；job 单一所有者，杜绝
  N 个 shell 对同一查询重复发起/互相中止（单来源击键
...
- **结果渲染**：query 非空时该来源的 `workspaceList` 整体替换为结果列表
  ...
  状态行：loading → `search.pending`；error → `search.unavailable` 横幅
   （**本地命中仍显示**——内容搜索失败不吞本地元数据命中；2026-09-11 文案补齐
   「仅显示名称匹配」）；
  ...
  **结果树可访问名**：`role="tree"` + `search.results.aria`
  ...
  （§4.3，2026-09-11 upstream-alignment T7）。
...
- **边界**：拖到折叠组无目标行（自然无 marker）；轮询刷新中途拖拽
  ...
  （官方亦然，注明已知限制）。
- **会话排序模式（对齐官方）**：每来源排序偏好 manual（默认）|
  updated（§3.1 `orderBy`，来源头 hover 操作簇排序按钮打开**显式菜单**——官方
  ViewOptionsMenu 模式，勾选标记当前模式）。**updated =
  手动序 + 活动置顶（官方 ui-workspace nextSessionOrderAccount 语义；不是
  "纯 recency 重排"）**：
  ...
  - updated 模式下会话拖拽只写 account 序（共享 view-prefs 持久化，不提交
    ...
    **有意的偏差（对齐声明的边界）**：切回 manual 时
    ...
    （design 01 §5 权威边界纪律），故 updated 模式是**活动视图**而
    非持久的手动排布层。
  - 投影签名（`serversProjectionSignature`）**纳入会话 updatedAt**——会话活动
    时间戳变化会重发布投影，推导 effect 才能及时置顶（排序由 account 推导
    驱动，故该字段不可从签名排除）。
- **双击重命名**：workspace 头直接 dblclick 进入行内重命名
  （头本身不可点击，无延迟）；**会话行单击立即打开（零延迟，对齐
  OpenChamber 的 immediate-open 模型）**，双击重命名由
  ...
  **行内形态**：workspace 的重命名编辑框**嵌入 workspace
  ...
  **停冒泡控件的 pending 清理纪律**：
  ...
  - **blank 行 ghost 槽（双击误中修复）**：双击空白行下方的真实会话时，
    ...
    已安全越过双击窗口。
  - **跨 shell 滚动锚点同步（`renderer/src/sidebar-scroll-sync.ts`，
...
- **来源级收拢（server 折叠）**：来源头左侧新增折叠开关（与 workspace 头
  同款槽位：常态 **MONITOR 电脑字形**（用户反馈定稿：原 folder 字形与
...
- **来源拖拽排序（显示序偏好）**：来源头为拖柄（HTML5 DnD，镜像 §2.2
  ...
  rail 来源按钮同序渲染（§7；2026-09-11 起 rail 每来源一个具名可操作按钮，
  颜色点与活动环的几何不变）。来源从注册表删除后其 id 由写时裁剪清出（与
  orderBy 同规则）。**取消即放弃**：dragend 时
  `dropEffect === 'none'`（ESC 取消）不提交最后 marker——§2.2"drop/end
  提交最后 marker"在来源级收窄为"仅非取消的结束提交"（取消即放弃）。
  另有两条同向收窄的判据：① dragend 时 `dataTransfer` 为
...
- 单键 `dsh-chamber.sidebar.v1`（整页共享 localStorage；所有实例 ctx 共读
  ...
  { v: 1,
    ...
    serverOrder?: string[] }
  ```
- **sourceFolded / serverOrder**：
  来源级收拢 + 来源显示序（见 §2.4）。均为可选字段——旧数据无字段即视为
  全展开 / 投影序，v 保持 1 不重播种；裁剪规则同 orderBy（本会话见过、
  现已消失的来源才裁）。
- **updatedOrder / sessionUpdatedAtByAccount**：updated 排序
...
- **sidebarWidth（ui-layout fork）**：`packages/dsh-chamber-client-ui-layout`
  ...
  ui-layout 注册（见 05 §6）。
- **orderBy**：每来源会话排序偏好 `'manual' | 'updated'`，
  默认 `manual`；v 保持 1 兼容旧数据（旧数据无此键即视为全 manual，不重
  播种），sanitize 丢弃非法值。**默认值 `manual`**（保持既有 wire 序呈现），
  与官方默认 `updated` 不同——有意取舍：多来源列表下 wire 序即用户/宿主排好
  的序。该默认值不因已实现官方活动提升（promotion）语义而改变：updated =
  手动序 + 活动置顶（§2），但 wire 序仍是默认第一。
- `shared/view-prefs.ts`：`loadViewPrefs()`/`saveViewPrefs(prefs)`，
  JSON 解析/写入 try/catch 兜底（非致命）、版本号不匹配即弃用重播种
  （官方 persist 引擎纪律）；纯函数，可单测。
- **共享实时存储（跨 ctx 实时联动）**：在读写函数之上新增
...
- **插件 = 投影**：上报端只做
  ...
  `completed`/`pending`，除 design 24 §12 的**purged 墓碑抑制集 + 收敛链**
...
- **运行中子 agent 计数**：除 running/completed/pending
...
- 每 ctx 插件 apply 内先为该来源注册一代 runtime producer，再由 effect 订阅
  `ctx.sessions.list`；订阅后立即通过 producer `report` 上报一次当前快照
  （zustand subscribe 不即时触发），其后每次变更继续上报
  ...
  调用同一 producer 的 `clear`。注册时的单调 token 使旧 ctx 的迟到
...
- 行尾为**固定 10px 状态槽**（非常驻身份点——来源身份由来源头折叠字形
  ...
  - **活动定时任务标记（2026-09-11 upstream-alignment T7）**：行标题之后渲染官方
    ...
    未挂载来源的 unary 兜底行读 `projections.values` 的同一事实。无定时任务的
    行零足迹——行距/几何不变。
  - **待交互（pending）**：不与运行中同形——槽加宽至
...
- **悬停替换（真正替换，零占位）**：行/头操作在静止时 `display:none`（不占
  ...
  状态环 ↔ **kebab 菜单**（重命名/分叉/归档——2026-09-11 upstream-alignment
  T2a：归档动词移入行菜单，行内不再有第二个归档按钮，因为归档只隐藏行、从不
  触碰会话日志，故既不破坏性也无确认门）；来源头：连接状态 ↔ 排序菜单 + 搜索 +
  添加工作区（官方 project-add 字形，`IconProjectAddOutline16`）；
  workspace：会话数徽标 ↔ `+`（新建会话）+ kebab（重命名/删除））。胶囊展开/
  菜单展开时操作簇保持
...
- **不再显示相对时间**：session 行不渲染"xx 前"时间单元格（`time.*`
  ...
  保留为共享工具）。
  相对时间列**暂不回归**，维持移除（多来源密度 + 行尾状态槽
  取代时间列）；若未来回归需同步修订 05 §2.1 的文案。
- **当前会话高亮 = 全局单选**：`server.runtime?.current`
  命中即高亮，但仅限**拥有当前可见 ctx 的来源**（渲染侧
  `server.id === chamberInstanceId` 门控）——各来源壳内"切换前最后一个"
  会话不全部高亮，全局只有一个高亮（正在查看的那个会话）。
  （通道机制不变——组件不直连 store，
...
- `shared/aggregate-store.ts`（通道 + `ChamberServerAggregate.runtime?` +
  ...
  `projections.values` 的同一事实）、`shared/session-row-window.ts`
...
- **同族残留（非本节修复面）**：同一份文档里还有其它
  ...
  **唯一已收窄的一处是 open-in 的 chevron 菜单**（2026-09-11 upstream-alignment，
  design 20 §5）：菜单本体是官方 `ui-primitives` `Menu`（同样 portal 到 body），
  但 chamber 保留的 N-ctx 归属守卫 `instance-view-guard.ts` 在打开的菜单所属
  `.instance-view` 失活（`instance-hidden`/`instance-pending`/`hidden`/
  `aria-hidden`/断开）时立刻关闭它，因此隐藏视图的菜单不会随视图复活、也不会
  让击键落到隐藏视图上；SettingsShell 的 Modal 与其它官方 portal 面仍是上面的
  未修形态；
...
- **完成发生在来源 shell 首次观察之前仍无蓝点**：App 侧
  ...
  冲突）。
- **App 侧蓝点跨断连保留**：上报事实（runtimeFacts）断连即清
...
主检出/普通 workspace **34%**（低饱和高明的柔和色板，色相分布与家族
层级不变）；未分组桶无 accent 回退
  caption ink。
  来源首个 git 快照发布前 accent 一律不渲染
...
- **当前会话高亮（对齐官方 selected 处理）**：session 行 = 官方
  ...
  （来源 accent 不参与 workspace 级高亮）——当前指示不重复编码于 workspace 级，两组高亮永不相邻融合，
  色调全为官方 token 浅档。
- **打开意图在途 ⇒ 不投影"非请求中"的 current（2026-12 修订，05 §2.2.1）**：来源
  还有在途 `openSession`、且它当前投影的 `current` **不是**请求的那个会话时，不投影
  `runtimeFacts.current`（`projectableCurrent` 纯函数）。这是"切到远程会话先闪一行
  高亮的新会话"的侧栏半边：冷 boot 期间官方初始导航策略先给自己选中一个 blank 会话，
  `(!blank || current)` 规则随即把它渲染成高亮的 New Session 行，下一次分发
  （最多 400ms 后）又消失。**幂等重开保持高亮**（current 已是要打开的会话 ⇒ 投影本就
  正确，摘掉再装回是纯闪烁）；离开的活动来源的 blank 行仍由既有 ghost 槽（§2.2）
  保护列表位移。
- **回声工作区行（2026-12 修订，05 §2.2.1）**：新建工作区后侧栏立刻渲染一行
  "回声"工作区（真实宿主 id、`sessionIds: []`、标题 = 路径 basename，与合成组
  同一规则）；它**不带 `synthetic`**，因此重命名/删除/新建会话等 workspace 级
...
- **a11y**：来源分组 `role="group"`、列表 `role="tree"`（**浏览树带可访问名
  ...
  **行级动作的可访问名带行名（2026-09-11 upstream-alignment T5）**：新建会话
  `action.newSession.aria`、workspace kebab `action.menu.workspace`、会话 kebab
  `action.menu.session` 都以 `{name}` 参数带上行标题（上游
  `actions.newSession.aria`/`actions.workspace.aria`/`actions.session.aria`）——
  一排读作同一句「新建会话/更多操作」的名字对 AT 毫无信息。**折叠 rail 每来源一个
  具名可操作按钮**（T7）：`aria-label` = 来源名 + 激活提示（复用来源头的
  `sourceHeaderTitle`/`sourceHeaderActivatable` 单一判定）、当前来源
  `aria-current`、**非当前且不可激活**（托管停机等）的来源 `aria-disabled` 且点击
  不动作；颜色点与活动环仍是
  内层 span 的既有几何（点 8px、间距 12px 不变），点/环 `aria-hidden`。
- **blank 行**：当前空白"新会话"行隐藏操作簇（kebab——重命名/分叉/归档全在其内，
  对齐官方 `!row.blank &&` 门控）——空白行无重命名/分叉/归档语义。双击同样
  被 `blank` 门控（不进入内联重命名，见 §2.2）；离开
...
- **拖拽鲁棒性**：行内控件（kebab/`+`/折叠/搜索/添加工作区/来源头激活/rail 来源
  ...
  确认对话框/菜单/建会话/切视图）；`rowHalf` 对零高行防御；chamber 列表
...
共享持久偏好），跳转从不改写；方位感由条目上下文（tooltip 形态见 §8.5——完整标题 +
状态 · 来源 · 工作区）、多来源时的来源色点与既有 current 高亮提供。业界同构
（GitHub/Linear inbox：跳转与树可见性解耦，树状态归用户；显式 reveal 仅 IDE 有、不入 v1）。
