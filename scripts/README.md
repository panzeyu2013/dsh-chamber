...
脚本按**使用者与运行时机**分目录（整理）。新增脚本时先选类，再在根 `package.json`
...
| `gates/` | 每次 push 的门（本地 = CI 同源） | 单入口 runner `run-checks.mjs`（`--jobs N` 是“全局文件池”预算：tests/full 先 dump 展开全部包清单再统一调度，static/typecheck 的整门命令同池并发；池内按包轮转启动、输出仍按声明顺序；默认 min(8, 核数)，实测并发超过它后单文件耗时膨胀（同窗口 c12 总工作 217s vs c8 141s），吞吐收益递减，`--jobs` 仅用于特殊场景；诊断 `DSH_TEST_TIMING=1` 打 `[timing]` 每文件画像、`DSH_TEST_MANIFEST_DUMP=1` 展开单包清单）；`scripts/` 测试单入口 `run-script-tests.mjs`（分组清单 + 命名校验）；测试接线门 `verify-test-wiring.mjs`；文档门 `verify-i18n.mjs` / `verify-md-links.mjs`；设计 token 门 `verify-style-tokens.mjs`；包边界门 `verify-package-boundaries.mjs`（生产面禁跨包相对 import + exports 面白名单，自带 `--self-test`）；CI 变更判类 `classify-ci-changes.mjs`；workflow 门 `verify-workflow-{action-pins,yaml-scalars}.mjs`。 | `pnpm run check:static` / `check:tests`；新增门须同时进 `run-checks.mjs` 的 `STATIC_CHECKS` 与根 `package.json` |
