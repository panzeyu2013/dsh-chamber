...
function checkI18n() {
  ...
  const result = spawnSync(process.execPath, ['scripts/gates/verify-i18n.mjs'], {
    cwd: REPO_ROOT,
    encoding: 'utf8',
  })
  if (result.status !== 0) {
...
if (IS_MAIN) {
  if (flags.actionsOnly) {
    ...
    if (VERSION === undefined) {
      console.error('usage: node scripts/release/release-preflight.mjs <version> --versions-only')
      process.exit(2)
    }
    runVersionsOnly()
  } else {
    if (VERSION === undefined) {
      console.error('usage: node scripts/release/release-preflight.mjs <version> [--fork-version <v>] [--offline] [--skip-install]')
      console.error('       node scripts/release/release-preflight.mjs <version> --versions-only')
      console.error('       node scripts/release/release-preflight.mjs --actions-only')
      process.exit(2)
    }
    await runPreflight()
  }
