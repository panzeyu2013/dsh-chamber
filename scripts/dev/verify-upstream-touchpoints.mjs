...
const FORKS = [
  {
    ...
    patched: {
      ...
      'src/client/index.ts': '[patch-mod] apply(ctx) 读 ctx.chamberBasePath → 载波装配（design 05 §3.6）+ SYSTEM_RESUME_EVENT/liveness 触发（design 14 D4）+ 头部 chamber 说明',
    ...
    ownNotes: {
      ...
      'src/client/liveness-triggers.ts': 'chamber sleep/wake 活性触发（design 14；原生 reconnect + 离线门）',
