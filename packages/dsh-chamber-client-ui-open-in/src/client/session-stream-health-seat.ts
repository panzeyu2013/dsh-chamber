 ...
 * remounts, keyed by sessionId. Registered BEFORE the open-in gates: a source
 * whose open-in id does not parse still gets the recovery chip.
 */
    ...
    try {
      ...
      const plan = planSessionStreamHealth(
        ladders.get(sessionId) ?? createSessionStreamHealthState(),
        {
          ...
          // The automatic rebuild’s evidence: tri-state, and only explicit `false` unlocks it.
      ...
      if (plan.action === 'heal') {
        ...
        // The ONE automatic rebuild: the plan asked only after the concrete face
        // reported that NO open is in flight, so it interrupts no request. Accounted
        // whether or not the method performed anything, exactly like the heal.
