...
/**
 * One generic-RPC call against this entry's instance. The carrier adds the
 * per-instance base path (`/api/i/<id>`) and the connection's own trust
 * handling; `args` is the wire envelope's argument map (parameter names).
 */
 ...
 * @param options - the injected RPC carrier.
...
export function createLocalCatalog(options: LocalCatalogOptions): LocalCatalog {
  return {
    async load(): Promise<OpenInApp[]> {
      try {
        const carrier = carrierOf(await options.call(OPEN_IN_APP_APPS_METHOD, {}))
    ...
    async icon(appId: string): Promise<string | null> {
      try {
        const carrier = carrierOf(await options.call(OPEN_IN_APP_ICON_METHOD, { app: appId }))
    ...
    async launch(appId: string, path: string): Promise<void> {
      const carrier = carrierOf(await options.call(OPEN_IN_APP_OPEN_METHOD, { app: appId, path }))
