 ...
 * offers, and — while the plan arms it or runs its automatic rebuild — the
 * per-session stream rebuild. Nothing here acts on its own.
 */
    ...
    <div className={styles.chip} data-chamber-stream-health={face.marker}>
      ...
      {face.reload ? (
        <>
          ...
          {/* The per-session lever: rendered while the plan ARMS it or executes
              its own evidence-gated automatic rebuild; the click is the only path. */}
          {face.resync ? (
