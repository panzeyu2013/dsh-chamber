...
async function freshCoordinator(): Promise<typeof import('../src/shared/coordinator.ts')> {
  importNonce += 1
  // The `?test=` query busts the ESM module cache between tests; TS cannot
  // resolve the queried specifier, so the shape is pinned by the cast.
  return import(`../src/shared/coordinator.ts?test=${importNonce}`) as Promise<typeof import('../src/shared/coordinator.ts')>
}
...
test('window focus refreshes an empty capability list without per-button polling', async () => {
  const coordinator = await freshCoordinator()
  // Object holder instead of a bare let: the closure assignments below
  // would otherwise narrow the let to `never` at the read site (TS 7 quirk).
  const focusState: { listener: (() => void) | null } = { listener: null }
  let current: unknown = []
  let calls = 0
  setBridge(async () => {
    calls += 1
    ...
    addEventListener(type, listener) {
      if (type === 'focus') focusState.listener = listener
    },
    removeEventListener(type, listener) {
      if (type === 'focus' && focusState.listener === listener) focusState.listener = null
    },
  })
  ...
  assert.equal(calls, 2)
  unsubscribe()
  assert.equal(focusState.listener, null)
})
