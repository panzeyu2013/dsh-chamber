...
test('turnEnds counts each settled edge by classification (I16: one follow per edge)', async t => {
  ...
  const [s1Edge] = harness.store.applyStatus('s1', false, 20)
  harness.store.settleCompletion(s1Edge!, { at: 20, turnEnd: { kind: 'completed', cause: null, at: 20, seq: 1 }, unreadable: false })
  harness.store.applyStatus('s2', true, 10)
  const [s2Edge] = harness.store.applyStatus('s2', false, 20)
  harness.store.settleCompletion(s2Edge!, { at: 20, turnEnd: { kind: 'aborted', cause: 'user', at: 20, seq: 2 }, unreadable: false })
  harness.store.applyStatus('s3', true, 10)
  const [s3Edge] = harness.store.applyStatus('s3', false, 20)
  harness.store.settleCompletion(s3Edge!, { at: 20, turnEnd: { kind: 'blocked', cause: null, at: 20, seq: 3 }, unreadable: false })
  harness.store.applyStatus('s4', true, 10)
  const [s4Edge] = harness.store.applyStatus('s4', false, 20)
  harness.store.settleCompletion(s4Edge!, { at: 20, turnEnd: null, unreadable: true })
