...
export function createGatewayDispatch(auth: AuthProvider, getProxy: () => GatewayProxy, getFeatures: () => FeatureHost, getRuntime: () => RuntimeRoutes, logger: Logger, requestPolicy: GatewayRequestPolicy, auditFile?: string | null): GatewayDispatch {
  const middleware: GatewayDispatch['middleware'] = async (req, res, url) => {
    ...
    // The route EXISTS only while a password is configured (design 17 §6:
    // "仅 password 形态存在"). The Phase-1 facade always exposes `login`, so
    // the old `auth.login === undefined` branch is replaced by the dynamic
    // `kind` getter — a token-only (or none) deployment answers 404 here.
    if (pathname === '/auth/login') {
      ...
      if (req.method === 'POST') {
        ...
        try {
          ...
          if (code === 'no_password') json(res, 404, { error: 'not_found', code: 'not_found' })
          else if (code === 'rate_limited') json(res, 429, { error: 'too many login attempts', code: 'rate_limited' })
