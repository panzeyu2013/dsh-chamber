...
function validateSubmission(input: PluginTaskSubmitInput, deps: ChamberPluginTasksDeps): ValidationOutcome {
  ...
  const decide = (op: 'install' | 'remove', version: string | null): ValidationOutcome | null => {
    ...
    // (design 18), so the resolution below runs behind a guarded accessor: the
    // throw is logged and degrades the ladder, never escaped as a submit() 500.
    // One resolution per judgement (resolveJudgementInputs): the protected set
    // and the runtime version fact come from the SAME snapshot.
