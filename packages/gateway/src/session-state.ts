 ...
 * `goal/activation-changed` for session ids that no projection ever carries,
 * so the map must stay bounded. Eviction is oldest-first (Map insertion order)
 * and is counted in `dropped.goalActivations` (never silent).
 */
...
export function createSessionStateStore(deps: SessionStateStoreDeps): SessionStateStore {
  ...
  function validateDocument(raw: Record<string, unknown>): { doc: SessionStateDocument; droppedSessions: number } {
    ...
    // (the never-silent discipline). The two PROCESS-LOCAL cumulative counters
    // (readMarks / goalActivations) are normalized to 0 here: the retained goal
    // activation edges are not persisted, so neither are the losses that belong
    // to them; adopting a stale persisted value would also make the load path
    // read a document field this validation can never produce non-zero.
