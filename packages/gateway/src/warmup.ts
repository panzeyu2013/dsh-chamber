/**
 * Login-phase background pre-warm: the ONE pre-auth route that may touch the
 * managed dsh. The login page prefetches one REAL /plugins/... bundle URL per
 * discovered row so the post-login boot comes from the URL-keyed HTTP cache;
 * the capability therefore travels as a short-lived cookie, never in the URL.
 *
 * Discipline: a domain-separated HMAC capability cookie (not path-bound); the
 * shape allowlist of the two real bundle forms (other /plugins/** stays
 * authenticated); a per-client rate bucket; bounded concurrency/buffered bytes;
 * a kill switch; fail closed; upstream sees only the spawn-minted host cookie,
...
/** The warm-up capability cookie name. */
export const WARMUP_COOKIE_NAME = 'dsh_gateway_warmup'

/** Cookie lifetime / Max-Age (seconds). Short by design: a stolen grant can only re-fetch public assets. */
export const WARMUP_COOKIE_TTL_SECONDS = 120

/** Cookie Path: the whole gateway origin; the route's shape allowlist is the actual boundary. */
export const WARMUP_COOKIE_PATH = '/'

/** Only this path prefix may ever be proxied pre-auth. */
export const WARMUP_ALLOWED_PATH_PREFIX = '/plugins/'

/** The two REAL bundle shapes this route may serve pre-auth: the combination request and a single-row client bundle. */
...
/** How long a FAILED discovery is remembered (ms). Short on purpose: a recovered
 *  dsh gets its links on a later render, while failures cannot re-probe per request. */
export const WARMUP_DISCOVERY_FAILURE_TTL_MS = 10_000

/** Hard ceiling on one index discovery read (ms); the page awaits it, so it is the page's worst-case added latency. */
export const WARMUP_DISCOVERY_TIMEOUT_MS = 500

/** Cap on one upstream bundle response (bytes): fits the ~4.35 MiB gzip combo and an identity retry, bounded against a bad upstream. */
export const MAX_WARMUP_RESPONSE_BYTES = 32 * 1024 * 1024

/** Aggregate budget: max in-flight warm-up upstream fetches. The browser opens
 *  ~6 per host, so 8 admits a real page while a replay burst is refused (503). */
export const MAX_WARMUP_CONCURRENT_FETCHES = 8

/** Aggregate budget: total bytes buffered across ALL in-flight warm-up fetches;
 *  over budget answers 503 warmup_capacity. Released on every settle. */
export const MAX_WARMUP_TOTAL_BUFFERED_BYTES = 64 * 1024 * 1024

/** Cap on discovered bundle URLs per login page, sized from the REAL roster (the
 *  index carries 59 distinct quoted /plugins/?? URLs, including manifest url fields). */
...
/** Cap on one warm-up route target / token subject (chars): far beyond real combo URLs and below Node's request-line budget. */
export const MAX_WARMUP_TARGET_CHARS = 8 * 1024

/** Cap on a presented cookie value before parsing; far above anything legitimately minted. */
...
/** Per-client token bucket: the burst must exceed one page's own need (up to
 *  MAX_WARMUP_BUNDLE_URLS) or the page throttles itself; the sustained rate stays low. */
export const WARMUP_RATE_LIMIT_CAPACITY = 128
export const WARMUP_RATE_LIMIT_REFILL_PER_SECOND = 5
/** Cap on simultaneously tracked client buckets (the oldest is dropped at the cap). */
...
/** Domain label for the capability cookie grant: the signing key is derived from
 * the store's jwt secret through this label, so a grant and a session JWT can
 * never satisfy each other's verifier. */
const WARMUP_COOKIE_DOMAIN_LABEL = 'dsh-gateway/warmup-cookie/v1'

/** The cookie payload subject marker: a token blindly replayed as a cookie (or
 * the reverse) fails here before any cryptographic check could matter. */
...
/** Verify one presented value under one derived key and return its payload or
 * null: strict structure (one dot, canonical base64url), constant-time MAC
 * compare, then expiry (exp inclusive). */
function openWarmupPayload(key: Buffer, token: string, nowSeconds: number, maxChars: number): string | null {
  ...
  // Reject non-canonical encodings: Buffer.from is lenient about trailing bits.
...
/**
 * Mint the warm-up cookie value: HMAC over 'exp|warmup|<client address>' under
 * the distinct domain label. The payload carries NO path — one grant covers
 * every prefetched URL — so the path allowlist is the route's. A client
 * address, when given, binds the grant to the address that rendered the page.
 */
...
/**
 * Verify one presented capability cookie. Structure, MAC, expiry and subject
 * come from the shared core; a grant bound to a client address only verifies
 * for that same non-empty address (an unbound grant verifies from anywhere).
 */
...
/** Read the warm-up cookie out of one Cookie header; exact name match, an empty
...
/** A managed dsh port is 1..65535; an out-of-range value must fail closed BEFORE
 * any loopback URL or request is built. */
export function isValidDshPort(port: number | null | undefined): port is number {
  return typeof port === 'number' && Number.isInteger(port) && port >= 1 && port <= 65535
}

/**
 * The pre-auth path allowlist: target under /plugins/, none of the rejected
 * byte/segment forms (backslash, '//', ANY percent-encoding, whitespace/control,
 * fragment, dot segment, over-length), ONE of the two real bundle shapes, and an
 * unchanged round-trip through the URL parser.
 */
export function isWarmupPathAllowed(pathAndQuery: string): boolean {
  ...
  // the raw target could swallow a '?' and let a caller probe arbitrary
  // one-segment /plugins paths (upstream 404) instead of this gateway's 401.
  // A query is a COMBO-only form; the combination request keeps its query.
  ...
  try {
    ...
    // A string the parser normalizes (dot segments, encoded slashes) is not the
    // target it appears to be: refuse rather than forward a reinterpreted path.
...
/** Decode the HTML entities an attribute can carry around '&' before the URL is
 * used as a real URL. */
...
/**
 * Extract the real bundle URLs from the managed index, allowlist-filtered and
 * deduplicated, capped at MAX_WARMUP_BUNDLE_URLS.
 *
 * ORDER IS THE POINT: the URLs the document itself loads (<script src> /
 * <link href>) come FIRST, then the inline boot manifest's quoted urls — a
 * document-order pass can spend the cap on manifest rows and drop the combo.
 */
 ...
 * writeHead must never clobber a policy header, so the merge is comma-joined,
...
/** Minimal structural request surface the route drives (dispatch adapts the real IncomingMessage). */
export interface WarmupHttpRequest {
  method?: string
  /** RAW origin-form target, forwarded byte-for-byte: never the parsed URL, so a dot segment cannot be normalized first. */
  url?: string
...
export interface WarmupHttpResponse {
  ...
  /** Present on the real ServerResponse: used to merge an already-set Vary instead of overwriting it. */
  getHeader?(name: string): unknown
}

/** Route outcome. 'rejected' carries the machine code dispatch audits (never the
 * token/cookie value or URL); 'unclaimed' = not this route's to answer, so the
 * auth gate below owns the verdict. */
export type WarmupHandleOutcome =
  | { kind: 'proxied' }
  | { kind: 'rejected'; code: string }
  | { kind: 'unclaimed' }

/** Links() result: the REAL bundle URLs to prefetch plus, when any was discovered, the grant Set-Cookie. */
...
export interface WarmupDeps {
  ...
  /** Spawn-minted browser-auth cookie for one managed loopback port: an internal
   *  host credential, never a user credential and never returned to the client. */
  getAuthCookie?: (port: number) => string | undefined
  /** Discovery seam; defaults to a loopback GET / with the spawn-minted cookie. */
  fetchIndex?: (url: string, signal: AbortSignal, authCookie: string | undefined) => Promise<string>
  /** Clock seam; defaults to Date.now. */
  now?: () => number
  /** Bounded aggregate budget seam; production constants by default. */
  budget?: {
...
export interface WarmupController {
  /** The login page's bundle URLs and grant Set-Cookie; empty when the switch is
   *  off, the dsh is not ready or discovery found nothing. Never throws. */
  links(options?: WarmupLinkOptions): Promise<WarmupLinks>
  /** Claim the route: 'proxied', 'rejected' (405/429 — dispatch audits the code)
   *  or 'unclaimed' (switch off, wrong shape, no valid capability cookie). */
  handle(req: WarmupHttpRequest, res: WarmupHttpResponse, url: URL, clientAddress?: string): Promise<WarmupHandleOutcome>
}

/** Default discovery read: loopback index with the spawn-minted cookie; redirects refused, bounded body. */
async function fetchIndexDocument(url: string, signal: AbortSignal, authCookie: string | undefined): Promise<string> {
  ...
  // `response.text()` would read the WHOLE body before the cap is checked;
  // cancel as soon as the cap is crossed, so the cost is one chunk over it.
...
export function createWarmupController(deps: WarmupDeps): WarmupController {
  /** Clock seam in MILLISECONDS; tokens/TTLs are epoch SECONDS, hence nowSeconds(). */
  ...
  /** In-flight discovery per dsh port: without single-flight, N concurrent
   *  renders issue N
   *  concurrent loopback index fetches against the managed dsh (measured 100
   *  on one socket); concurrent callers share ONE fetch, so the map size bounds it. */
  ...
  async function discover(port: number): Promise<string[]> {
    ...
    // Past the aggregate cap: render without links (fail-soft, never a queue).
  ...
  /** Forward one allowlisted bundle request to the managed dsh. Buffered (per
   *  response cap AND global byte budget); headers are written only once the
   *  upstream ended, so a failure can still answer 502; the slot is held whole. */
  function proxyBundle(
    ...
    return new Promise<void>(resolve => {
      ...
      // The spawn-minted cookie is an INTERNAL host credential (not a user
      // credential): the caller's own Cookie/Authorization never crosses.
      const authCookie = authCookieFor(port)
        ...
        upstream = httpRequest({
          ...
          upstreamRes.on('end', () => {
            ...
            for (const name of WARMUP_PASSED_RESPONSE_HEADERS) {
              // `unknown`, not the declared union: TS narrows a
              // string|string[]|undefined through Array.isArray to never here.
  ...
  return {
    async links(options: WarmupLinkOptions = {}): Promise<WarmupLinks> {
      ...
      try {
        ...
        // Discovery failures are owned by discover() above; this outer arm is
        // reachable only from the grant mint. It stays as the login-page
        // fail-soft guard: the login page must never fail because of warm-up.
    ...
    async handle(req: WarmupHttpRequest, res: WarmupHttpResponse, url: URL, clientAddress?: string): Promise<WarmupHandleOutcome> {
      ...
      // The RAW target when provided: a dot-segment target must not be
      // normalized into the allowlist before it is checked.
      ...
      // the rate bucket and cookie binding must agree with dispatch's decision.
      const requestClient = clientAddress !== undefined && clientAddress !== '' ? clientAddress : (req.socket?.remoteAddress ?? '')
      // CAPABILITY FIRST: spending the bucket before checking the cookie would
      // let a caller WITHOUT a grant drain it and 429 a legitimately logged-in
      // NAT client on this pre-auth leg, before the auth gate's verdict or its
      // audit. Absent/stale/tampered/foreign cookie ⇒ unclaimed, so the route
      ...
      // A verified capability still pays the sustained bucket: the abuse bound, not an auth verdict.
      if (!limiter.consume(requestClient, now())) {
        // Documented refusal: 429 warmup_rate_limited.
