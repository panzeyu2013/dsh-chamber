/**
 * Per-source registry (B2 core, P4): ONE current generation per source id.
 *
 * WHY A REGISTRY AND NOT A KEYED RECORD. The first container keyed records by the
 * pair (sourceId, fingerprint), which made a re-registration start a NEW record -
 * but nothing removed the old one, and every projection walked all records of the
 * id. Two incarnations of one source therefore showed through as a merged,
 * contradictory view (a flag from the retired life, a hidden window from the new
 * one), and an event dispatched with the old fingerprint still reduced the old
 * record and still emitted its effects. The invariant is one sentence: a source id
...
// ---------------------------------------------------------------------------
// Projections: each one replaces exactly one of the App's refs. Every projection
