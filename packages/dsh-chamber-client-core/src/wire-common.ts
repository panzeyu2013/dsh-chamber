 ...
 * unary 调用机件:
 * - A packages/dsh-chamber-client-core/src/instance-api.ts
 * - B packages/dsh-chamber-client-core/src/plugin-graph-recheck.ts
 ...
 * 直接 import postUnary）；B/F 的 host-graph 状态/信封分类已收敛到
 * `plugin-graph-classify.ts`（本文件的 classifyGraphChannelFailure 今日只由它
 * 消费）；E 仅参考，不收其任何实现。
 *
 ...
 *   (今日消费方是 D 与 `plugin-graph-classify.ts`，调用点只做字符串键读取,`Record<PropertyKey, unknown>` 与
 ...
 *   ('internal'/'实例返回未知错误');B/F 的信封判定已收敛到
 *   plugin-graph-classify.ts（report/throw 风格且不做 rpcId/type 校验）;D 的骨架相近但文案前缀('plugin-inventory:')与
 ...
 *   InstanceUnavailableError 类身份/中止透传均不同;B/F 的 503 判定统一在
 *   plugin-graph-classify.ts(instance_unavailable → 不可判定静默路径,无文案;
 *   其他 503 → channel 失败)。
 ...
 *   不同体。
 *
 * 零运行时依赖、零 node 内建、零 import:client-core 会被 renderer
 ...
 * - 503 instance_unavailable 检测/动作与 wrapWireError/wrapGraphTransportFailure
 *   折行:见文件头——动作与文案逐载体不同（B/F 的折行在 plugin-graph-classify.ts）。
...
/** host-graph 通道失败分类(B≡F 的正则三分支,逐字同体):由
 *  `plugin-graph-classify.ts` 单一消费——它把 B/F 的状态/信封/文案整棵树收在一处,
 *  本函数是其中的正则件。调用面错误文案(code+message 拼串)提及 not-found /
 *  unknown-method / missing / unsupported → 通道在答但方法/行缺失 →
 *  'not-injected';其余一律 'graph-unreachable'。纯函数,零运行时依赖;文案
 *  (「宿主启动图:graph 调用失败:…」)由该消费方统一产出,正则不可能再漂移。 */
