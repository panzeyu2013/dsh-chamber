...
export interface InstanceRuntimeReport {
  ...
  /**
   * 会话事实单一权威的快照（runningSince / stuckSince / progressStamp）；缺席 =
   * 本记录内从未请求过。App 的升级 ladder 只读事实决定 reconnect 与 notice。
   * 执行端是 session-fact-reconcile.ts（reducer + probe ladder + I/O）。
   */
