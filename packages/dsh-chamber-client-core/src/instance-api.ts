...
// One derivation for the active-Schedule fact and the display-title ladder:
// both live in derive.ts (which this module only type-imports, so no runtime
// cycle); `basenameOf` is re-exported below for existing importers.
...
/** Default timeout for bounded unary calls; the single home of the 30s bound. */
const INSTANCE_UNARY_TIMEOUT_MS = 30_000
...
/** Browser origin with a non-browser fallback. */
...
class InstanceApiClient {
  ...
  readonly archiveCleanup = {
    purge: (
      ...
      this.call(
        archiveCleanupEndpoint(ARCHIVE_CLEANUP_PURGE_METHOD),
        { args: archiveCleanupPurgeArgs({ sessionIds, force, protectSessionIds }) },
        undefined,
        {
...
const OPTIONAL_PURGE_COUNTS = ['clearedOrphanMembers'] as const

type RequiredPurgeCount = (typeof REQUIRED_PURGE_COUNTS)[number]
type OptionalPurgeCount = (typeof OPTIONAL_PURGE_COUNTS)[number]

/** One purge count out of the decoded value: a non-negative finite number;
 *  absence is legal exactly when `required` is false. */
function purgeCount(value: unknown, key: string, required: boolean): number | undefined {
  const field = (value as Record<string, unknown> | null | undefined)?.[key]
  if (field === undefined) {
    if (!required) return undefined
    throw new Error(`归档清理域返回了畸形结果：purge 结果缺少必需的 ${key} 计数（宿主契约漂移）`)
  }
  if (typeof field !== 'number' || !Number.isFinite(field) || field < 0) {
    throw new Error(`归档清理域返回了畸形结果：purge 结果的 ${key} 不是非负有限数`)
  }
  return field
}

function requiredPurgeCount(value: unknown, key: RequiredPurgeCount): number {
  return purgeCount(value, key, true) as number
}

function optionalPurgeCount(value: unknown, key: OptionalPurgeCount): number | undefined {
...
function purgeTruncatedFlag(value: unknown): boolean {
  ...
  if (typeof field !== 'boolean') {
    throw new Error('归档清理域返回了畸形结果：purge 结果的 truncated 不是布尔值')
  }
  return field
}
...
export async function purgeArchivedSessions(
  ...
  const errors: ArchiveCleanupPurgeItemError[] = Array.isArray(rawErrors)
    ...
    : []
  const clearedOrphanMembers = optionalPurgeCount(value, 'clearedOrphanMembers')
  const residentRetainedRoots = optionalIdList(value, 'residentRetainedRoots')
  return {
    deletedSessions: requiredPurgeCount(value, 'deletedSessions'),
    deletedSubagents: requiredPurgeCount(value, 'deletedSubagents'),
    skippedRunning: requiredPurgeCount(value, 'skippedRunning'),
    skippedLoaded: requiredPurgeCount(value, 'skippedLoaded'),
    forcedLoaded: requiredPurgeCount(value, 'forcedLoaded'),
    skippedProtected: requiredPurgeCount(value, 'skippedProtected'),
    truncated: purgeTruncatedFlag(value),
    ...(clearedOrphanMembers === undefined || clearedOrphanMembers === 0 ? {} : { clearedOrphanMembers }),
