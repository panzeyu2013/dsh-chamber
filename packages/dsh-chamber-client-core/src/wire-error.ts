 ...
 * writes) and renderer host-graph.ts (different package) resolves null — both
 * consume one classification/copy module, plugin-graph-classify.ts, which owns
 * the `宿主启动图不可达：` transport fold.
 *
