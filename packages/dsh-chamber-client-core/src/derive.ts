...
/**
 * Active-Schedule fact of one session: mirrors the official
 * `(projectionValues?.schedule?.length ?? 0) > 0`. The bag here is
 * unknown-typed, so absent / not-an-array / empty all mean "no active
 * schedule" — never a claim about the future.
 */
 ...
 * because the display-title resolver below needs it and instance-api
 * value-imports this module — a value import the other way would be a runtime
 * cycle. instance-api re-exports it so existing importers keep their site.
 */
...
/**
 * THE session display-label resolver — one rule, official semantics: durable
 * title, then the canonical cwd's basename, then the raw session id, with
 * EMPTY treated as absent. It never returns an empty string (a label never
 * turns "unknown" into 「未命名会话」). `displayTitle` is a producer-resolved
 * value that wins when present; `cwdBasename` lets a caller that owns the cwd
 * hand in the basename without this module importing instance-api.
 */
export function sessionDisplayTitle(source: {
  ...
  // `typeof === 'string'` (not `!== undefined`): a JSON producer can deliver
  // `null`, which is not a label and must fall through the ladder.
  ...
  // the official rule answers '' and falls through to the session id.
...
export function mergeRuntimeFacts(
  ...
  // `stale` alone is not content: no report, no armed dot, no overlay row
  // means there is nothing to attach, so the two-argument early return stands.
...
export function runtimeReportSignature(
  ...
  // is indistinguishable from "no runtime attached" unless it carries a receipt
  // or listComplete — 回执/权威门本身就是内容，不得被去重吞掉。
...
/** Archived-session metadata row carried to archive-manager surfaces (the
 *  manager lists WHAT is archived, with workspace attribution for grouping). */
export interface ArchivedSessionMetaRow {
  ...
  updatedAt?: number
  /** Workspace attribution: the host workspace whose registry membership
   *  contains this session — or failing that, whose path equals the session's
   *  canonical cwd. Absent = not accounted by any live workspace (the manager
   *  lists it in the trailing ungrouped bucket). */
