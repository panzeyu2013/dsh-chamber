...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => {
    ...
    /**
     * 权威读（单一权威链）：控制面 HTTP 代理上的一次独立 unary `session.list`——与被守卫
     * 的 WS 事实通道是两条载体，一次 502/代理重启不会误伤判定。返回 `complete: true`：宿主
     * unary list 是全量列表，缺席因此是证据；「缺席作证」由 reducer 的 N=2 把关。
     */
