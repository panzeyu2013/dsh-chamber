...
export function apply(ctx: ClientContext): void {
  ...
  // chamber patch (06 §4.3): the runtime-facts channel's producer end. Every
  // boot is its own ctx with its own sessions store, so this plugin — mounted
  // in every ctx — reports THIS instance's runtime facts (current session,
  // pending interactions, completions) to the chamber bridge. The App layer
  // merges the report into the multi-source projection (server.runtime), and
  // the sidebar renders dots/highlights from that for every source. The
  // component no longer subscribes to the store itself; boot frames before
  // the first report simply render no highlight (06 §4.3). zustand subscribe
  // does not fire on mount, so the snapshot is reported immediately.
  ctx.effect(() => {
    ...
    const sync = (): void => {
      ...
      chamberBridge.reportInstanceRuntime(chamberInstanceId, projectRuntimeFacts(snapshot))
