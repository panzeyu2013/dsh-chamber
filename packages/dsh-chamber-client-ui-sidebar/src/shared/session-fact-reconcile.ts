 ...
 * (P2 of docs/progress/todo/session-authority-refactor.md).
 *
 ...
 *
 * The former 190 s receipt chain (official refresh + phase timers + attempt retries +
 * verdict branches) is RETIRED: a probe already has a carrier (the unary read), and the
 * refresh added a second, observably opaque one (vendor `refreshList()` resolves on
 * failure), which is exactly the fallback layering this refactor removes.
 *
...
/**
 * P5 evidence surface: what the authority actually did, in bounded form. It travels
 ...
 * drop" without a debugger. A file-backed surface is still host work (STATUS ⑩).
 */
...
export class SessionAuthorityReconciler {
   ...
   * is handed to the host's P5 persistence seam (exactly once, in production order).
   */
