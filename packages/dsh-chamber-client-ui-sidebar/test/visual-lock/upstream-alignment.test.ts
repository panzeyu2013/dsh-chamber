...
import { hasCjk, normalize, source, stripComments } from '../support/source-lock.ts'
...
test('T7 active-Schedule marker: rendered at the upstream position, both row kinds', () => {
  ...
  // behavioural proof lives in session-rows/schedule-label-reuse.test.ts).
  assert.ok(
...
test('the test-only vendor loader stays environment-free (CI regression)', () => {
  ...
  const loader = stripComments(source('./vendor-loader.mjs'))
  ...
  assert.match(source('./vendor-store-double.mjs'), /export function createSnapshotStore/,
