...
test('set/clear + version counter bumps on change only', () => {
  ...
  clearWorkspaceGitFlags('s'); assert.equal(getWorkspaceGitFlag('s', 'w1'), undefined)
...
test('retain prunes only the non-kept ids of the source', () => {
  ...
  clearWorkspaceGitFlags('s'); clearWorkspaceGitFlags('other')
})

test('repo layouts publish + clear with the source', () => {
  setSourceRepoLayouts('s', [{ repoKey: 'r', mainWorkspaceId: 'm', unregistered: [] }])
  assert.deepEqual(getSourceRepoLayouts('s'), [{ repoKey: 'r', mainWorkspaceId: 'm', unregistered: [] }])
...
test('git-flags-loaded marker: per-source, idempotent, reset on clear ( F4)', () => {
  ...
  clearWorkspaceGitFlags('s'); assert.equal(isSourceGitFlagsLoaded('s'), false, 'disconnect/clear resets the marker (reconnect re-gates)')
...
test('repo-group fold predicate (08 §11.7): derived rows hide only while the main is folded AND still present', () => {
  ...
  const hidden = hiddenByMainWorkspaceFold
  assert.equal(hidden(derived, true, true), true, 'main folded + present: derived row hidden with the group')
  assert.equal(hidden(derived, false, true), false, 'main expanded: derived row visible')
  assert.equal(hidden(derived, true, false), false,
    'main vanished from the aggregate: no fold control, so the derived row must not be locked hidden')
  assert.equal(hidden(undefined, true, true), false, 'no flag (non-git / unresolved): visible')
  assert.equal(hidden(derivedNoMain, true, true), false, 'derived of an UNREGISTERED main (no main row to fold): visible')
  assert.equal(hidden(mainFlag, true, true), false, 'the main row itself is never hidden by its own fold')
})
