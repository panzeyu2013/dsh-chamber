 ...
 *  7. **写回 seam**（2026-12 彻底修复，design 14 §D4 tier-3）：只在 verify 判 stale 时
...
test('失败后按注入的调度重试，第二次（refresh+verify 都过）才 ok:true', async () => {
  ...
  const h = harness({
    ...
    // refresh 被拒也进权威相位（2026-12 四轮修订）：用权威「证伪且无法写回」制造一次
    // 可重试失败，第二次权威确认才收敛。
...
test('单飞悬挂（refresh 永不结算）下写回仍可达：权威证伪并纠正 ⇒ converged + corrected', async () => {
  // 2026-12 四轮独立复核抓出的缺陷：此前 refresh 相位超时直接结算失败，写回不可达，
...
test('refresh 在权威相位在飞时结算/被拒，不得并发开第二轮（authorityStarted 栅栏）', async () => {
  ...
  const h = harness({
    ...
    // 第一次权威相位悬挂在 verify 上（在飞），此时 refresh 才被拒 —— 没有栅栏就会并发开第二轮。
    verify: (): Promise<SessionFactVerdict> => {
      verifyCalls += 1
...
test('decideAfterFirstAuthorityRead：正面证伪走 N=2；「权威沉默」无条件不算健康', () => {
  ...
  // 2026-12 五轮复核（HIGH）：官方 refresh 的**主流**失败形态是 resolve 成 ok:false，
...
// ---- 判定纯函数的行为锁（2026-12 四轮独立复核：接线文件只有源码正则，
...
// ---- tier-3 写回（2026-12 彻底修复，design 14 §D4）----
