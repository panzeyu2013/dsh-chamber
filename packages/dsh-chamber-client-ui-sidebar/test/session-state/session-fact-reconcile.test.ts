/**
 * 对账链契约（src/shared/session-fact-reconcile.ts）。锁：1) **权威判定**：官方 `refreshList()`
 * 对拉取失败照常 resolve ——「promise 解决」不算成功，只有 `verify` seam 确认拿到权威结论才
 * 结算 ok:true（守卫允许升级 L2/L3 的唯一依据，2026-12 独立复核致命缺陷的回归锁）；2) 单飞
 * （在途重复请求只推进 requestedAt）、有界重试、结算回执；3) 单次尝试的硬超时（悬挂的
 * refresh/verify 不得永久卡死单飞链）；4) seam 缺失的永久失败语义（WARN 一次、不重试）；
 * 5) dispose：取消在途超时与排期重试，且晚到的结算不得回调 onSettled；6) **权威判定规则
 * 本身**（sessionFactsConverged 纯函数）：子代理行与权威缺失行不作证、只有权威明确说
 * 「没在跑」才判未收敛（假升级＝reconnect 风暴，保守优先）。
 */
...
// Optional seams are OMITTED when absent (never passed as undefined); refresh/scheduleImpl explicit.
const definedOptions = <T extends object>(value: T): Partial<T> =>
  Object.fromEntries(Object.entries(value).filter(([key, entry]) => key !== 'refresh' && key !== 'scheduleImpl' && entry !== undefined)) as Partial<T>
...
function harness(options: {
  ...
  const reconciler = new SessionFactReconciler({
    ...
    schedule: options.scheduleImpl ?? ((run, ms) => { const entry = { run, ms }; scheduled.push(entry); return entry }),
...
test('refresh 解决但权威正面证伪（stale）⇒ ok:false（官方「失败也 resolve」的回归锁）', async () => {
  const h = harness({ refresh: () => Promise.resolve({ ok: false, error: { code: 'transport' } }), maxAttempts: 1,
...
test('辅助探针失败（unknown）⇒ 既非健康也不重试（不制造假升级）', async () => {
  const h = harness({ refresh: () => Promise.resolve(undefined), verify: async (): Promise<SessionFactVerdict> => 'unknown' })
  h.reconciler.request()
  await tick()
  const snapshot = h.reconciler.snapshot()
...
test('refresh 快而 verify 慢：两个相位各有独立预算（慢宿主不得被误判为未收敛）', async () => {
  ...
  const h = harness({ refresh: () => Promise.resolve(undefined), maxAttempts: 1, attemptTimeoutMs: 20, verifyTimeoutMs: 100,
    verify: async (): Promise<SessionFactVerdict> => { await new Promise(resolve => { setTimeout(resolve, 30) }); return 'converged' } })
  h.reconciler.request()
...
test('verify 缺失或抛错 ⇒ ok:false（绝不把「没校验」当成功）', async () => {
  ...
  const throwing = harness({ refresh: () => Promise.resolve(undefined), maxAttempts: 1,
...
test('失败后按注入的调度重试，第二次（refresh+verify 都过）才 ok:true', async () => {
  let calls = 0
  const h = harness({ verify: alwaysTrue,
    refresh: () => { calls += 1; return calls === 1 ? Promise.reject(new Error('boom')) : Promise.resolve(undefined) } })
  h.reconciler.request()
  await tick()
...
test('单飞：在途期间的重复请求只推进 requestedAt，不叠加第二条链', async () => {
  ...
  let calls = 0
  const h = harness({ verify: alwaysTrue, refresh: () => { calls += 1; return new Promise((resolve) => { resolveFirst = resolve }) } })
  h.reconciler.request()
  h.reconciler.request()
...
test('dispose：取消已排期的重试与在途超时，且晚到的结算不得回调 onSettled', async () => {
  ...
  const h = harness({ refresh: () => new Promise((resolve) => { resolveLate = resolve }), verify: alwaysTrue })
  h.reconciler.request()
...
test('nextSettledAt：冻结时钟下连续两次结算仍单调推进（水位比较不得吞掉后一次）', async () => {
  // 守卫只认「结算时刻严格大于上次消费水位」的回执；同一毫秒的第二次结算若不 +1 就会被
  // 当成旧回执丢弃（守卫永远看不到结论）。此前用例没断言单调性（2026-12 独立复核）。
  const h = harness({ refresh: () => Promise.resolve(undefined), verify: alwaysTrue })
  h.reconciler.request()
  await tick()
...
test('权威判定：官方与权威一致（都 running）⇒ 收敛（长工具/长推理不得升级）', () => {
  assert.equal(sessionFactsConverged({ a: { running: true }, b: { running: false } },
    [{ sessionId: 'a', running: true }, { sessionId: 'b', running: false }]), true)
})

test('权威判定：官方位卡住而权威说已结束 ⇒ 未收敛（允许升级 L2/L3）', () => {
  assert.equal(sessionFactsConverged({ a: { running: true } }, [{ sessionId: 'a', running: false }]), false)
})

test('权威判定：子代理行不作证（权威快照刻意过滤它们 ⇒ 否则永远假升级）', () => {
  assert.equal(sessionFactsConverged({ child: { running: true, origin: 'subagent' } }, []), true)
})
...
test('生产形状 deps（不注入 schedule/cancel）：成功结算不得被 attempt 超时定时器改写（二轮复核 critical 回归）', async () => {
  // 生产装配只传 refresh/verify/now/warn/onSettled。此前默认 schedule=setTimeout 而 cancel
  // 恒为 undefined ⇒ 超时定时器永不取消，在成功结算后开火把它改写成 ok:false；健康的通道上
  // 必然出现假失败回执 ⇒ 假 L2 + 假横幅。
  let refreshCalls = 0
  let settledCount = 0
  const reconciler = new SessionFactReconciler({ refresh: () => { refreshCalls += 1; return Promise.resolve(undefined) },
    verify: async (): Promise<SessionFactVerdict> => 'converged', now: () => Date.now(), warn: () => undefined,
    onSettled: () => { settledCount += 1 }, attemptTimeoutMs: 20, retryMs: 5 })
  reconciler.request()
  await new Promise(resolve => { setTimeout(resolve, 80) })
...
test('dispose 后仍在飞的 refresh continuation 不得再武装 verify 相位或调用探针（teardown 无副作用）', async () => {
  let resolveLate: ((value: unknown) => void) | undefined
  let verifyCalls = 0
  const h = harness({ refresh: () => new Promise((resolve) => { resolveLate = resolve }),
...
test('verify 相位超时结算为 unknown（辅助探针不归还 ≠ 权威证伪）', async () => {
  const h = harness({ refresh: () => Promise.resolve(undefined), verify: () => new Promise(() => {}), maxAttempts: 1, verifyTimeoutMs: 500 })
  h.reconciler.request()
  await tick()
...
test('dispose 之后排期的重试不再执行', async () => {
  let calls = 0
  const h = harness({ verify: alwaysTrue, refresh: () => { calls += 1; return Promise.reject(new Error('boom')) } })
  h.reconciler.request()
  await tick()
  h.reconciler.dispose()
  h.scheduled.filter(entry => entry.ms === 1_500)[0]?.run()
