 ...
 *
 * 接线锁（2026-12 裁决后仅剩 workspace-echo.test.ts）只证明调用形状；这里补出口的运行时契约：
 * 事实在 wire 成功后发布，携带宿主返回的 workspaceId 与 **canonical path**（不是浏览器侧那条
 * 可能含符号链接的请求路径）以及可选锚点；装饰（beforePublish）在事实**之前**、同一同步续体里
 * 运行——"回声行首帧就是最终形态"由此成为结构保证；装饰抛错被吞掉（宿主上的创建已经提交，
 * 渲染期装饰绝不能把成功变成 saga 的失败/补偿分支）；事实是稀疏的：没有锚点时 afterWorkspaceId
 * 字段根本不出现。
 *
 * 打桩方式：`getInstanceClient` 按 instanceId 缓存同一个 InstanceApiClient，直接替换该缓存对象
 * 的 `workspace.create`。真实 wire 在这里是 fetch（node 发不出），但其余链路——callAndThrow 的
 * 错误折叠与 decodeWorkspaceCreateValue 的结构校验——全部走真代码。
 */
...
function stubCreate(sourceId: string, value: unknown): { calls: number } {
  ...
  client.workspace.create = async (): Promise<UnaryResult<unknown>> => { counter.calls += 1; return { ok: true, value } }
  return counter
}
...
test('decorations run BEFORE the fact, and a throwing decoration never aborts the create', async () => {
  ...
  // The swallowed failure must still be loud in the console (silent success is a different bug):
  // capture it here so the runner output stays clean AND the logging contract is asserted.
  ...
  try {
    ...
    assert.equal(created.workspaceId, 'w3',
