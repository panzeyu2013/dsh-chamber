...
const row = (bootGap?: ChamberServerAggregate['bootGap']): ChamberServerAggregate => ({ bootGap } as ChamberServerAggregate)
...
test('sourceBootGapNote: each kind maps to its OWN key and carries its structured facts', () => {
  // Representative payloads (not empty ones — those legitimately share the generic sentence): the point is that
  ...
  for (let i = 0; i < KINDS.length; i += 1) assert.match(notes[i]!, /^source\.bootGap\.[A-Za-z]+/, KINDS[i])
  // Distinct sentences per kind: a re-badged kind that reuses another's key would make the sidebar claim the wrong cause.
  assert.equal(new Set(notes).size, KINDS.length, notes.join(' | '))
  assert.equal(sourceBootGapNote(row({ kind: 'required-services-missing', services: ['sidebarRight', 'slots'] }), spy),
    'source.bootGap.requiredServicesMissing({"services":"sidebarRight, slots"})')
  assert.equal(sourceBootGapNote(row({ kind: 'deferred-registration-failed', failedIds: ['a', 'b'] }), spy),
...
test('every gap key the sidebar can select exists in BOTH dictionaries, non-empty', () => {
  ...
  // The two parameterized sentences must keep their placeholders in both languages, otherwise a named service
  // silently disappears from the copy.
...
test('ServerSection renders the gap through the SHARED source-note live region', () => {
  ...
  // The warning tone is a MODIFIER of that one line, selected by construction (never by comparing rendered copy).
...
test('the gap note modifier class exists in the sidebar stylesheet', () => {
  ...
  assert.match(css, /\.sourceNoteBootGap \{[^}]*var\(--dsw-alias-state-warn-primary\)/,
