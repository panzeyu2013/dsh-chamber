...
/** The veil predicate on the cold-boot base (`blankCurrent` true); each case sets the rest. */
const veil = (overrides: Partial<Parameters<typeof shouldHoldViewVeil>[0]> = {}) =>
  shouldHoldViewVeil({ failed: false, pendingIntent: true, showsRequestedSession: false, blankCurrent: true, ...overrides })
...
test('projectableCurrent: only the active view projects a current, and never one the open is replacing', () => {
  ...
  assert.equal(projectableCurrent('ssh-b', 'ssh-b', 'blank-1', 's1'), undefined,
  ...
  assert.equal(projectableCurrent('ssh-b', 'ssh-b', 's1', 's1'), 's1',
    'an idempotent re-open keeps its highlight — the projection is already correct')
  assert.equal(projectableCurrent('ssh-b', 'ssh-b', undefined, undefined), undefined, 'no current and no open → nothing to project')
})

test('shouldHoldViewVeil: holds only while the view would show NOTHING legitimate (2026-09-11 review S1)', () => {
  assert.equal(veil(), true, 'cold boot: the requested session is not current and the view is blank — the hold hides the blank row')
  assert.equal(veil({ blankCurrent: false }), false,
    'a WARM shell showing a legitimate session must not be covered by the opaque veil (design 05:162)')
  assert.equal(veil({ showsRequestedSession: true, blankCurrent: false }), false,
    'already showing the requested session → reveal now (idempotent re-open, or the early-open arm won)')
  assert.equal(veil({ showsRequestedSession: true }), false, 'the showsRequestedSession exclusion wins over a blankness flag that disagrees with it')
  assert.equal(veil({ failed: true }), false,
  ...
  assert.equal(veil({ pendingIntent: false }), false, 'no open in flight → the shell reveals at settle')
  assert.equal(veil({ pendingIntent: false, showsRequestedSession: true, blankCurrent: false }), false, 'no open in flight at all')
})
...
test('shouldEarlyOpenSession: needs BOTH a live intent and an addressable session', () => {
  ...
  assert.equal(shouldEarlyOpenSession(undefined, true), false, 'a released intent must retire the arm (the App already owns the outcome)')
  assert.equal(shouldEarlyOpenSession('s1', false), false, 'opening an unknown id would make the official controller throw sessions/select-unknown')
})
