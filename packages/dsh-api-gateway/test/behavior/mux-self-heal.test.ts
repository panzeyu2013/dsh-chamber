...
// (1.1a): this suite drives the REAL client against those values, not a local copy.
...
test('a silent socket is REPLACED when an opening item times out on it', async (t) => {
  ...
  assert.deepEqual(facts.map(fact => fact.kind).slice(0, 2), ['opening-timeout', 'socket-silent'])
  // R2 (2026-12): the replaced socket's claim is released when its episode (this
  // stream) ends, so the release fact is part of the expected trail.
  assert.ok(
    facts.some(fact => fact.kind === 'carrier-forensic' && fact.cause.startsWith('episode-claim-released')),
    'the replaced socket claim is released when its episode ends',
  )
  await client.close()
  t.mock.timers.reset()
})
...
test('a logical stream torn down on a socket that never delivered a frame replaces it', async (t) => {
  ...
  assert.equal(FakeSocket.instances[0].readyState, FakeSocket.CLOSED)
  assert.deepEqual(facts.map(fact => fact.kind).slice(0, 1), ['socket-silent'])
  // R2 (2026-12): the teardown escalation the line above just triggered creates a
  // rebuild claim; the finally's LAST step releases it as the episode ends.
  assert.ok(
    facts.some(fact => fact.kind === 'carrier-forensic' && fact.cause.startsWith('episode-claim-released')),
    'the teardown-created claim is released in the same finally',
  )
  await client.close()
  t.mock.timers.reset()
})
