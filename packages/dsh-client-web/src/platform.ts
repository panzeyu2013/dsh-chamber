 ...
 * the boot fails with "invalid plugin … received object". `dsh-client-ui-attachment`
 * (it has a client half) and `dsh-client-web-react` / `dsh-client-schema-form`
 * are excluded for that same structural reason. `@deepseek-ai/dsh-client-store` stays a shell-shared
 * singleton like cordis — never a host-graph row.
 *
 * `@deepseek-ai/dsh-client-ui-primitives` is deliberately absent from this
 * table while upstream still externalizes it (at the pinned rc.1 the
 ...
 * seed.ts) consciously. The upstream-added `@deepseek-ai/dsh-client-ui-dockkit`
 * word is answered by the composite's covered factory instead of this seed, for the
