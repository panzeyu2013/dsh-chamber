...
/**
 * The managed-dsh pid record: written atomically by writePidRecord with fixed
 * columns plus optional extra fields (e.g. ownerInstanceId).
 */
...
/**
 * Write the managed-dsh pid record for a spawned child; the write is atomic
 * (tmp + rename) and extra fields (e.g. ownerInstanceId) are appended to the fixed
 * columns. `entryPath` is the spawned entry token (argv0 for installed layouts, the
 * script path for the dev-tree source layout): the reaper re-verifies the live
 * process against it, and without it a crashed dev tree can never match and leaves
 * the writer-quiescence latch closed.
 */
export function writePidRecord(stateDir: string, pid: number, port: number, ownerPid: number, extra: Record<string, unknown> = {}, entryPath?: string | null): void {
  ...
  const record = {
    ...
    ...additional,
    port,
    // Record the exact CLI entry for this process. The reaper compares this
    // absolute path against the live command line; a basename marker is too broad.
    binary,
