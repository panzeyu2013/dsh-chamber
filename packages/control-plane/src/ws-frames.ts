 ...
 *
 * The proxy needs only two things: an encoder for the unmasked ping frames it
 * injects downstream (the proxy is the ws server to the browser, which
 * auto-pongs per RFC) and a passive scanner that detects the browser's pong
 * frames in its raw byte stream. The scanner never consumes bytes (the splice
 * pipe stays untouched): it skips frames by declared lengths and reports when
 * a pong-opcode frame starts. It still handles masked data frames and extended
 * lengths defensively so it stays in sync across arbitrary chunk boundaries.
 */
...
/** Encode one complete unmasked ping frame (server→client); payload ≤ 125
 *  bytes, echoed back by the ponging peer. */
 ...
 * Keeps its own frame-position state, so arbitrary chunk boundaries and
 * interleaved data frames stay correct.
 */
