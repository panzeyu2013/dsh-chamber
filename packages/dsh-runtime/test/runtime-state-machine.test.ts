...
test('restart-dsh: present in every non-busy managed phase, absent from busy/applying/snapshot-failed', () => {
  // design 18 §3.6 项 8: the transactional control-plane restart is single-
  // flight and re-seeds mounted plugins; it is a process refresh, not a
  // version mutation, so it stays available in every non-busy phase — but
  // never while an install/apply transaction is in flight.
  for (const phase of ['idle', 'available', 'applied', 'rollback', 'failed', 'error'] as const) {
    assert.ok(allowedActions(phase).includes('restart-dsh'), `${phase} must allow restart-dsh`)
  }
  for (const phase of ['checking', 'downloading', 'installing', 'pending', 'applying', 'snapshot-failed'] as const) {
    assert.equal(allowedActions(phase).includes('restart-dsh'), false, `${phase} must not allow restart-dsh`)
  }
  // Capability bits must neither suppress nor invent it.
  assert.ok(allowedActions('failed', { canRetryRestore: true, canRetryApply: true }).includes('restart-dsh'))
  assert.equal(allowedActions('snapshot-failed', { canRetryApply: true }).includes('restart-dsh'), false)
});
