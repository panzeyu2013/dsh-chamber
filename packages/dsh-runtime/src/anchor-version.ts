/**
 * 内建 dsh 锚点版本读取的单一来源（design 18 §9.3）。
 *
 * apply-phase 以「切换来源」的真实 semver 作为快照名，锚点工作区必须贡献它的
 * @deepseek-ai/dsh 版本。三个候选按优先级：工作区根 package.json 的
 * dependencies[@deepseek-ai/dsh]（源码 checkout 的声明）→ 已安装包清单
 * (node_modules/@deepseek-ai/dsh/package.json) 的 version → 源码 checkout
 * apps/cli/package.json 的 version。读不到 / 非法（accept 拒绝）继续下一候选，
 * 全部失败返回 null——调用方 loud 失败，绝不伪造快照名。
 *
 * 消费方：gateway/src/runtime-manager.ts（本轮，builtin/env 锚点版本）；
 * desktop 侧下一轮接线。
 */
...
/**
 * 读锚点工作区的 dsh 版本；accept 默认 {@link isSafeVersion}（精确 semver，
 * 路径安全预校验）。返回命中的原始字符串（与既有各 flavor 一致，不做 trim）。
 */
