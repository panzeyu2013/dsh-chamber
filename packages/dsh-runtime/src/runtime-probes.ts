 ...
 * fallback fires the caller's `warn` sink — a REQUIRED RuntimeProbeOptions
 * member since the 2026-12 audit found every production caller had omitted the
 * optional one, which made the downgrade silent (the desktop control-plane's
 * own identity probes warn on the same condition via their logger — see
 * control-plane dsh-client probeHostIdentity).
 *
