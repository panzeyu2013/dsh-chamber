...
export function registerOpenIn(ctx: OpenInWiringCtx): OpenInWiring {
  ...
  const openInCtx: OpenInLaunchContext = {
    ...
    stat: (p) => classifyLocalPath(fsp.stat, p),
    openPath: async (p) => {
      // shell.openPath 部分失败模式（win32/linux）存在 reject 路径——与
      // openVscodeUrl 封装同款纪律：reject 归一为错误串（loud），绝不落
      // transport rejection。
      const error = await invokeOpenPath(shell.openPath, p)
      if (error !== null) console.error('[dsh-chamber] 打开路径失败：', error)
      return error
    },
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_IN_APPS, trustedIpc(() => ({
    apps: listOpenInApps(openInCtx, (appId, error) => {
      console.error(`[dsh-chamber] ${appId} capability probe failed:`, error)
    }),
  })))
