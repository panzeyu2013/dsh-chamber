...
export interface ChamberSettings {
  ...
  /** 调试模式：镜像同步纪律同 notifications（结构与 chamber-settings.ts 的
   *  ChamberDebugSettings 及 renderer global.d.ts 保持一致）。 */
  debug: ChamberDebugSettings
}

/** 调试模式子块：开启后 Swift 原生壳置 WKWebView.isInspectable（Safari Web
 *  Inspector 可附着）；默认关闭。 */
 ...
 *  renderer global.d.ts 的 ChamberSettingsSupported 逐字段一致——镜像同步纪律）。 */
export interface ChamberSettingsSupported {
  ...
  /** 调试模式平台能力：只有 Swift 原生壳（WKWebView isInspectable）可用；
   *  本版 Electron 腿未接线 → false（设置页禁用开关并给出原因）。
