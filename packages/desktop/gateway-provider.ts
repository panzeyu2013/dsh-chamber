...
// Shared corrupt/unbound preserve + legacy-`.tmp` sweep mechanics for the owner-only store files (single source).
...
// Re-exported for module consumers (gateway-session login pinning, tests).
...
/** The legacy (schemaVersion 1) mirror file name: non-empty values cannot be auto-bound and stay preserved + disabled. */
const LEGACY_TOKEN_FILE_NAME = 'gateway-tokens.json'

/** This store's unbound-preserve wording: the shared mechanics live in
 *  store-file-hygiene.ts, only these sentences are store-specific. */
...
export function configureGatewaySecretStore(
  ...
  if (parsed.schemaVersion === 2) {
    // No safe automatic adoption exists: this may be the new-target credential half
    // left by a crash before the registry commit; non-empty stays preserved.
...
/** Safely handle a schemaVersion 1 `gateway-tokens.json` beside the bound v3 file:
 *  empty legacy state converges automatically, non-empty stays disabled (unbindable). */
function migrateLegacyTokenFile(file: string): string | null {
  const legacyPath = join(dirname(file), LEGACY_TOKEN_FILE_NAME)
  // If the embedding configures the legacy path itself, the v1 in-place load above already handled it.
  ...
  // v1 values are always plaintext (schemaVersion 1 never encrypted) — identity resolver, no blob detection.
...
/** The http transport provider: validate → direct-endpoint (no child) → http(s) probe.
 *  Serves the GATEWAY target only (dsh×http is disabled; see the file head). `kind` stays
 *  the legacy registry key it is registered under; transport-keyed `{ http: … }` also works. */
