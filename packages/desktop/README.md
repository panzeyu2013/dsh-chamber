...
- **dsh 运行时 = 官方发布包**：脚本用 `pnpm add @deepseek-ai/dsh@0.1.5-alpha.2`（默认精确 pin；`DSH_CHAMBER_DSH_VERSION` 只接受精确 semver 做显式升级验证，拒绝 `latest`/range/URL；`--force` 仅刷新当前精确版本）。这个 pin **只约束桌面应用内嵌的本地 runtime，不约束远程实例版本**；各远程可独立升级，连接时只检查所需协议能力是否兼容。发布包自带完整插件依赖图 + 已构建 lib，**不克隆源码、不 tsc/tsdown 构建、不需要 tsx**。
...
- **SSH 隧道**：`ssh -N -o ServerAliveInterval=30 -o ServerAliveCountMax=3 [-p <sshPort>] -L <localPort>:127.0.0.1:<remotePort> <user@host>`；localPort 由 `net listen(0)` 分配。dsh 目标以统一身份方法 `session/canOpenWorkspacePath`（固定小体积 boolean；老 runtime 树答 404 时 signature 路径回退 legacy `session/list` 识别为 "check or upgrade"）握手；gateway 目标以认证后 `/chamber/runtime/status` 固定 marker 握手，使 managed dsh blocked/down 时恢复面仍可达。gateway tunnel 的 HTTP Host/session authority 固定远端 `127.0.0.1:<remotePort>`，绝不复用 SSH alias。非目标服务均显式失败，绝不呈现假 ready。
