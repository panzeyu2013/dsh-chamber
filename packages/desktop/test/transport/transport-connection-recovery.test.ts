/**
 * Transport manager — part 2: load recovery, auth-failure terminality,
 * degraded → reconnect with jittered backoff, real dsh identity verification
 * through the tunnel, ring-buffer bounds and disconnect.
 *
 * Sibling parts: transport-manager.test.ts, transport-exec-and-registry.test.ts,
 * transport-providers.test.ts (shared harness in
 * test/support/transport-manager-harness.ts).
 */
...
import type { TransportInstanceInput } from '../../transport-provider.ts'
import { probeChamberHostLive, sshProvider, verifyDshEndpoint } from '../../ssh-provider.ts'
import { silentLogger, FakeChild, makeManager, tempDir, sleep, waitFor, type StatusWithNoUrlLeak } from '../support/transport-manager-harness.ts'
