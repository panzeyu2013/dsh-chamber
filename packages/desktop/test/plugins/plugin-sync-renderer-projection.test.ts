 ...
 * Sibling parts: plugin-sync.test.ts, plugin-sync-remote-read.test.ts, plugin-sync-apply.test.ts, plugin-sync-seed.test.ts.
 */
...
test('redactLocalPluginManifest: local-path spec values are masked, registry values untouched', () => {
  const manifest = {
    dependencies: {
      'file-dep': 'file:/Users/x/pkg', 'link-dep': 'link:../pkg', 'rel-dep': './pkg', 'abs-dep': '/opt/pkg', 'home-dep': '~/pkg',
      'registry-dep': '^1.2.3', 'pinned-dep': '1.2.3', 'tag-dep': 'latest', 'workspace-dep': 'workspace:*', 'npm-dep': 'npm:some-alias',
      'git-dep': 'git+ssh://git@example.com/x/y.git', 'github-dep': 'github:user/repo', 'url-dep': 'https://example.com/pkg.tgz',
    },
    bundles: ['file-dep'],
