...
export default async function beforePack() {
  ...
  const isLink = existsSync(targetDir)
    && (lstatSync(targetDir).isSymbolicLink() || !lstatSync(targetDir).isDirectory())
  if (isLink) rmSync(targetDir, { recursive: true, force: true })
