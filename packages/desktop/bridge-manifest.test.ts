 ...
 *   ③ 通道数守恒：counts {invoke:61, push:9, total:70} 与两列表长度自洽；
 *   ④ 无死键：manifest 键集 == ipc-events.ts IPC_CHANNELS 表键集（70 全覆盖、
 *      键/通道无重复、invoke/push 无交集 —— 生成器内部同样校验并 loud
 *      失败，此处以测试侧复刻解析把该事实变成可见断言）；
 *   ⑤ 方向抽查：dsh-chamber:info / desktop_ssh_instances_get 属 invoke，
 *      dsh-chamber:settings-changed 等 9 个 push 通道为精确集（与 main 侧
...
const COMMITTED_SWIFT_PATH = join(REPO_ROOT, 'macos', 'Sources', 'DSHChamber', 'Generated', 'BridgeManifest.swift')
...
test('③ 通道数守恒：counts {invoke:61, push:9, total:70} 与两列表长度自洽', () => {
  const { manifest } = regenerated()
  // 当前仓库事实（70 = 61 + 9，与 ipc-surface-mirror.test.ts 的集合断言
  // 同一批事实）；通道增删时须与两提交物同步更新。
  assert.deepEqual(manifest.counts, { invoke: 61, push: 9, total: 70 })
...
test('④ 无死键：manifest 键/通道集 == ipc-events.ts IPC_CHANNELS 表（70 全覆盖、无重复、无交集）', () => {
  const table = ipcChannelTable()
  assert.equal(table.length, 70, 'IPC_CHANNELS 应恰为 70 键（当前事实）')
  const { manifest } = regenerated()
...
test('⑤ 方向抽查：invoke/push 归属与 main 侧注册事实一致（含 9 push 精确集）', () => {
  const { manifest } = regenerated()
  ...
  // push 面精确集：与 main 侧 9 处 rendererPush/webContents.send 注册点一一对应
  ...
  // SSH_INSTANCES_CHANGED / RUNTIME_STATE_CHANGED / RENDERER_STALL_EVIDENCE）。
  ...
  assert.deepEqual(
    ...
    goldenPushChannels,
    'push 通道集必须 == main 侧 9 个推送注册点的精确集',
  )
