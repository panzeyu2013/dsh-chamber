...
function send(frame: OutboundFrame): void {
  ...
  try {
    ...
    console.error('[sidecar-stub] stdout write failed: ' + describeError(err))
  }
}
...
function parseRequestFrame(rawLine: string): RequestFrame | null {
  ...
  if (parsed === undefined) {
    console.error('[sidecar-stub] malformed frame (not JSON), ignored: ' + preview(rawLine))
    return null
  }
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    console.error('[sidecar-stub] malformed frame (not a JSON object), ignored: ' + preview(rawLine))
    return null
  }
...
function readInstancesFile(instancesPath: string): string | null {
  try {
    ...
    console.error('[sidecar-stub] desktop_ssh_instances_get: cannot read ' + instancesPath + ' (' + describeError(err) + ') — answering poc-no-registry')
    return null
  }
}
...
function parseInstancesJson(instancesPath: string, rawText: string): Json | null {
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.error('[sidecar-stub] desktop_ssh_instances_get: content of ' + instancesPath + ' is not a JSON array — answering poc-no-registry')
      return null
    }
    return parsed as Json
  } catch (err) {
    console.error('[sidecar-stub] desktop_ssh_instances_get: not valid JSON in ' + instancesPath + ' (' + describeError(err) + ') — answering poc-no-registry')
    return null
  }
}
...
async function disconnectHandler(payload: Json | null): Promise<Json> {
  const instanceId = instanceIdFromPayload(payload)
  if (instanceId === null) {
    console.error('[sidecar-stub] desktop_ssh_disconnect: payload.instanceId must be a string')
    throw new HandlerError('invalid-payload')
  }
...
function statusHandler(payload: Json | null): Json {
  const instanceId = instanceIdFromPayload(payload)
  if (instanceId === null) {
    console.error('[sidecar-stub] desktop_ssh_status: payload.instanceId must be a string')
    throw new HandlerError('invalid-payload')
  }
...
async function dispatch(method: string, payload: Json | null, id: number): Promise<void> {
  ...
  try {
    ...
    if (!(err instanceof HandlerError)) {
      console.error('[sidecar-stub] handler threw for ' + method + ': ' + describeError(err))
    }
    send({ id: id, ok: false, error: err instanceof HandlerError ? err.message : 'sidecar-stub-internal-error' })
  }
}
...
rl.on('close', (): void => {
  ...
  console.error('[sidecar-stub] stdin EOF — exiting 0')
  process.exit(0)
})
...
console.error('[sidecar-stub] started; --instances=' + (INSTANCES_PATH === null ? '(none)' : INSTANCES_PATH))
