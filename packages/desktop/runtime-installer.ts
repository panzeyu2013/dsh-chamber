...
const INSTALL_ENV_WHITELIST = /^(PATH|HTTP_PROXY|HTTPS_PROXY|NO_PROXY)$/
...
export class RuntimeInstallerSupervisor {
  ...
  async run(args: string[], opts: RunOptions): Promise<RunResult> {
    if (this.disposing) throw new Error('runtime installer is shutting down')
...
function resolveInstallerNodeExecutable(): { file: string; args: string[]; env: Record<string, string> } {
  // Keep this local: the packaged app intentionally excludes the workspace
  // control-plane package. A bare @dsh-chamber/control-plane import here would
  // work in development but fail after packaging.
...
function createOperationDeadline(external: AbortSignal | undefined, timeoutMs: number): {
  ...
  if (runtimeInstallerDisposing) throw new Error('runtime installer is shutting down')
