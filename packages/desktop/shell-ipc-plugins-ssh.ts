...
export function registerSshPluginHandlers(ctx: ShellIpcCtx): void {
  ...
  // gateway 插件三注册体在 shell-ipc-plugins-gateway.ts：trustedIpc 围栏由装配侧
  // 注入的 registrar 包装，注册参数读取（getGatewaySyncRegistration 纯模块，读写
  // 同表不分叉）与 ready 位复验在注册体侧；手动 sync 的上传执行闭包经
  // ctx.syncGatewayChamberPluginsFor 与 ready 自动 sync 共用同一路径，语义不分叉。
  // 确认对话框、无存活主窗预检与插件源 pick 均由 edges（electron-edges.ts）提供。
}
