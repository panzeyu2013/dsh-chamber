...
// Stage-3 domain registrars (2026-12 shell-core split; order of the calls below is
...
/** Stage-3（2026-12）：拆分后 shell-ipc-settings.ts 经 ctx.state 读写这三个模块级
...
/** Stage-3 (2026-12) domain-split context: the ShellAssemblyCtx deps plus the
 *  shared local helpers/module state the extracted domain registrars close over.
...
export function installIpcHandlers(deps: {
   ...
   *  蓝点不受影响。 */
  // Stage-3 (2026-12) 域拆分上下文：单点构造，晚定义的 helper 以 getter 延迟解析，
  // 注册体经 shellIpcCtx 取共享 helper/模块状态（顺序/错误语义与拆分前逐字一致）。
  const shellIpcCtx = {
    deps,
    deliveryEdges,
    quittingLeaf,
