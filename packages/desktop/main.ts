...
function installRendererRecovery(win: BrowserWindow): void {
  ...
  win.webContents.on('did-finish-load', () => {
    loadedOnce = true;
    // ready() may run while late subresources keep isLoading() true; finish is
    // the deterministic replay edge. Guard window identity so an old window
    // cannot drain/reset a newer main window's queues (shell-core owns them).
    if (mainWindow === win) {
