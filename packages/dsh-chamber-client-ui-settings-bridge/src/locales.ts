...
export const zh = {
  ...
  generalNotificationsTestReason: '原因：',
  generalNotificationsPermissionHint: '若通知权限被拒绝，macOS 不会再弹出授权框：请在「系统设置 → 通知」中找到 dsh-chamber 并允许通知。',
  generalNotificationsOpenSettings: '打开系统设置',
  ...
  // 原生（Sparkle）阶段没有百分比：不定量下载文案。
  updateDownloadingIndeterminate: '正在下载更新…',
  updateDownloaded: '已下载，退出时安装',
  updateRestartAction: '重启并安装',
  // 重启失败行（phase 保持 downloaded，仅经 restartFailureText 携带失败）。
  updateRestartFailed: '重启并安装失败：{error}',
  // 重启在途行（已点击重启并安装、退出窗口尚未完成时的诚实进展文案）。
  updateRestarting: '正在重启并安装…',
  // 原生安装中行（Sparkle 正在替换 bundle；Electron 不产生该相位）。
  ...
  dshRuntimeVersionsRefreshTimeout: '版本列表拉取超时，请重试',
  ...
  dshRuntimeRestartConfirm: '重启 dsh 以刷新插件挂载？会话数据不受影响；完成后界面会重新加载一次（需重新打开设置）。',
  dshRuntimeRestartGatewayConfirm: '将受控重启该 gateway 托管的 dsh：插件挂载随之刷新，本连接与该 gateway 上其他用户的会话将短暂断开；运行时版本与数据不动。完成后界面会重新加载一次（需重新打开设置）。',
  dshRuntimeRestartNotServed: 'dsh 未在限定时间内恢复就绪，界面未重新加载；请查看当前状态后重试，或手动刷新界面。',
  dshRuntimeRestartRefusedNotRunning: '重启被拒绝：托管 dsh 未在运行（409 {code}）——请改用「启动实例」',
  dshRuntimeRestartRefusedBusy: '重启被拒绝：运行时正忙或正在恢复（409 {code}），请稍后重试',
  dshRuntimeStatusTimeout: '运行时状态读取超时，正在按周期重试…',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  generalNotificationsTestReason: 'Reason: ',
  generalNotificationsPermissionHint: 'If notification permission was denied, macOS will not prompt again: allow dsh-chamber under System Settings → Notifications.',
  generalNotificationsOpenSettings: 'Open System Settings',
  ...
  // Native (Sparkle) phases carry no percentage: indeterminate download copy.
  ...
  // Restart-failure line (phase stays downloaded; the failure rides restartFailureText).
  updateRestartFailed: 'Restart & install failed: {error}',
  // Restart in-progress line (honest progress copy while the quit window runs).
  updateRestarting: 'Restarting to install…',
  // Native installing row (Sparkle is replacing the bundle; Electron never emits it).
  ...
  dshRuntimeVersionsRefreshTimeout: 'Version list refresh timed out — please retry',
  ...
  dshRuntimeRestartConfirm: 'Restart dsh to refresh mounted plugins? Session data is unaffected; the window reloads once afterwards (reopen Settings to see it).',
  dshRuntimeRestartGatewayConfirm: 'Restart the gateway-managed dsh to refresh mounted plugins? This connection and other users\u2019 sessions on this gateway will briefly disconnect; runtime version and data are untouched. The window reloads once afterwards (reopen Settings to see it).',
  dshRuntimeRestartNotServed: 'dsh did not return to ready within the limit, so the window was not reloaded; check the current state and retry, or reload the window manually.',
  dshRuntimeRestartRefusedNotRunning: 'Restart refused: the managed dsh is not running (409 {code}) — use "Start instance" instead',
  dshRuntimeRestartRefusedBusy: 'Restart refused: the runtime is busy or recovering (409 {code}); retry shortly',
  dshRuntimeStatusTimeout: 'The runtime status read timed out; retrying on the poll cadence…',
