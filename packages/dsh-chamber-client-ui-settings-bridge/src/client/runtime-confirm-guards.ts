 ...
 * confirmation dialog, and the request carries the guard predicate that allowed the arm.
 * The predicate must be evaluated against the CURRENT facts: the dialog can stay open
 * across the section's ~3s status poll, so a render-scope boolean captured at ARM time
 * would let the accept fire an action the live facts refuse (e.g. `restore-builtin`
 * armed at `phase=idle` while the poll flips the gate to `phase=installing`). Three
 * actions also carry a target captured at arm time (`cleanup-version` a version,
 * `restore-pre-rollback` a stash name, `apply-now` the pending version), so a stale
 * accept is not even always an honest 409.
 *
 * One predicate per action, evaluated against a LIVE fact snapshot — the arm-time early
 * return and the accept-time `stillValid` hook call the SAME function, so they cannot
 * drift. Every gate is the section's existing pure projection recomputed over the
 * snapshot; this module is pure and plain-node testable.
 */
import {
 ...
 * mirrors them on every render (React state is not readable synchronously from an
 * event handler's older-render closure).
 */
...
/**
 * 「立即应用」(apply-now) is still offered AND still targets the version the dialog
 * named (a stale target would apply a different pending version).
...
/**
 * 清理已安装版本 (cleanup-version) is still offered AND still targets a version the
 * server currently lists as removable — the captured version may have become active
 * (or left the list) while the dialog was open.
 * @param version - the candidate captured when the dialog was armed.
 */
 ...
 * guard share this one definition, so they cannot drift.
 *
 * The startup-block test keeps the row's own STRICT `=== null` (the section's other
 * projections treat an empty-string reason as "not blocked"). Keeping the strict
 * form means this row can only ever be HIDDEN in a state the gates would allow,
 * never offered in one they refuse — the conservative direction.
...
/**
 * 恢复回滚前数据 (restore-pre-rollback) is still offered AND the captured stash is
 * STILL the latest one — the route restores whatever stash it is handed, so a stale
 * name would restore a different snapshot than the dialog named.
