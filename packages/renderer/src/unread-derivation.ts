 ...
 *      通道边沿该会话即被事实结算（aborted+user 不得因通道边沿假武装）。
 *
...
/** 同形布尔表比较（账本 identity 闸）：App 以
 *  `sameBooleanMap as sameBooleanLedger` 复用本实现。 */
 ...
 * 落盘 + ack + 重算；undefined/0 不臆造。
 * 有 facts 行时取 max(updatedAt, completedAt)——但 completedAt 只在**host 域**时参与
 * （completedAtDomain === 'observer' 的降级戳只武装、不推进读水位；见 factsWatermark）；
