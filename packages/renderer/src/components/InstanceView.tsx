...
/**
 * 单调时基：持有时钟与相位计窗只做差值比较，绝不能受墙钟步进影响——一次性定时器
 * 算负 elapsed 会拒绝释放且不会重臂，有界出口就此静默消失。`performance.now()`
 * 恒在，缺失时退回墙钟（测试/异常环境）。
 */
...
export default function InstanceView({
  ...
  return (
    <div className={shellHeld ? `${viewClass} instance-veil-held` : viewClass} data-instance={instanceId}>
      ...
      {/* a11y：动作出现后遮罩不能是 aria-busy=true——它会把重试/连接/切换的更新播报
          压后；排队文案在反馈窗之外，同样必须立刻播报。 */}
      {veilVisible && (
