 ...
 *
 * 新旧 API（Wave3 迁移已收口）：`planRuntimeNotifications` / `planFactsNotifications`
 * 只为既有语义用例保留导出面（notification-projection.test.ts 仍直测它们）；**生产
 * 调用点已全部迁到 reconcile**——App.tsx 与 use-bridge-subscriptions.ts 只经
 * completion-observation.ts 的 observeSource/applyObservationBatch 进入（wiring 锁见
 * session-authority-wiring.test.ts），App/桥内不再有第二 planner。
 */
import {
...
import { completionWatermark, isWatermark, maxWatermarkValue, nextNotifiedWatermark, shouldNotifyWatermark } from './watermark.ts'
