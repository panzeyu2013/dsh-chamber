/**
 * 切源「无白帧」判据（W3 验收仪器 I7，2026-12）：对**逐帧观测**做三形态判定，
 * 取代「逐帧无空白」的口头判据（审计 §4-R7 明确：现有 switch-measure.mjs:87 只验
 * settle 下界、measure-ui.mjs:88-100 只记帧间隔，都不足以判白帧）。
 *
 * 三形态（蓝图 §7.1；任一命中即 FAIL）：
 ...
 *  另有第 4 条（蓝图 §7.3-3）：目标在切换前**已 settle**（温壳）时，切换窗口内不应出现
 *  任何进度面帧——温壳互切本就该同帧揭示。
 *
 ...
 * 照 `--require-hover` 先例）。
 *
 * 已知边界（写在判据里，不藏在报告里）：Leg B 的像素采样率受 screencast 帧率限制，
 * 静止期可能没有帧；因此 Leg A（DOM 逐帧）是 CI 主判据、Leg B 是实机像素证据。
 * 本判据不声称「内容区有 ink」——那是审计 §4-R7 的更强判据（与同主题稳定帧差分的
 * ink 阈值），需要基线帧，不在三形态内。
 */
