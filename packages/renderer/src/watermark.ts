/**
 * 水位（watermark）原语单一来源（2026-12 阶段 2 单源化）。
 *
 * WHY：同一套「host 域整数水位」契约此前在 unread-store / session-facts-source /
 * notification-dedupe 里各写一份 isWatermark，完成水位的 max / 单调记忆 / 递增判定
 * 也散在多处；归口后「什么样的数是合法水位」「什么算更高」只有一处答案。
 *
 ...
 * 启动/来源重挂时，基线里已经完成的会话不得补发通知（主计划 §5-5）。
 *
