/**
 * N-ctx 视图保留策略（对 design 05 §1/§4 的有意偏差）。
 *
 ...
 * 刷新其会话行与 running 位（design 05 §2.3）——侧栏仍可见该源的最新会话。
 * 已知取舍（见 App.tsx 回收点）：被回收壳内运行中任务的
...
export interface ReclaimDecisionInput {
   ...
   *  收自动预热壳（预热壳 settle 的 hiddenSince 恒晚于用户切走
...
export function decideReclaimCandidates(input: ReclaimDecisionInput): string[] {
  ...
  // 在途预热/收割壳不计入隐藏壳数：它此刻通常尚未 settle、
...
/**
 * 后台相位门控：文档隐藏（Electron 最小化/隐藏到托盘/后台
 * 标签语义）时不推进周期性/后台工作——空闲预热 boot、30s 聚合 watchdog 拉
 * 取与 reconnect、3s 失败重试、保留回收拆壳——这些在用户不可见期没有
