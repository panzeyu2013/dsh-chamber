/**
 * 视图调度簇（2026-12 阶段 3 从 App.tsx 平行移动）：预热资格 / 收割保温 /
...
import { forgetDegradedRetry } from '../degraded-retry.ts'
...
export interface ViewSchedulerDeps {
  ...
  // refs
  abandonedViewsRef: { current: Map<string, string> }
  activeViewRef: { current: string }
  autoPrewarmedRef: { current: Set<string> }
  ...
  prewarmSuppressedRef: { current: Set<string> }
