...
export function useBridgeSubscriptions(deps: BridgeSubscriptionsDeps): void {
  ...
  useEffect(() => {
    return chamberBridge.onRuntimeReport((sourceId, report, sourceFingerprint) => {
      ...
      // 重算与收敛读到的都是最新壳行，不依赖渲染期赋值（v5 §3.2）。
      // 单入口收敛（设计 19 + goal-aware v5 §3.2–§3.4）：壳行与当前 facts 快照在 observeSource
      // 里做权威合并（新鲜壳行优先 / 非 stale facts / unknown），候选（壳边沿、ask/request、
      // facts 水位）交唯一 reconcile；本处只接线 + emit。
        ...
        sink: {
          emitNotification: (notification, result) => {
            ...
            emitSessionNotification({
              ...
              ...(row?.updatedAt === undefined ? {} : { hostObservedAt: row.updatedAt }),
            })
          },
