...
/** 刷新需要兜底/刚重连的就绪实例；未就绪实例落 not-connected（已推送挂载来源除外）。 */
  ...
  // Staleness watchdog: the edge logic pulls only newly-ready or never-pushed sources,
  // so a mounted producer whose push channel silently dies (no withdrawal fires) would
  // stay stale forever. Every tick, pull any ready source whose last PUSHED snapshot is
  // older than the threshold; actively pushing sources are never pulled. The bounded
  // wave keeps quiet-fleet cost tiny and the signature dedup keeps state churn-free.
  // Render-phase mirror of aggregates for this interval: the timer must stay stable
  // across commits (re-creating it would stretch the cadence under activity).
  // remoteStatusRef above).
  const watchdogAggregatesRef = useRef(aggregates)
  watchdogAggregatesRef.current = aggregates
  // 运行位活性守卫：runtimeFacts 的 render-phase 镜像（timer 稳定，不因每次上报重建）。
