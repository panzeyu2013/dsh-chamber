...
- **文档级全局是已知风险面**：单一 renderer 文档下 `<html lang>`、body portal、document `drop` 等全局共享（逐条登记在 `docs/progress/STATUS.md`）。
- **vendor 补丁是构建期精确文本重写**（`scripts/vendor-patches.mjs`，7 补丁 / 21 锚点），永不写 vendor 源码；`verify-vendor-patch-applied` 在构建链上验证命中。
