 ...
 * 释放判定（原 shouldReleaseVeilForSurface）已迁到共享 arbiter
 * （@dsh-chamber/dsh-stream-state 的 decidePresentation），其真值表——电平释放、本次持有起点
 * 计窗、未 settle / 时钟未建立 / 回拨一律 fail-closed——现在由新包的 presentation 套件覆盖。
 ...
 *   同款 first-match 事故）；取不到锚点/祖先 ⇒ `absent`，取值未知 ⇒ `unknown`
 *   （与 absent 同走 2s 兜底，不再按 hero 的 70s 外层保险持有）。
 */
...
test('常量、属性字面量与相位→上界映射都是导出契约', () => {
  // 两个阈值已收归共享表的 presentation 段（P2），本 leaf 不再持有副本：
  // 单一所有者 + tables.json 锁步（tests/tables-parity.test.ts）。
  ...
  // 相位 → 上界的**唯一**映射已随  迁到共享 arbiter（surfaceBoundMs），其完整值域由
  // 新包 presentation 套件覆盖（absent/unknown 走 2s、hero/settling 走 70s、active 为 0）。
