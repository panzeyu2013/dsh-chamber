...
import { bootGapSignature, type ShellDegradedFact } from '../../src/boot-gap.ts'
import {
  FIBER_STATE,
...
test('bootInstanceShell: a graph-less boot keeps its cause over a late lower-priority probe verdict', async () => {
...
test('bootInstanceShell: pre-settle reports are LAST-wins under priority and a retraction clears the settled fact (FIX 1/FIX 3)', async () => {
