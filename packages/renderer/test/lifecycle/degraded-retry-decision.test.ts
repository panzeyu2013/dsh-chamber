/**
 * The self-heal DECISION lives in the registry.
 *
 * These cases pin the self-heal truth table against the registry. The App drives the
 * registry with exactly the event sequence below (phaseChanged per source, then
 * bootSettled for each degraded shell), so this IS the production path, not a parallel one.
 */
...
test('the mark of a retired source is dropped, not carried', () => {
  // Marks for sources the roster no longer lists are pruned; the registry
