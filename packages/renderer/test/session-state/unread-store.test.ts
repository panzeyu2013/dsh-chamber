/**
 * 未读 v4 落盘存储契约：键常量、宽松清洗（含 pending/outcomes）、
 * v2 → v4 一次性迁移（旧通知表 → 身份哨兵，新表原样携带）、单调 max 合并、
 * 读水位推进、有界化 LRU、client-install id、ack 请求、隐私键白名单、boot token。
 */
...
test('sanitize is lenient field-wise: bad entries are dropped, good ones survive', () => {
  const loaded = loadUnread(fakeStorage({
    [UNREAD_V4_KEY]: JSON.stringify({
      ...
      pending: { a: { s1: { watermark: 5, goalId: 'g1', at: 100 }, s2: { at: 'nope' }, s3: 7 } },
...
test('corrupt persisted identities are rejected at the restore boundary', () => {
  const loaded = loadUnread(fakeStorage({
    [UNREAD_V4_KEY]: JSON.stringify({
      v: 4, read: {}, edge: {}, pending: {}, outcomes: {},
      notifiedRuns: {
        a: {
          good: 'host:turn%2F7',
          badEncoded: 'host:%',
...
test('a corrupt v4 payload falls back to the v2 migration source, never to removed keys', () => {
  ...
  assert.deepEqual(loaded.read, { a: { s1: 9 } }, 'v2 is the only migration source')
  assert.ok(data.has(UNREAD_V4_KEY))
  assert.ok(!data.has(UNREAD_V2_KEY), 'the source journal is removed after the v4 write')
})

test('a v2 payload migrates to v4: old notification tables become identity sentinels; pending/outcomes carry over', () => {
  ...
  const removeIndex = calls.indexOf('remove:' + UNREAD_V2_KEY)
  assert.ok(setIndex !== -1 && removeIndex !== -1 && setIndex < removeIndex, 'v4 is written before v2 is removed')
...
test('a failing v2 -> v4 write keeps v2 for a later attempt', () => {
  const data = new Map<string, string>([[UNREAD_V2_KEY, JSON.stringify({ v: 2, read: { a: { s1: 9 } }, edge: {}, notified: {} })]])
  const storage: UnreadStorageLike = {
  ...
  const loaded = loadUnread(storage)
  assert.deepEqual(loaded.read, { a: { s1: 9 } })
  assert.ok(data.has(UNREAD_V2_KEY), 'the source journal survives a failed migration')
})

test('removed legacy keys are never a fallback: a v1/v3-only install starts empty', () => {
  const legacyOnly = fakeStorage({
    'dsh-chamber.unread.v3': JSON.stringify({ v: 3, read: { a: { s1: 1 } }, edge: {} }),
    'dsh-chamber.unread.v1': JSON.stringify({ a: { s1: true } }),
  })
  const loaded = loadUnread(legacyOnly.storage)
  assert.deepEqual(loaded.read, {})
  assert.deepEqual(loaded.edge, {})
})

test('saveUnread prunes empty tables and never throws', () => {
  const { storage, data } = fakeStorage()
  assert.equal(saveUnread(storage, { v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: {}, outcomes: {} }), true)
  assert.equal(
    data.get(UNREAD_V4_KEY),
    JSON.stringify({ v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: {}, outcomes: {} }),
    'saveUnread 自动带上 v5 表（§3.1）',
  )
  ...
  assert.equal(saveUnread(throwing, { v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: {}, outcomes: {} }), false)
})
...
test('P1: same-tick immediate requests coalesce into ONE save of the latest authoritative state', () => {
  const { storage, calls, data } = fakeStorage()
  let payload: UnreadV4Payload = { v: 4, read: { a: { s1: 1 } }, edge: {}, notifiedRuns: {}, pending: {}, outcomes: {} }
  ...
  queue.drain()
  assert.deepEqual(calls, ['set:' + UNREAD_V4_KEY], '同一 tick 三次 immediate 只落一次盘')
  // 合并落盘读的是**最新**权威内存：先到的 durable 变更不会被后到的覆盖丢。
  assert.deepEqual(JSON.parse(data.get(UNREAD_V4_KEY)!), {
    v: 4,
    read: { a: { s1: 1 } },
    edge: {},
  ...
  saves.request()
  queue.drain()
  assert.deepEqual(calls, ['set:' + UNREAD_V4_KEY, 'set:' + UNREAD_V4_KEY], '下一 tick 重新合并并落盘')
})
...
test('P1: critical-path flush lands synchronously, cancels the pending microtask, and never double-writes', async () => {
  ...
  saves.flush()
  assert.deepEqual(calls, ['set:' + UNREAD_V4_KEY], '关键路径同步落盘一次')
  assert.equal(saves.pending(), false, 'flush 取消待办')
  assert.deepEqual(JSON.parse(data.get(UNREAD_V4_KEY)!).read, { a: { s1: 9 } }, '写的是 flush 时刻的最新状态')
  await Promise.resolve()
  assert.deepEqual(calls, ['set:' + UNREAD_V4_KEY], '被取消的微任务不得二次写盘')
...
test('bounded LRU: per-source read keeps the highest watermarks only', () => {
  ...
  assert.deepEqual(pruneUnreadPayload(payload, 0), { v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: {}, outcomes: {} })
})
...
test('privacy whitelist: the serialized payload carries ids, watermarks and timestamps only', () => {
  ...
  // 键白名单：顶层只有五张表 + v；行内条目只允许水位/时间戳/id 字段
  // （sourceId/sessionId/goalId 是动态键，由上面的 forbidden 子串检查兜底）。
  const parsed = JSON.parse(raw) as Record<string, unknown>
  assert.deepEqual(Object.keys(parsed).sort(), ['edge', 'notifiedRuns', 'outcomes', 'pending', 'read', 'v'])
...
test('REGRESSION(A3-2): sanitizeUnreadPayload and createCompleteLedger agree on an invalid deferred (field-level)', () => {
  // 同一份**原始**载荷（未经清洗）分别经两条加载路径（未读 v4 清洗 / complete 账本
  ...
  const sanitized = sanitizeUnreadPayload({ v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: raw, outcomes: {} })
...
test('missing/corrupt pending/outcomes degrade to empty loudly and never throw', () => {
  ...
  try {
    const missing = loadUnread(fakeStorage({
      [UNREAD_V4_KEY]: JSON.stringify({ v: 4, read: {}, edge: {}, notifiedRuns: {} }),
    }).storage)
    ...
    const corrupt = loadUnread(fakeStorage({
      [UNREAD_V4_KEY]: JSON.stringify({ v: 4, read: {}, edge: {}, notifiedRuns: {}, pending: 'nope', outcomes: 42 }),
    }).storage)
