...
function local(
 ...
 bundleLines: string[] = [],
 rows: PluginRowShape[] = []): LocalFixture {
...
function remote(
 ...
 profileExists = true,
 error?: string,
 rows: PluginRowShape[] = []): RemoteFixture {
 return {
 dependencies,
 bundles,
 profileExists,
...
test('classifySpec: path classification is the shared wire ruler (design 21 decision 18)',() => {
 // The ONE definition (wire `isMaterializedValue`, consumed through
 // client-core): file:/link:, relative (./ ../ — including bare . / .. and
 // backslash forms), absolute (/ \\ C:\) and home-relative (~ ~/x ~\x).
 // : all four consumers (gateway / control-plane / desktop main /
 // client) now call this same ruler — the desktop main's old narrower
 // isMaterializeSpec was deleted in R6 phase 2, so no divergence window remains.
 for (const spec of ['./x', '../x', '~/x', '.', '..', '~', '~\\x', 'C:\\x', '\\\\srv\\share']) {
 assert.deepEqual(classifySpec(spec), { type: 'materialize' }, spec)
 }
 // A bare dot-name (`.foo`) is NOT a path, and `~1.2.3` is a tilde RANGE —
 // both stay non-materialize, never silently passed to `dsh plugin add`.
