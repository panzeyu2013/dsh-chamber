 ...
 * （method / draftKey / projectionKey），不引入布尔开关：bridge 判空 → clear-only 调用
 * （取值恒为 null，凭据永不进 renderer）→ 失败只置 formError，成功则清空该维度的草稿字段、
 * 把实例行与编辑目标的「已设置」投影翻回 false。
 */
...
/**
 * 清除一个凭据维度。bridge 或编辑目标缺失即 no-op；失败只置 formError（不改状态）；成功则
 * 清空该维度草稿字段、翻转「已设置」投影并清空 formError。
