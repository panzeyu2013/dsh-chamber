...
export interface OrderedApplyOps {
  ...
  adds: ModelPluginAdd[]
  defer: boolean
  /** Remove entries dropped by the NET rule (same name also added, so the remove would be a
   *  no-op preface — the re-add wins). Intra-group duplicates drop silently. */
  coalesced: string[]
}
...
/** The two failure regimes: registryAndRemove = 'fail-fast' at its submission/refusal
 *  boundary — a wholesale refusal aborts the whole batch; it does NOT describe ssh
 *  execution-time row failures, which are serially isolated (executed+partial).
 *  materializeRows = 'isolated' in every phase. */
